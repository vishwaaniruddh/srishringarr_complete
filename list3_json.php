 ...
 if($pick_date!='' && $delivery_date!='' && $booking_status !='Returned') {
    ...
    //  $booking_date ='' ; 
        ...
        // $booking_date = '<div><span style="color:red;">Booking Status Dates</span> <br>'  .  date("d-m-Y", strtotime($pick_date)) .' - '. date("d-m-Y", strtotime($delivery_date)) . '</div>';
         $booking_date = '<span style="color:red;">Booking Status Dates</span> '  .  date("d-m-Y", strtotime($pick_date)) .' - '. date("d-m-Y", strtotime($delivery_date)) . '';
...
// }
    ...
    $data[] = ['snno'=>$i,'product_name'=>$product_name,'selling_price'=>$rero[0],'rent_price'=>$rentprice,'image'=>$destination_img,'sku'=>$prcode,'deposite'=>$final_deposit,'discount'=>$discount,'link'=>$link,'statement'=>'1', 'booking'=>$booking_date,'imageframe'=>$imgframe];                        
    // $data[] = ['snno'=>$i,'product_name'=>$product_name,'selling_price'=>$rero[0],'rent_price'=>$rentprice,'sku'=>$prcode,'deposite'=>$final_deposit,'discount'=>$discount,'link'=>$link,'statement'=>'1'];                        
