...
function getmonthdetails1($month){
    $my=$month;
        ...
        return $cnt10."_".$cnt30."_".$cnt1."_".$cnteg."_".$ot."_".$bt."_".$absent."_".$hw."_".$roundsal;

}

function getmonthdetails2($month){
    $my = $month;
...
function GeneratePayroll(){
	...
	global $db;
	$requestData = $_REQUEST;
	$month = $_POST['activity'];
	
	$sql = mysqli_query($db, "select * from employee order by ssn asc");
    ...
    $fname = $row[8];
        ...
        if ($row[10] > 3 and $row[10] != 9) {
            $getmonthdetails1 = getmonthdetails1($month); 
            ...
            $salary = $getmonthdetails1_arr[8];
        } else if ($row[10] == 9) {
            $getmonthdetails2 = getmonthdetails2($month);
