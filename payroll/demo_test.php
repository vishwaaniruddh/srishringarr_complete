...
function datetimeval($id,$date){
    global $db;
    $in_out = array();
    $sql = mysqli_query($db,"select * from raw_attendance_test where emp_id='".$id."' and cast(attendance_date as date)='".$date."'");
...
<body class="hold-transition skin-blue sidebar-mini">
  ...
  crossorigin="anonymous"></script>
      ...
      $(document).ready(function() {
		...
		"processing": true,
		"serverSide": true,
		"ajax": "<?php echo BASE_URL; ?>json/view_attendance_test.php",
		"dom" : 'Bfrtip',
		"buttons": [
        			    ...
        			    <div class="box">
        			        <div class="box-body">
                                   ...
                                   // date_default_timezone_set('Asia/Kolkata');   
                                    ...
                                    for ($i = 0; $i <= $row; $i++) {
                                        ...
                                        if(isset($id) && $id!='' ){
                                            ...
                                            $check_sql = mysqli_query($db,"select * from raw_attendance_test where emp_id='".$id."' and attendance_date='".$datetime."' and status=1");
                                            if( $check_sql_result = mysqli_fetch_assoc($check_sql)){}
                                            else{
                                                $sql = "insert into raw_attendance_test(emp_id,attendance_date,status,created_at)
                                ...
                                foreach($in_ar as $key => $val){
                                    ...
                                    $sql = "insert into new_attendance_test(emp_id,attendance_date_in,attendance_date_out,status,created_at) values('".$emp_id."','".$in."','".$out."',1,'".$created_at."')";
                                    mysqli_query($db,$sql);
