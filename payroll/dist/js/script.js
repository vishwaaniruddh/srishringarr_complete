 /* Generate Payroll Script Start */
    if ( $('#filter').length > 0 ) {
        $('#filter').on('submit', function(e) { 
            ...
            //     async    : true,
            //     cache    : false,
            //     url      : baseurl + "ajax/?case=GeneratePayroll",
            //     data     : form.serialize(),
            ...
            var table = $('#generate_payroll').DataTable({ 
                            "processing": true,
                            "serverSide": true,
                            "ajax": baseurl + "ajax/?case=GeneratePayroll&month="+month,
                            "order": [0, 'desc'],
                            "columnDefs": [{
                                "targets": 0,
                                "className": "dt-center"
    ...
    /* End of Script */
     ...
     /* Generate Payroll Script Start */
    if ( $('#manual_attendance').length > 0 ) {
        $('#manual_attendance').on('submit', function(e) { //alert("y");
            e.preventDefault();
            ...
            $.ajax({
                ...
                async    : true,
                cache    : false,
                url      : baseurl + "ajax/?case=GenerateManualAttendance",
                data     : form.serialize(),
                success  : function(result) {
                    if ( result.code === 0 ) {
                        // window.location.reload();
    ...
    /* End of Script */
      ...
      /* Generate Payroll Script Start */
    if ( $('#manual_salary').length > 0 ) {
        $('#manual_salary').on('submit', function(e) { //alert("y");
            e.preventDefault();
            ...
            $.ajax({
                ...
                async    : true,
                cache    : false,
                url      : baseurl + "ajax/?case=GenerateManualSalary",
                data     : form.serialize(),
                success  : function(result) {
                    if ( result.code === 0 ) {
                        // window.location.reload();
    ...
    /* Add Employee Script Start*/
    if ( $('#add-employee-form').length > 0 ) {
        $('#add-employee-form').on('submit', function(e) {
            ...
            $.ajax({
                ...
                async    : true,
                cache    : false,
                url      : baseurl + "ajax/?case=AddEmployee",
                data     : form.serialize(),
                success  : function(result) {
                    if ( result.code == 0 ) {
                        $.notify({
...
$(document).ready(function() {
    /* Attendance Table Script Start */
    if ( $('#bulk_upload_attendance').length > 0 ) {
        var att_bulk_table = $('#bulk_upload_attendance').DataTable({ 
            "processing": true,
            "serverSide": true,
            "ajax": baseurl + "ajax/?case=LoadingBulkUploadAttendance",
            "order": [0, 'desc'],
            "columnDefs": [{
                "targets": 0,
                "className": "dt-center"
    ...
    if ( $('#bulk_upload_attendance1').length > 0 ) {
        var att_bulk_table = $('#bulk_upload_attendance1').DataTable({ 
            "processing": true,
            "serverSide": true,
            "ajax": baseurl + "ajax/?case=LoadingBulkUploadAttendance1",
            "order": [0, 'desc'],
            "columnDefs": [{
                "targets": 0,
                "className": "dt-center"
