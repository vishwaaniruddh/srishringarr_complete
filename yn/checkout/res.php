...
function get_cart_discount($cartid){
    global $con;
    $sql=mysqli_query($con,"select * from cart where active=1 and cart_id='".$cartid."' and ac_typ=2");
    $sql_result=mysqli_fetch_assoc($sql);
    return $sql_result['discount'];
}

function get_cart_mrp($cartid){
    global $con;
    $sql=mysqli_query($con,"select * from cart where active=1 and cart_id='".$cartid."' and ac_typ=2");
    $sql_result=mysqli_fetch_assoc($sql);
    return $sql_result['mrp'];
}
...
function delete_from_cart($userid){
    ...
    $delete="update cart set active=0 where user_id='".$userid."' and ac_typ=2";
...
if($razor_status=='captured'){
    ...
    try{
        ...
        $sql  = "update Order_ent set amount ='".$amount."',status=0, razorpay_payment_id = '".$razorpay_payment_id."',date = '".$datetime."',delivery_add='".$delivery_add."' ,  
        ...
        foreach($crtids as $key => $val) {
            ...
            $cart_sql = mysqli_query($con,"select * from cart where active=1 and cart_id='".$val."'");
            $cart_sql_result = mysqli_fetch_assoc($cart_sql);
            
            $totalMrp = $cart_sql_result['mrp'];
            $qty = $cart_sql_result['qty'];
            $mrp = $totalMrp / $qty ;
            
            $product_image = $cart_sql_result['image']; 
            $deposit_date = $cart_sql_result['deposite_date'];
            $total_amount_ = $mrp * $qty;
            $discount_amount = $cart_sql_result['discount'];
            
           $insert_order_details = "insert into order_details(order_id,cart_id,user_id,product_type,product_id,qty,product_amt,total_amt,date,status,ac_typ,custom_data,rent_dt,return_dt,deposit_amt,image,discount_amount,mrp) 
           VALUES 
           ('".$id."','".$val."','".$userid."','".get_product_type_from_cart($val,$userid)."','".get_product_from_cart_by_cartid($val)."','".$qty."','".$product_amt."','".$total_amount_."','".date('Y/m/d h:i:s')."','1','2','".$custom_data."','".$rental_date[0]."','".$rental_date[1]."','".$rental_date[2]."','".$product_image."','".$discount_amount."','".$mrp."')";
    ...
    if($people_sqlresult = mysqli_fetch_assoc($people_sql)){
        ...
        $approval_sql = "insert into approval(cust_id,bill_date,status,paid_amount,transaction_id,bill_by,typ) 
                values('".$userid."','".$date."','S','".$amount."','".$txnid."','online','2')"; 
               if(mysqli_query($con3,$approval_sql)){
                   $approval_id = $con3->insert_id;
                    ...
                    foreach($crtids as $key => $val){
                        ...
                        $dis_amount = get_cart_discount($val) ;
                        $mrp = get_cart_mrp($val);
                        $approval_detail = "insert into approval_detail(bill_id,item_id,qty,amount,final_amount,dis_amount)
                        values('".$approval_id."','".$sku."','".$qty."','".$total_amount_."','".$total_amount_."','".$dis_amount."')";
...
<html lang="en">
														...
														<td valign="top" style="padding:48px 48px 32px">
															<div id="m_3716871841364271743body_content_inner" style="color:#636363;font-family:&quot;Helvetica Neue&quot;,Helvetica,Roboto,Arial,sans-serif;font-size:14px;line-height:150%;text-align:left">
																	...
																	<thead><tr>
																					...
																					<th scope="col" style="color:#636363;border:1px solid #e5e5e5;vertical-align:middle;padding:12px;text-align:left">SKU</th>
    ...
    $subject  = "Thanks You For Shopping At Yosshitaneha";
        $data = array(
        'subject' => $subject,
        'to' => $to,
        'message' => $link,
                                        ...
                                        <table class="table">
                                            ...
                                            <tbody>
                                                <?php
                                                        ...
                                                        $total_amt=$sql_result['product_amt'];
                                                        $qty=$sql_result['qty'];
                                                        $name = get_sku($product_id,$type);
                                                        $image_url = $sql_result['image'];
                                                ...
                                                <tr class="woocommerce-table__line-item order_item">
                                                    ...
                                                    <td class="woocommerce-table__product-total product-total">
                                                        <span class="woocommerce-Price-amount amount"><span
                                                                class="woocommerce-Price-currencySymbol">₹</span>
                                                            <? echo $thisFinalAmount = $qty * $total_amt; ?>
                                                        </span>
                                                    </td>
                                                </tr>
                                                <?php 
                                                    
                                                    $overall_amount = $overall_amount+$thisFinalAmount; 
                                                    } ?>
                                            </tbody>
