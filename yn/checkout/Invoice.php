...
function get_shippingaddress_pdf($id,$param){
    global $con;
    

    $sql = mysqli_query($con,"select * from shippingInfo where userid='".$id."'");
    ...
    if($param==1){
        return  $person_name .  ', ' .$person_contact .', ';        
    }else if($param ==2){
...
$_address = $user_sql_result['address'];
$contact_number = $user_sql_result['Mobile'];

$name = $fname.' '.$lname;

// $net_amt = $total_amount + $shipping_charges;
...
$pdf->Cell(94,5,'MANUFACTURERS AND RETAILERS OF BRIDAL SETS,','0',1,'L',1);//end of line

// Image(string file [, float x [, float y [, float w [, float h [, string type [, mixed link]]]]]])
$pdf->SetFont('Arial','B',8);
$pdf->Cell(94,5,'HAIR ACCESSORIES AND BROOCHES,','0',0,'L',0); 
$pdf->Cell(75, 5, $pdf->Image('yoss.jpeg',145,25,50,25),0, 1, 'L', false );
//end of line
// $pdf->Cell(25,,'HAIR ACCESSORIES ,','0',0,'R',0);
...
$pdf->Ln(1);
$pdf->SetFont('Arial','B',9);
$pdf->Cell(95,5,'Address : '.$address,'0',0,'L');
$pdf->Cell(95,5,'Bill No : '.$order_id,'0',1,'R');
$pdf->Ln(1);

$pdf->SetFont('Arial','B',9);
$pdf->Cell(63,4,''.get_shippingaddress_pdf($userid,1),'0',1,'L'); //end of line
$pdf->Cell(63,4,''.get_shippingaddress_pdf($userid,2),'0',1,'L'); //end of line
$pdf->Cell(64,4,''.get_shippingaddress_pdf($userid,3),'0',1,'L'); //end of line
$pdf->Ln(1);


$pdf->SetFont('Arial','B',9);
$pdf->Cell(94,5,'Contact No : '. $contact_number,'0',1,'L');//end of line
$pdf->Ln(5);
...
$pdf->Cell(22,10,'Amount','1',0,'C');
$pdf->Cell(22,10,'Discount','1',0,'C');
$pdf->Cell(30,10,'Discount per Item','1',0,'C');
$pdf->Cell(30,10,'Total Amount','1',1,'C');//end of line


$pdf->SetFont('Arial','B',9);


// echo "select * from order_details where order_id = '".$order_id."'" ; 

$pro_sql = mysqli_query($con,"select * from order_details where order_id = '".$order_id."' and status=1");
$i = 1; 
$totalFrossAmount = 0 ;
$totalQty = 0 ; 
while($pro_sql_result = mysqli_fetch_assoc($pro_sql)){
    
    $pro_amount = $pro_sql_result['mrp'];
    $pro_qty = $pro_sql_result['qty'];
    $pro_type = $pro_sql_result['product_type'];
    $product_id = $pro_sql_result['product_id'];
    $discount_amount = $pro_sql_result['discount_amount'];
    ...
    $product_amount = $pro_sql_result['product_amt'];
// $gross_amt = gross_amount($pro_amount);
$t_amt = $product_amount * $pro_qty;
$price = $t_amt - $discount_amount;
...
$totalDiscountAmount = $discount_amount * $pro_qty ; 

$totalAmountWithMultiply = $pro_amount*$pro_qty ; 


$percentageDiscount = ($discount_amount*$pro_qty)*100/$totalAmountWithMultiply .'%'; 

$totalAmount = $totalAmountWithMultiply - $totalDiscountAmount ; 

$pdf->Cell(13,6,$i,'1',0,'C');
$pdf->Cell(25,6,$product_name,'1',0,'C');
$pdf->Cell(25,6,$pro_amount,'1',0,'C');
$pdf->Cell(22,6,$pro_qty,'1',0,'C');
$pdf->Cell(22,6,$totalAmountWithMultiply,'1',0,'C');
$pdf->Cell(22,6,$percentageDiscount,'1',0,'C');
$pdf->Cell(30,6,'Rs. '.$discount_amount,'1',0,'C');
$gross_amt = gross_amount($pro_amount);
$pdf->Cell(30,6,'Rs. '.$totalAmount .'/-','1',1,'R');//end of line    
$i++;

$totalFrossAmount = $totalFrossAmount+ $totalAmount ;
$totalQty = $totalQty + $pro_qty ; 
}
...
$pdf->Cell(30,6,'Rs.  '.$shipping_charges_curr.'.00/-','1',1,'R');//end of line    
}
...
$pdf->SetFont('Arial','B',9);
$pdf->Cell(63,6,'Total Qty : '.$totalQty,'1',0,'R');
$pdf->Cell(66,6,'Gross Total:'.'Rs. '.$sum ,'1',0,'R');
$pdf->Cell(30,6,'Paid Amount','1',0,'R');
$pdf->Cell(30,6,'Rs. '.$totalFrossAmount.'.00/-','1',1,'R');//end of line
