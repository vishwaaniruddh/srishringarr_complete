...
$typ=$_POST['type']; /* if 1 its jewel 2 is garment */
...
$fianlprice=$_POST['price']; 

$actyp=1; /* if 1 its rent 2 is sell */ 

//$deposit=$_POST['deposit'];
 
//$orgdeposit=$_POST['orgdeposit'];

$orgdeposit=$_POST['deposit'];
...
//if(isset($_SESSION['gid']) & $_SESSION['gid']!="" & $_SESSION['gid']>0) {
    ...
    if($_POST['rent_date']!="")
    {
        $dt=date("Y-m-d",strtotime(str_replace("/","-",$_POST['rent_date'])));
    }
    
    if($_POST['return_date']!="")
    {
        $retdt=date("Y-m-d",strtotime(str_replace("/","-",$_POST['return_date'])));
    }
