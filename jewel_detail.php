...
$type  = 1;
if($productName){
    $getsql = mysqli_query($con,"select * from product where newProductName='".$productName."'");
    $getsqlResult = mysqli_fetch_assoc($getsql);
    $id = $getsqlResult['product_id'];
    $product_id = $prid = $id;    
}else{
    $id = $_REQUEST['id'];
    $product_id = $prid = $id;
...
$categorysql = mysqli_query($custom_con,"select * from product where product_id='".$product_id."'");
$categorysqlResult = mysqli_fetch_assoc($categorysql) ; 

$category_name = $categorysqlResult['name'];
$meta_title = $categorysqlResult['meta_title'] ? $categorysqlResult['meta_title'] : $categorysqlResult['product_name'];
$meta_keywords = $categorysqlResult['meta_keywords'];
$meta_description = $categorysqlResult['meta_description'] ? $categorysqlResult['meta_description'] : $categorysqlResult['product_desc'];





$a="SELECT * FROM `product_images_new` WHERE `product_id`='$prid' order by rank";
$ab=mysqli_query($con,$a);
$abc=mysqli_fetch_array($ab);

// $ogImage =  'https://www.srishringarr.com/'. $abc['comimage'] ; 
...
$period    = new DatePeriod($dateStart, $interval, $dateEnd);
    ...
    youtube,CAST(REGEXP_SUBSTR(product_code,'[0-9]+') AS UNSIGNED) as sku,ss_seo_keywords from `product` where product_id='".$product_id."' order by sku desc";
...
// When the exit button i<s clicked
							...
							<div class="">
							    ...
							    <div class="" style="border-color:#E6BE6E;">
    								<div class="bo1 input-container lengthdate" style="border-color:#353746;">
								        ...
								        <input type="text" name="daterange" id="dateRangeP" value="<?php echo $start_date . ' - ' . $end?>"  
								        style="text-align: left; width: 100%;padding-left: 3%;">
    								</div>
							    </div>
						    </div>
...
$sellingPrice = round(currencyAmount($currency,$rero[0]),2) * 0.5 ; 


?>
<input type="hidden" id="sellingPrice" name="sellingPrice" value="<?= $sellingPrice; ?>" />
...
$(document).on('click', '.lefty', function() {
  ...
  // API endpoint URL
  var apiUrl = "/getJewelProductsSameCategory.php?id=<? echo $category; ?>";
...
$(document).on('click', '.righty', function() {
  ...
  $.ajax({
    ...
    $(document).ready(function() {
      ...
      $.ajax({
        url: '/like_jewel.php?id=<? echo $category; ?>',
        dataType: 'json',
  ...
  $.ajax({
    url: '/history_jewel.php?sku=<? echo $sku; ?>', // Update the URL with the correct endpoint
    dataType: 'json',
