...
$t1=  mysqli_query($con,"update approval set paid_amount=paid_amount+$amt where cust_id='$cid' and bill_id='".$bill_id."' LIMIT 1");
$t2=mysqli_query($con,"insert into `paid_amount`(bill_id,amount,return_date,payment_by,bid) values('$cid','$amt',STR_TO_DATE('".$amt_date."','%d/%m/%Y'),'$pay_by','".$bill_id."')");
...
VALUES ('','".$acc."','receit','".$amt."',STR_TO_DATE('".$amt_date."','%d/%m/%Y'),'payment from customer $row[0] $row[1]','NO',now() ,'".$bill_id."' )");
