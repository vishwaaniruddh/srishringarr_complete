 ...
 <?php 
      ...
      /*   $result1 = mysqli_query($con,"SELECT sum(qty),sum(return_qty) FROM  approval_detail where item_id='$row[0]' and not qty=0");
         $sum=0;
			  ...
			  //echo "SELECT sum(qty),sum(return_qty) from approval_detail where `bill_id` in (SELECT bill_id FROM approval where bill_date>'2014-03-31' and status='S' or status='A')  and   not qty=0 and item_id='$row[0]'";
			 // echo  "<br>";
         $result2 = mysqli_query($con,"SELECT sum(qty),sum(return_qty) from approval_detail where bill_id in (SELECT bill_id FROM approval where bill_date>'2014-03-31' and status='S' or status='A')  and not qty=0 and item_id='$row[0]'");
