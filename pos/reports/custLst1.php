...
$qry="SELECT * FROM `phppos_people` WHERE `person_id` not in (SELECT `person_id` FROM `phppos_suppliers` ) and `first_name`!=''  ORDER BY `phppos_people`.`first_name` ASC";
...
Customer List<br/>
<a href="people.php?mode=customerList" target="_new">New Customer</a>
<br/></td></tr>
<tr><td>
...
<td width="78">
    ...
    <a href="/pos/reports/custDel.php?id=<?php echo $row[11]; ?>" onclick="return confirm('Are you sure??')" style="font-size:18px;font-weight:bold;">Delete</a>&nbsp;&nbsp;&nbsp;<br/>

    <a href="/pos/reports/custEdit.php?id=<?php echo $row[11]; ?>" target="_new" style="font-size:18px;font-weight:bold;">Edit</a></td>
 </tr>
<?php $i++;   } ?>			
</table>
