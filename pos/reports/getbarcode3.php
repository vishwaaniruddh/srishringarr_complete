...
<?php
 ...
 $sq="SELECT item_id,sum(qty),sum(return_qty),aid FROM `approval_detail` WHERE item_id='$barcode' and bill_id in (select bill_id from approval where cust_id='$cid' and status='S') group by item_id";
 
 }else if($it==""){
///echo "2";
 $barcode2=$_GET['barcode2'];
 $qry=mysqli_query($con,"select name from  phppos_items where item_number='$barcode2' and is_deleted = 0");
...
$sq="SELECT * FROM phppos_items WHERE name='$row2[0]' and is_deleted = 0";
