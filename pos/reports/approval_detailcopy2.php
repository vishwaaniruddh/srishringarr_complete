...
mysqli_query($con,"BEGIN;");
	if($myflag){
	$t1=mysqli_query($con,"insert into `approval`(cust_id,bill_date,status,paid_amount,discount_status,discount_wise,discount_per,bill_by,pay_by,note,typ,statetyp) values('$cid',STR_TO_DATE('".$odate."','%d/%m/%Y'),'$bs','$amt','$dis_st','$wise','$disper','$by','$pay_by','$note',1,'".$_POST['statetyp']."')");
...
$t4=mysqli_query($con,"insert into approval_detail(bill_id,item_id,qty,discount,dis_amount,amount,item_per,`gstpec`, `sgstamt`, `cgstamt`, `igstamt`, `typ`,sgstperc,cgstperc,igstperc) values('$rowordno[0]','$design[$i]','$prz[$i]','$ds','$p','$sum','$itemper','".$_POST['gstrate'][$i]."','".$_POST['sgstamt'][$i]."','".$_POST['cgstamt'][$i]."','".$_POST['igstamt'][$i]."',1,'".$_POST['sgstrate'][$i]."','".$_POST['cgstrate'][$i]."','".$_POST['igstrate'][$i]."')");
