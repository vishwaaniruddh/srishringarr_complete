...
$t1=mysqli_query($con,"insert into `paid_amount`(bill_id,amount,return_date,payment_by) values('$cid','$amt',STR_TO_DATE('".$bill_date."','%d/%m/%Y'),'$radio2')");
}
///echo "update approval set return_date=STR_TO_DATE('".$bill_date."','%d/%m/%Y'),paid_amount=paid_amount+$amt where bill_id='$cid'";


$sq3=mysqli_query($con,"select * from approval where cust_id='$cid' and status='A'  Order By bill_id DESC");
...
$sq=mysqli_query($con,"SELECT bill_id,qty,return_qty,aid,amount FROM `approval_detail` WHERE item_id='$it[$i]' and  bill_id in (select bill_id from approval where cust_id='$cid' and status='A' ) Order By bill_id DESC");
...
$perpiece=(float)$sqrow[4]/(float)$sqrow[1];
$retamt=(float)$perpiece*(float)$sum;
 ...
 ?>
  ...
  <td><font size="2" ><?php echo $j++; ?></font></td>
    ...
    <td align="center"><font size="2" ><?php echo $sum; ?></font></td>
     <td align="center"><font size="2" ><?php echo (float)$perpiece*(float)$sum; $totr=(float)$totr+(float)$perpiece*(float)$sum; ?></font></td>
...
$sql = mysqli_query($con,"select * from phppos_items where name='".$it[$i]."'");
...
mysqli_query($con,"insert into `paid_amount`(bill_id,amount,return_date,payment_by) values('$cid','$amt',STR_TO_DATE('".$bill_date."','%d/%m/%Y'),'$radio2')");
}
///echo "update approval set return_date=STR_TO_DATE('".$bill_date."','%d/%m/%Y'),paid_amount=paid_amount+$amt where bill_id='$cid'";
...
//echo "update approval set return_date=STR_TO_DATE('".$bill_date."','%d/%m/%Y'),paid_amount=paid_amount+$sum1 where bill_id='$sqrow3[0]'";
    ...
    mysqli_query($con,"update approval set paid_amount=paid_amount+$sum1 where bill_id='$sqrow3[0]'");
   mysqli_query($con,"update approval set `return_date` = STR_TO_DATE('".$bill_date."','%d/%m/%Y') where bill_id='$sqrow3[0]'"); 
...
//////////////echo $aa."/".$sqrow3[0]."final sale ><br/>";
     ...
     mysqli_query($con,"update approval set paid_amount=paid_amount+$aa where bill_id='$sqrow3[0]'");
   mysqli_query($con,"update approval set `return_date` = STR_TO_DATE('".$bill_date."','%d/%m/%Y') where bill_id='$sqrow3[0]'");
...
{
$ab=0;
$sq=mysqli_query($con,"SELECT bill_id,qty,return_qty,aid FROM `approval_detail` WHERE item_id='$it[$i]' and  bill_id in (select bill_id from approval where cust_id='$cid' and status='A' ) Order By bill_id DESC");

$sum=$prz[$i];

while($sqrow=mysqli_fetch_row($sq)){
...
if($sum==0){}else{
		...
		$ab=$sqrow[0];
  mysqli_query($con,"update approval_detail set return_qty=return_qty+$sum where aid='".$sqrow[3]."'");
  mysqli_query($con,"insert into `return_qty`(bill_id,qty,return_date,item_code) values('$sqrow[0]','$sum',STR_TO_DATE('".$bill_date."','%d/%m/%Y'),'$it[$i]')");
 $bdate1=mysqli_query($con,"select * from approval_detail where bill_id='$sqrow[0]' and item_id='$it[$i]'");
	$blrow1=mysqli_fetch_row($bdate1);
	$ramt= $blrow1[7];
	$am=$ramt/$sum;
...
if($bal==0){}else{
		...
		$ab=$sqrow[0];
 mysqli_query($con,"update approval_detail set return_qty=return_qty+$bal where aid='".$sqrow[3]."'");
  mysqli_query($con,"insert into `return_qty`(bill_id,qty,return_date,item_code) values('$sqrow[0]','$bal',STR_TO_DATE('".$bill_date."','%d/%m/%Y'),'$it[$i]')");
...
///echo $ab."UP<BR/>";
  ...
  mysqli_query($con,"update approval set status='S' where bill_id='$ab'");
