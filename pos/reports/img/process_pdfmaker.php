...
$pdf->Cell(0, 7, 'Sri Shringarr Fashion Studio', 0, 1, 'C');
...
foreach($_POST as $radioName => $selectedValue){
    $type=2;
        ...
        $sku = $nameParts[0];
        $product_id = $nameParts[1];
        $selectedImage = explode('-',$selectedValue)[2];
        $sqlimg = "SELECT img_name FROM `product_images_new` WHERE `gproduct_id`='$product_id'" ;   
        
        if(isset($selectedImage) && !empty($selectedImage)){
            $sqlimg.= " and img_name like '%".$selectedImage."%'";
        }
...
// echo $sqlimg ;
    ...
    $source_img = trim("https://srishringarr.com/" . $rowimg[0]);
    ...
    $x = ($i % 2 == 1) ? 0.5 : 5.1;
    $y = ($i % 2 == 1) ? $pdf->GetY() : ($pdf->GetY() - 8);

    $pdf->Image($destination_img, $x, $y, 4.4);
    $pdf->Link($x, $y, 4.4, 8, 'https://srishringarr.com/'.$link);


$pdf->SetTextColor(75, 73,172,1);

    // Add SKU below the image:
    $pdf->SetXY($x, $y + 6); // Move below the image with a 6-unit gap
    $pdf->Cell(4.4, 0, 'View : '. $sku, 0, 0, 'C'); // Center the SKU within the image width
...
$pdf->Output($pdfName.'.pdf', 'I'); // 'I' indicates inline display, you can change it to 'D' for download
