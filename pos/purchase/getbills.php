...
$gtitd=mysqli_query($con,"SELECT item_id FROM `phppos_items` WHERE `name`='".$_GET['itnm']."'");
$gtrws=mysqli_fetch_array($gtitd);

	   $qrytrans.=	" and pur_id in(select pur_id FROM `phppos_purchase_details` where item_id='".$gtrws[0]."')";
...
<tr id="disname"><td colspan="13" align="center"> <strong>Supplier Name : <?php echo $suppname[0];?></strong></td></tr>
<?php //}?>
...
<th width="23%" colspan="2">Options</th></tr>
...
while($restrans=mysqli_fetch_row($trans))
{
	...
	// echo "select company_name from `phppos_suppliers` where `person_id`= $restrans[2]";
	
	   $suup_name=mysqli_query($con,"select `company_name` from `phppos_suppliers` where `person_id`= '$restrans[2]'");
	   $supp_name=mysqli_fetch_row($suup_name);
	
	
	  echo $supp_name[0]; ?></td><?php } ?><!--Supplier name-->
    ...
    <td align="center"><?php echo date('d/m/Y',strtotime($restrans[3]));?></td>
        ...
        <td><a href="Timeline/bill_his.php?sup_id=<?php echo $restrans[2];?>&bill_id=<?php echo $restrans[0];?>&link=detail" target="_blank"><input type="button" value="View Bill"></a></td>

    
    <?php if($type=='0' or $type=='2'){ ?>
    <td><a href="#" onClick="newWin('edit_bill.php?bill_id=<?php echo $restrans[0];?>');"><input type="button" value="Edit Bill"></a></td>
    <?php  } ?>
...
{ echo "6"; }else echo "5"; ?>" align="right"> Total</td><td align="right"><?php echo $ttlpur;?></td> <td align="right">   </td><td align="right"><?php echo $tnetamt;?> </td><td align="right"> <?php echo $ttoutstanding;?>  </td><td colspan="8"></td></tr>




<tr id="hide"><td colspan="5" align="center"> Amt : </td>
<td align="right"><input type="text" name="payamt" readonly="readonly" id="payamt" align="right"></td>
<td colspan="7"><input type="submit" value="Pay Amount" name="paybtn" id="paybtn"></td>
</table>
<?php }
