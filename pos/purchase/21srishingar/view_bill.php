...
<td width="1308"  valign="top"><center>
    ...
    <hr>
	 ...
	 ?>
     <div id="back">
     <table width="70%" border='1'><tr><th width="10%">Sr. No</th><th width="30%">Item Name</th><th width="15%">Price</th><th width="20%">Quantity</th><th width="25%">Total Amount</th></tr>
     <?php while($resfetch=mysqli_fetch_row($qryfetch)){?>
     <tr>
     <td align="center"><?php echo $i++;?></td><td align="justify"><?php $itemqry=mysqli_query($con,"SELECT `name` from `phppos_items` where `item_id`='$resfetch[2]'"); $item=mysqli_fetch_row($itemqry);
	 echo $item[0];//Item Name?></td><td align="right"><?php echo $resfetch[4];//Price ?></td><td align="right"><?php echo $resfetch[3];$qty+=$resfetch[3]//quantity?></td><td align="right"><?php echo $res=($resfetch[3]*$resfetch[4]); $total+=$res;?></td>
     </tr>
     <?php }
