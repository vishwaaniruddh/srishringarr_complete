...
for($i=0;$i<count($billno);$i++)
{
	...
	if($resbill[6]<=$amt)
	{
		...
		$insqry[$i]=mysqli_query($con,"INSERT INTO `phppos_purchase_payments`(`trans_id`, `bill_no`, `mode`, `amt`, `paid_date`) VALUES ('','$resbill[0]','$mode','$resbill[6]',STR_TO_DATE('".$paydate."','%d/%m/%Y'))");
		echo $tr=mysqli_insert_id($con);
		if($insqry)
		{
			$amt-=$resbill[6];
	...
	else if($resbill[6]>$amt)
	{
		...
		$insqry[$i]=mysqli_query($con,"INSERT INTO `phppos_purchase_payments`(`trans_id`, `bill_no`, `mode`, `amt`, `paid_date`) VALUES ('','$resbill[0]','$mode','$amt',STR_TO_DATE('".$paydate."','%d/%m/%Y'))");
	    $tr=mysqli_insert_id($con);
		if($insqry)
		{
			$resbill[6]-=$amt;
...
mysqli_query($con,"INSERT INTO `bank_transaction`(`trans_id`, `bank_id`, `trans_type`, `trans_amt`, `trans_date`, `trans_memo`, `reconcile`,`enrty_date`) VALUES ('','$acc','payment','".$amt1111."',STR_TO_DATE('".$paydate."','%d/%m/%Y'),'payment for supplier ".$supp."','NO',now())");
 ?>
