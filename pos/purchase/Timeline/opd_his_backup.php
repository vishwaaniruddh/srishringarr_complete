...
<tr><td width="712">
<input type="button" style="width:100px; height:30px; color:#ac0404;" 
<?php if($link=='detail') { ?>onClick="javascript:location.href = '../patient_detail.php?id=<?php echo $id; ?>';"<?php } 
if($link=='detail1') { ?>onClick="javascript:location.href = '../view_patient.php';" <?php } 
else{  ?>onClick="javascript:location.href = '../view_opd.php';"<?php } ?> value="Go Back" />
...
<td width="176"><h1>History </h1></td></tr></table>
</center>
...
<div id="timeline">
	<ul id="dates">
		...
		$supp_idc=mysql_query("Select supp_id from phppos_purchase where `pur_id`='".$_GET['bill_id']."' ");
	    $supp_idd=mysql_fetch_array($supp_idc);     
        
		echo $check2 ="select bill_id from phppos_purchase where supp_id='".$supp_idd[0]."' ";		
		$bill_idall=mysql_query("select bill_id from phppos_purchase where supp_id='".$supp_idd[0]."' ");
		while($bill_idalla=mysql_fetch_row($bill_idall)){
         ?>
         
		<li> <a href="#<?php echo $opd1[0]."o"; ?>"> <?php echo $bill_idalla[0] ; ?></a></li>
      <?php } ?>
     </ul>
  ...
  ?>
     ...
     <?php $itemqry=mysql_query("SELECT `name`,`unit_price` from `phppos_items` where `item_id`='$resfetch[2]'"); 
