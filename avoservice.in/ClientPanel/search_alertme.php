...
if ($_POST['br'] != 'all') {
    ...
    $src = mysqli_query($conc,"select state from state where state_id in (" . $br1 . ")");
...
if (isset($_POST['eng']) && $_POST['eng'] != '') {
    ...
    $alrtid = mysqli_query($conc,"select alert_id from alert_delegation where engineer='" . $eng . "' ");
    $alertarry = array();
...
if (isset($_POST['atmid']) && $_POST['atmid'] != '') {
    $id = $_POST['atmid'];
    $qr = mysqli_query($conc,"select track_id from atm where atm_id LIKE '%" . $id . "%'");
    $qr2 = mysqli_query($conc,"select amcid from Amc where atmid LIKE '%" . $id . "%'");
    $qr3 = mysqli_query($conc,"select atm_id from alert where atm_id LIKE '%" . $id . "%'");
...
$table = mysqli_query($conc, $sql);
$qry = mysqli_query($conc,"select cust_name from customer where cust_id='" . $cid . "'");
...
if (mysqli_num_rows($table) > 0) {
    while ($row = mysqli_fetch_row($table)) {
        ...
        if (($row[17] == 'service' || $row[17] == 'pm' || $row[17] == 'dere') && $row[21] == 'amc') {
            $atm = mysqli_query($conc,"select atmid from Amc where amcid='" . $row[2] . "'");
        }

        if (($row[17] == 'service' || $row[17] == 'pm' || $row[17] == 'dere') && $row[21] == 'site') {
            $atm = mysqli_query($conc,"select atm_id from atm where track_id='" . $row[2] . "'");
        }

        //}
        $tab = mysqli_query($conc,"select feedback,standby,feed_date from eng_feedback where alert_id='" . $row[0] . "' order by id DESC");
...
<?php
$oldeng = mysqli_query($conc,"select engineer from alert_delegation where alert_id='" . $row[0] . "'");
        $getold = mysqli_fetch_row($oldeng);
        $fetchengid = mysqli_query($conc,"Select engg_name,phone_no1 from area_engg where engg_id='" . $getold[0] . "'");
...
<td>
<?php //if($row[19]=='Y')
        if ($row[15] == 'Done' || $row[16] == 'Done') {echo "Call Closed";} else if ($row[16] == 'Rejected') {echo "Call Rejected by AVO";} else if ($row[16] == 'onhold') {echo "Call is on Hold for Your action";} else {
            echo "Pending";
        }
        ?>
 </td>
