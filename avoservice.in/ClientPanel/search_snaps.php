...
$cntrowsa=mysqli_query($conc,$sql);
$count=0;
$Num_Rows = mysqli_num_rows ($cntrowsa);
 ?>
...
while($row= mysqli_fetch_row($table))
{
	...
	$tab=mysqli_query($conc,"select feedback,standby,feed_date from eng_feedback where alert_id='".$row[0]."' order by id DESC limit 1");
	$update=mysqli_fetch_row($tab);
		?>
<tr>
...
<?php 
  if($row[41] =='Done') {   
$snapqry=mysqli_query($conc,"select * from snap_inst where alert_id='".$row[0]."'");
