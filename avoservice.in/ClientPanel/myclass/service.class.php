...
$qry2ro=mysqli_fetch_row($qry2);
    //    $qry2=mysqli_query($conc,"select cust_name from customer where cust_id='".$this->cust."'");
    //    $qry2ro=mysqli_fetch_row($qry2);
    //    echo $qry2ro[0];
...
/*	$sql = mysqli_query($conc,"INSERT INTO `alert` (`alert_id`, `cust_id`,`atm_id`, `bank_name`, `area`, `address`, `city`, `branch_id`, `pincode`, `problem`, `entry_date`, `alert_date`, `caller_name`, `caller_phone`, `caller_email`, `status`, `call_status`, `alert_type`, `close_date`, `standby`, `po`, `assetstatus`, `appby`, `appref`,`state1`,`createdby`,`subject`,`custdoctno`,`ccmail`) VALUES (NULL, '".$this->cust."','".$this->atm."' , '".$this->bank."', '".$this->area."', '".$this->add."', '".$this->city."', '".$this->branch."', '".$this->pin."', '".$this->prblm."', '".$dt."', '".$this->alertdt."', '".$this->caller."', '".$this->con."', '".$this->email."', 'Pending', 'Pending', '".$this->type_call."', '', '', '".$this->po."','".$this->type."', '".$this->appby."', '".$this->how."','".$this->state."','".$qry2ro[0]."_".date("ymd").$num3."','".$this->sub."','".$this->doc."','".$this->ccm."')");*/
	...
	$sql = mysqli_query($conc,"INSERT INTO `alert_customer` (`alert_id`, `cust_id`,`atm_id`, `bank_name`, `area`, `address`, `city`, `branch_id`, `pincode`, `problem`, `entry_date`, `alert_date`, `caller_name`, `caller_phone`, `caller_email`, `status`, `call_status`, `alert_type`, `close_date`, `standby`, `po`, `assetstatus`, `appby`, `appref`,`state1`,`subject`,`custdoctno`,`ccmail`) VALUES (NULL, '".$this->cust."','".$this->atm."' , '".$this->bank."', '".$this->area."', '".$this->add."', '".$this->city."', '".$this->branch."', '".$this->pin."', '".$this->prblm."', '".$dt."', '".$this->alertdt."', '".$this->caller."', '".$this->con."', '".$this->email."', 'Pending', 'Pending', 'service', '', '', '".$this->po."','".$this->type."', '".$this->appby."', '".$this->how."','".$this->state."','".$this->sub."','".$this->doc."','".$this->ccm."')");
	// 	 	echo count($this->assets);
	$row=mysqli_insert_id();
	//echo "Update alert set createdby='".$this->crby."_".date("Ymd").$num3."' where alert_id='".$row."'";
	//$qry4=mysqli_query($conc,"Update alert set createdby='".$this->crby."_".date("Ymd").$num3."' where alert_id='".$row."'");
	//echo "Update alert_id set createdby='".$this->crby."_".$row."' where alert_id='".$row."'";
	if($this->type=='site')
	$at=mysqli_query($conc,"select atm_id from atm where track_id='".$this->atm."'");
	elseif($this->type=='amc')
	$at=mysqli_query($conc,"select atmid from Amc where amcid='".$this->atm."'");
	...
	for($i=0;$i<count($this->assid);$i++)
		{
			...
			//echo $ini;
			$iniqr=mysqli_query($ini);
			$iniro=mysqli_fetch_row($iniqr);
			//echo "<br>select * from assets_specification where ass_spc_id='".$iniro[1]."'";
			$qry2=mysqli_query($conc,"select * from assets_specification where ass_spc_id='".$iniro[1]."'");
				$row2=mysqli_fetch_row($qry2);
				//echo "Select * from assets where assets_id='".$row2[1]."'<br>";
				$qry3=mysqli_query($conc,"Select * from assets where assets_id='".$row2[1]."'");
				$row3=mysqli_fetch_row($qry3);
				$assets=$row3[1]." (".$row2[2].")";
		$qry=mysqli_query($conc,"Insert into alert_assets(`alert_id`,`po`,`assets`,`qty`) Values('".$row."','".$asst[0]."','".$assets."','1')");
		...
		{
		$escem=array();
		$escpple=mysqli_query($conc,"select email,level from esclatingpeople where level='0' and state like '%".$this->state."%' and status=0 order by id ASC");
while($pplro=mysqli_fetch_row($escpple))
$escem[]=$pplro[0];

$esclation=implode(",",$escem);
		$esc=mysqli_query($conc,"INSERT INTO `escalation` (`alertid`, `startdt`, `endtime`, `escalateto`, `type`, `level`, `status`) VALUES ('".$row."', '".$dt."', '".date( "Y-m-d H:i:s", strtotime( "$dt +15 minute" ) )."', '".$esclation."', '0', '1', '0')");
...
$headers = "From: <HelpDesk@avoservice.in>\r\n";
		...
		$qry1=mysqli_query($conc,"Select state_id from state where state='".$this->state."'");
		if(mysqli_num_rows($qry1)>0)
		{
		$resltrow=mysqli_fetch_row($qry1);
		$message2="You have  New Alerts";
			$qry2=mysqli_query($conc,"Select * from login where designation='3'");
			$srno=array();
			while($max1=mysqli_fetch_row($qry2))
				...
				$qryreslt=mysqli_query($conc,"Select gcm_regid from notification_tble where logid in($logid)");
