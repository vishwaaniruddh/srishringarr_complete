...
//echo "select atm_id from atm where track_id='".$row[2]."'";
    ...
    $tab = mysqli_query($conc,"select feedback,standby from eng_feedback where alert_id='" . $row[0] . "' order by id DESC");
    $row1 = mysqli_fetch_row($tab);
    //echo "eng stat".$row[15];

    $qry = mysqli_query($conc,"select cust_name from customer where cust_id='" . $row[1] . "'");
...
//======Branch========
    $branch = mysqli_query($conc,"select name from avo_branch where id='" . $row[7] . "'");
...
//==============
    if ($row[28] == '1') {
        ...
        $buy = mysqli_query($conc,"select * from buyback where alertid='" . $row[0] . "'");
...
//========Last FeedBack ========
    if ($row1[0] != '') {
        ...
        $al = mysqli_query($conc,"select max(id),feedback from eng_feedback where alert_id='" . $row[0] . "'");
