...
$alert = mysqli_query($conc,"select a.createdby,a.cust_id,a.atm_id,a.bank_name,a.address,a.state1,a.problem,a.entry_date,a.alert_type,a.assetstatus,c.cust_name from alert a,customer c where a.alert_id='" . $id . "' and a.cust_id=c.cust_id");
$alertro = mysqli_fetch_row($alert);
if($alertro[9] != '' || $alertro[9] != NULL){

if ($alertro[9] == 'amc') {
    $atm = mysqli_query($conc,"select atmid from Amc where amcid='" . $alertro[2] . "'");
}
elseif ($alertro[9] == 'site') {
    $atm = mysqli_query($conc,"select atm_id from atm where track_id='" . $alertro[2] . "'");
}

$atmro = mysqli_fetch_row($atm);
$atm_id=$atmro[0];
} else $atm_id = $alertro[2];
?>
<tr><th>Client</th><td><?php echo $alertro[10]; ?></td><th>Atm ID</th><td><?php echo $atm_id; ?></td></tr>
...
$del = mysqli_query($conc,"select engg_name from area_engg where engg_id=(select engineer from alert_delegation where alert_id='" . $id . "' order by id DESC limit 1)");

if (mysqli_num_rows($del) > 0) {
    $delro2 = mysqli_fetch_row($del);
    $eng = $delro[0];
...
$tab = mysqli_query($conc,"select * from eng_feedback  where alert_id='" . $id . "' order by feed_date DESC");
while ($row = mysqli_fetch_row($tab)) {

    $qry = mysqli_query($conc,"select * from login where srno='" . $row[2] . "'");
...
<td><?php if (isset($row[4]) and $row[4] != '0000-00-00') {
        echo date('d/m/Y h:i:s a', strtotime($row[4]));
    }
    ?></td>
