...
$alert=mysqli_query($con1,"select a.createdby,a.cust_id,a.atm_id,a.bank_name,a.address,a.state1,a.problem,a.entry_date,a.alert_type,a.assetstatus,c.cust_name from alert a,customer c where a.alert_id='".$id."' and a.cust_id=c.cust_id");

$alertro=mysqli_fetch_row($alert);

if($alertro[9] !=  '' || $alertro[9] !=NULL ){
if($alertro[9] ==  'amc'){
    $atm=mysqli_query($con1,"select atmid from Amc where amcid='".$alertro[2]."'");
    
} elseif($alertro[9] == 'site') {
...
$del2=mysqli_query($con1,"select engg_name from area_engg where engg_id=(select engineer from alert_delegation where alert_id='".$id."' order by id DESC LIMIT 1)");
    ...
    <th>Call Logged On</th><td><?php echo $alertro[7]; ?></td>
...
$tab=mysqli_query($con1,"select * from eng_feedback  where alert_id='".$id."' order by feed_date DESC");
...
<td><?php if(isset($row[4]) and $row[3]!='0000-00-00') echo $row[4]; ?></td>
