   ...
   <script  type="text/javascript" src="https://maps.googleapis.com/maps/api/js?key=AIzaSyCBE1Xgn2mQmGOtUevIuFYw6443BkKCjbI&sensor=false" ></script>
    ...
    <div id='input'>
      ...
      //  echo $engg_logid;
        ...
        $resultx = mysqli_query($con1,"SELECT loginid,engg_name,engg_id FROM `area_engg` where loginid=(select srno from login where username='".$engg_logid."') and status=1 and deleted=0");
       
     //  echo "SELECT loginid,engg_name FROM `area_engg` where loginid=(select srno from login where username='".$engg_logid."') and status=1 and deleted=0";
