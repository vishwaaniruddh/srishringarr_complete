...
$eng=$_POST['eng'];
	...
	$alrtid=mysqli_query($con1,"select alert_id from alert_delegation where engineer='".$eng."' ");
 	$alertarry = array();
...
//echo $sql;
$table=mysqli_query($con1,$sql);
$qry=mysqli_query($con1,"select cust_name from customer where cust_id='".$cid."'");
...
<td>
<?php 
//$bm=mysqli_query($con1,"select up from alert_updates where alert_id='".$row[0]."' order by id DESC limit 1");
...
//echo $row[16];
 if(($row[16]=='Delegated' || $row[16]=='2' || $row[16]=='1' || $row[16]=='Done' || (mysqli_num_rows($qrytmp)>0) && $row[26]!='1') )
 {
 ?>
