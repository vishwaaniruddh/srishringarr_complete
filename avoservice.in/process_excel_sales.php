 ...
 $contents.="Sr no \t Customer Vertical \t so_trackid \t PO Number \t PO ID  \t Buyer Name \t ATM ID \t Branch \t Delivery Type \t Installation Request \t   Buyback \t Contact Name \t  Contact Phone \t Contact Email \t Success\t  Message \t";
    ...
    for($i = 1; $i<=$row; $i++){
      ...
      $check_po_sql = mysqli_query($con1,"select * from purchase_order where po_no like '".$po_no."'");
        ...
        } else { $po_id= $check_po_sql_result['id']; }
        

  /*      $check_so = mysqli_query($con1,"select * from new_sales_order where po_trackid ='".$po_id."'");
        if($check_so_result = mysqli_fetch_assoc($check_so)){
            $error.='Sales Order Already Exist !, ';            
        } */
        if(!$consignee_name){
    ...
    if($error == '0'){
        ...
        while($sales_assets_sql_result = mysqli_fetch_assoc($sales_assets_sql)){
            ...
            $sales_asset_insert = "insert into new_sales_order_asset(so_trackid,po_trackid,po_product,po_model,po_qty,po_warr,po_rate) values('".$so_trackid."','".$po_id."','".$assets_name."','".$specs."','".$qty."','".$warranty."','".$rate."')";
       ...
       foreach($po_product_id as $key=>$val){
           ...
           $consumption_sql = "insert into po_consumption(po_trackid,so_trackid,po_product,po_model,po_cap,po_qty,po_consumqty,po_status,so_assetID) values('".$po_id."','".$so_trackid."','".$assetTrackid[$count]."','".$po_model."','0','".$po_quantity."','".$po_quantity."','0','".$val."')";
        ...
        $buyback_product = $rowData[$i][0][9];
        $buyback_cap = $rowData[$i][0][10];
        $buyback_qty = $rowData[$i][0][11];
        $buyback_value = $rowData[$i][0][12];
