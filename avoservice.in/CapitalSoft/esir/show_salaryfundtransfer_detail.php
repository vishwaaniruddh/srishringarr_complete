...
if(isset($_POST['apps'])){ 
    if(!empty($_POST['apps'])){
       ...
       // echo '<pre>';print_r($app);echo '</pre>';
        if(!empty($app)){
            $transresult=mysqli_query($con,"select max(trans_id) from mis_salary_fund_transfer"); 
            ...
            for($x=0;$x<count($app);$x++){  
                ...
                $postsql="select account_number,trans_id from mis_salary_fund_transfer where id='".$app[$x]."'";
        	    $posttable=mysqli_query($con,$postsql);    
                $postrow=mysqli_fetch_row($posttable);
               // $fund_details = $postrow[0];
                $account_number = $postrow[0];
                $trans_id = $postrow[1];
                
               //$updatesql = "update mis_salary_fund_transfer SET status= 3,trans_id='".$tid."' WHERE account_number = '".$account_number."' and fundDetails ='".$fund_details."'"; 
               $updatesql = "update mis_salary_fund_transfer SET status= 3,trans_id='".$tid."' WHERE trans_id='".$trans_id."' and account_number = '".$account_number."'"; 
...
if(isset($_POST['SelectDept'])){
    if($_POST['SelectDept']=='Search'){
       ...
       // $statement = "select * from mis_staff_salary where status=5 and department='".$department."' order by id desc" ;  
        if($particulars!=""){
             $sql = "select t1.beneficiary_name, t1.account_number, t1.ifsc_code, sum( t1.amount ), t1.id,t1.fund_remark,
                   t2.month, t2.year, t2.department, t2.remark
                       from mis_salary_fund_transfer as t1, mis_staff_salary as t2 where t1.req_id = t2.id and t1.status=2 and t2.department='".$department."' and t2.particulars='".$particulars."'
                       group by t1.account_number order by t1.id desc";
        }else{
        $sql = "select t1.beneficiary_name, t1.account_number, t1.ifsc_code, sum( t1.amount ), t1.id,t1.fund_remark,
                   t2.month, t2.year, t2.department, t2.remark
                       from mis_salary_fund_transfer as t1, mis_staff_salary as t2 where t1.req_id = t2.id and t1.status=2 and t2.department='".$department."'
                       group by t1.account_number order by t1.id desc";
        }
        $postview = 1;
    }
            ...
            $total_data = "select sum(amount),department from mis_staff_salary where status=6 group by department order by id desc" ;  
            $totsql = mysqli_query($con,$total_data);
            $tot_team_sql = mysqli_query($con,$total_data);
            
             $total_particulardata = "select sum(amount),particulars from mis_staff_salary where status=6 and particulars!='' group by particulars order by id desc" ;  
                            ...
                            <div class="page-body">
                                <div class="card">
                                    <div class="card-block" style=" overflow: auto;">
                                        ...
                                        <form action="show_salaryfundtransfer_preview.php" method="POST">
                                        <table id="myTable" class="table table-bordered table-striped table-hover dataTable no-footer" style="width:100%">
                                            ...
                                            <tbody>
                                                <?  
                                                    $total = 0;
                                                  /*  $sql="select t1.beneficiary_name, t1.account_number, t1.ifsc_code, sum( t1.amount ), t2.fundDetails, t1.id,t1.fund_remark,t2.type
                                                           from mis_salary_fund_transfer as t1, mis_staff_salary as t2 where t1.req_id = t2.id and t1.status=2 
                                                    ...
                                                    <tr>
                                                      ...
                                                      <!--   <td id="remarks_<? echo $i; ?>"><? echo $row[4] ?></td> 
                                                        ...
                                                        <td><select class="form-control" id="fund_detail_<? echo $i; ?>" onchange="updateFundRemark(<? echo $id ?>,this.value)">
                                                            ...
                                                            ?>
                                                            <option <? if($_fundremark==$fundremarkrow['fund_remark']){ echo 'selected'; }else{ if($fundremarkrow['fund_remark']=='STAFF SALARY'){ echo 'selected'; }} ?> value="<? echo $fundremarkrow['fund_remark'] ?>"><? echo $fundremarkrow['fund_remark'] ?></option>
                                                            <? } ?>
                                                        </select></td> 
                                                        <td>
                                                          <a id="reject_<? echo $i; ?>" data-toggle="modal" data-reqid="<? echo $row[4]; ?>" data-checkboxid="<? echo $i; ?>" data-req_amt="<? echo $row[3]; ?>" data-id="<? echo $id; ?>" class="open-RejectDialog btn btn-danger" href="#rejectModal">Reject</a>
                                                        </td>
                                                    </tr>         
                                                <?  } 	}
                                                ?>
...
$('#rejectModal form').on('submit', function (e) {
          ...
          $.ajax({
            type: 'post',
            url: 'process_salaryfundtransfer_action1.php',
...
function updateFundRemark(id,val){
   ...
   $.ajax({
            type: 'post',
            url: 'update_salary_fund_remarks.php',
