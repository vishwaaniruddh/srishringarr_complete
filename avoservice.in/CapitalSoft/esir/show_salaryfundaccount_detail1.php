    ...
    if(!empty($_POST['apps'])){
        ...
        if(!empty($app)){
           // echo '<pre>';print_r($app);echo '</pre>';die;
            $result=mysqli_query($con,"select max(trans_id) from mis_salary_fund_transfer"); 
             ...
             for($x=0;$x<count($app);$x++){  
                ...
                $sql="select amount from mis_salary_fund_requests where status=5 and action=1 and req_id='".$app[$x]."'";
                ...
                $requestedusersql="select beneficiary_name,account_number,ifsc from mis_staff_salary where id='".$app[$x]."'";
                ...
                $insert_sql = "insert into mis_salary_fund_transfer(trans_id,req_id,amount,status,beneficiary_name,account_number,ifsc_code,action_by) 
                        values('".$tid."','".$req_id."','".$amount."','".$action."','".$beneficiary_name."','".$account_number."','".$ifsc_code."','".$created_by."')";
...
if(isset($_POST['SelectDept'])){
    if($_POST['SelectDept']=='Search'){
        ...
        if($particulars!=""){
             $statement = "select * from mis_staff_salary where status=5 and department='".$department."' and particulars='".$particulars."' order by id desc" ; 
        }else{
        ...
        $postview = 1;
            ...
            $total_data = "select sum(amount),department from mis_staff_salary where status=5 group by department order by id desc" ;  
            $totsql = mysqli_query($con,$total_data);
            $tot_team_sql = mysqli_query($con,$total_data);
            
             $total_particulardata = "select sum(amount),particulars from mis_staff_salary where status=5 and particulars!='' group by particulars order by id desc" ;  
                                    ...
                                    <div class="card-block" style=" overflow: auto;">
                                        ...
                                        <table id="myTable" class="table table-bordered table-striped table-hover dataTable no-footer" style="width:100%">
                                            ...
                                            <tbody>
                                                <?
                                                if($postview==0){
                                                   $statement = "select * from mis_staff_salary where status=5 order by id desc" ;  
                                                }
                                                ...
                                                $_sql="select * from mis_salary_fund_requests where status=5 and action=1 and req_id='".$id."'";
