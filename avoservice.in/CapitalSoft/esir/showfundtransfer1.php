...
if($_SESSION['username']){ 
   ...
   /* if(isset($_POST['reqs'])){
    ...
    $postview = 0;
    $selectedstatus = 3;
    $sql="select sum(approved_amt),trans_id,batch_no,id from mis_fund_transfer where status=3 and current_status in (1,2,3) group by trans_id order by trans_id ASC";
if(isset($_POST['SelectStatus'])){
    if($_POST['SelectStatus']=='Search'){
        $status = $_POST['current_status'];
        if($status=='4'){
            $sql="select sum(approved_amt),trans_id,batch_no,id from mis_fund_transfer where status=3 and current_status=4 group by trans_id order by trans_id ASC";
            $selectedstatus = 4;
        }
        if($status=='3'){
            $sql="select sum(approved_amt),trans_id,batch_no,id from mis_fund_transfer where status=3 and current_status in (1,2,3) group by trans_id order by trans_id ASC";
        ...
        if($status=='2'){
            $sql="select sum(approved_amt),trans_id,batch_no,id from mis_fund_transfer where status=3 group by trans_id order by trans_id ASC";
             $selectedstatus = 2;
        }
                                                    ...
                                                    while($row=mysqli_fetch_array($table)){
                                                      /*  if(!in_array($row[8],$accountno_array)){
                                                       ...
                                                       // echo '<pre>';print_r($row);echo '</pre>';die;
                                                         $id = $row[3];
                                                         $req=mysqli_query($con,"select transferred_date,current_status from mis_fund_transfer where batch_no='".$row[2]."' and trans_id='".$row[1]."'");
				                                        // $reqro=mysqli_fetch_array($req);
                                                         ...
                                                         $i++;
                                                         	...
                                                         	$req_no_sql="select req_id from mis_fund_transfer where trans_id=".$row[1]." and current_status>0" ;
                                                            $req_no_table=mysqli_query($con,$req_no_sql); 
                                                            
                                                            $rejsql="select sum(approved_amt) from mis_fund_transfer where status=3 and current_status=0 and trans_id='".$row[1]."'";
                                                        ...
                                                        <tr>
                                                            <td><? echo $row[1] ?></td>
                                                             ...
                                                             <td><? echo $tot; ?></td>  
                                                             <td><a href="show_fund_pay_transfer1.php?id=<?php echo $row[1]; ?>" target="_blank"><input type='button' class="form-control" name="view"  value="View Details" /></a></td>
                                                              <td>
                                                                ...
                                                                <form action="show_fund_pay_test.php" method="POST">
                                                                    <?
                                                              ...
                                                              <td>
                                                                 <a href="fundtransfer_full_details1.php?id=<? echo $row[1]; ?>" target="_blank"><input type='button' class="form-control" name="fulldetails_det"  value="Full Details" /></a>
