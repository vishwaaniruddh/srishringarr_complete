                                                ...
                                                <?  
                                                	...
                                                	for($x=0;$x<count($app);$x++){  
                                                     ...
                                                     /*  if($transferred_date=='0000-00-00'){
                                                       ...
                                                       if($view==1){ $i++;
                                                           $req_id = $app[$x];
                                                           $statement = "select * from rnm_fund where id='".$req_id."' and created_by='".$user_id."' order by id desc" ;  
                                                           $rnm_fundsql = mysqli_query($con,$statement);
                                                           $no_of_rows = mysqli_num_rows($rnm_fundsql);
                                                           while($sql_result = mysqli_fetch_assoc($rnm_fundsql)){ 
                                                            //if($sql_result['type']=='UTILITY-BILLS'){
                                                        ?>
                                                    <tr>
                                                         <td><? echo $i; ?></td>
                                                         <td><? echo $row[9] ?></td>  
