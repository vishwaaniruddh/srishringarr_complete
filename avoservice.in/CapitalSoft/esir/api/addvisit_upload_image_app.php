<?php 
        ...
        $target_filedir =  "../addvisituploadimageapp/".$atmid.'/'; 
        $target_dir = "addvisituploadimageapp/".$atmid.'/';
      ...
      //  $filename = time()."_".$name.".".$ext; 
        ...
        if (move_uploaded_file($_FILES["image"]["tmp_name"][0], $target_file)) {
            ...
            $select_qry = "SELECT * from addvisit_upload_images_app WHERE img_name='".$_imagefilename."' AND atmid='".$atmid."'";
            $query_result = mysqli_query($con,$select_qry);
            if(mysqli_num_rows($query_result)>0){
                $del_qry = "DELETE FROM addvisit_upload_images_app WHERE img_name='".$_imagefilename."' AND atmid='".$atmid."'";
                mysqli_query($con,$del_qry);
            }
            
            $link = 'https://cssmumbai.sarmicrosystems.com/css/dash/esir/'.$target_dir. $filename ; 
            $_ins_sql = "insert into addvisit_upload_images_app(img,img_name,eng_id,atmid,created_at) values('".$link."','".$_imagefilename."','".$eng_id."','".$atmid."','".$datetime."')" ; 
