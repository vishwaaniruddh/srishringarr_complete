...
if(isset($_POST['download'])){
    if($_POST['id']){
        ...
        $getimagesql = "select link from checkquality_images_app where visitid='".$visitid."'"; 
...
if(isset($_POST['videos'])){
    if($_POST['id']){
        $visitid = $_POST['id'];
        $fileName = 'my-archive.zip';
        $getimagesql = "select link from checkquality_videos_app where visitid='".$visitid."'"; 
        $getimagesdata = mysqli_query($con,$getimagesql);
        $imagesdata_array = array();
        if(mysqli_num_rows($getimagesdata)>0){
            while($getimages_result = mysqli_fetch_array($getimagesdata)){
...
?>
<script>
  window.location = "view_checkquality.php";
</script>