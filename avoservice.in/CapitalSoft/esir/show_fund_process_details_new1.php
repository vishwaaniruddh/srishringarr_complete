...
if($_SESSION['username']){ 
    ...
    if(isset($_POST['submit'])){
        ...
        $_misfundrequestsql = "select req_id,id from mis_fund_requests group by req_id";
        $misrequest_table=mysqli_query($con,$_misfundrequestsql);  
         
        while($misfundreqtabledata = mysqli_fetch_assoc($misrequest_table)){
    		$reqidarray[] = $misfundreqtabledata['req_id'];
    	}
    	
    	$reqidarray=json_encode($reqidarray); 
    	$reqidarray=str_replace( array('[',']','"') , ''  , $reqidarray);
    	$arr=explode(',',$reqidarray);
    	$reqidarray = "'" . implode ( "', '", $arr )."'";
    	
    	$misrequestsql="select req_id,action from mis_fund_requests where req_id IN (".$reqidarray.") AND status<6";
    	$_action = 0;
            ...
            if($_POST['status']=="all"){
                ...
                if($_POST['status']=="4"){
                    ...
                    if($_POST['status']==1){
                      ...
                      $_action = 0;
                      $misrequestsql .= " and action='".$_POST['status']."'";
                    }
       ...
       // echo $misrequestsql;die;
        ...
        if($_action!='4'){
            ...
            if($misrequestsqlrowcount>0){
                 while($mis_fund_req_table_data = mysqli_fetch_assoc($misrequestsqltable)){
            		$uniquereq_id = $mis_fund_req_table_data['req_id'];
            		array_push($app,$uniquereq_id);
            	 }
            }
        }
    	
    	
        $mainsql="select req_id,current_status from mis_fund_transfer where req_id IN (".$reqidarray.") and status=3";
        ...
        $_misrequestsqltable=mysqli_query($con,$mainsql); 
        $_misrequestsqlrowcount=mysqli_num_rows($_misrequestsqltable);
        if($_misrequestsqlrowcount>0){
           array_push($app,$uniquereq_id);
            while($mis_fund_trans_table_data = mysqli_fetch_assoc($_misrequestsqltable)){
            		$uniquetransreq_id = $mis_fund_trans_table_data['req_id'];
            		array_push($app,$uniquetransreq_id);
            	 }
        }
                                            ...
                                            <tbody>
                                                <?  
                                                	...
                                                	for($x=0;$x<count($app);$x++){  
                                                        ...
                                                        $currentsql="select current_status,transferred_date,approved_amt,trans_id,status,remarks,batch_no from mis_fund_transfer where req_id='".$app[$x]."'";  
                                                        $currenttable=mysqli_query($con,$currentsql); 
                                                        if(mysqli_num_rows($currenttable)>0){
                                                            $currentrow=mysqli_fetch_row($currenttable);
                                                            $_currentstatus = $currentrow[0];
                                                            $transferred_date = $currentrow[1];
                                                            $trans_id = $currentrow[3];
                                                            $_customer_total_amt = $currentrow[2];
                                                            $_position = $currentrow[4];
                                                            $_remarks = $currentrow[5];
                                                            $_batchno = $currentrow[6];
                                                        }else{
                                                            $misrequestfinalsql="select action,req_id,approved_amt,status,remarks from mis_fund_requests where req_id='".$app[$x]."' order by id desc limit 1";
                                                            $misrequestfinalsqltable=mysqli_query($con,$misrequestfinalsql); 
                                                            $misrequestfinalsqltablerow=mysqli_fetch_row($misrequestfinalsqltable);
                                                            $_currentstatus = $misrequestfinalsqltablerow[0];
                                                            $_customer_total_amt = $misrequestfinalsqltablerow[2];
                                                            $transferred_date = '';
                                                            $trans_id = '';
                                                            $_position = $misrequestfinalsqltablerow[3];
                                                            $_remarks = $misrequestfinalsqltablerow[4];
                                                            $_batchno = "";
                                                        }
                                                    ...
                                                    <tr>
                                                         ...
                                                         <td><? echo $_remarks ?></td> 
                                                         <td><? if($trans_id!=''){
                                                                        if($_currentstatus==0){ echo 'Rejected';}
                                                                      ...
                                                                      if($_currentstatus>0){ 
                                                                          if($_position=='2'){
                                                                              echo 'Pending in Operations Level';
                                                                          }
                                                                          if($_position=='3'){
                                                                              echo 'Pending in Manager Level';
                                                                          }
                                                                          if($_position=='4'){
                                                                              echo 'Pending in Director Level';
                                                                          }
                                                                          if($_position=='5'){
                                                                              echo 'Pending in Account Level';
                                                                          }
                                                         ...
                                                         <td>
                                                             <? if($_currentstatus==4 && $transferred_date!='0000-00-00'){ echo $transferred_date;}
                                                                if($_currentstatus==0){ echo $row[18];}
                                                             ?>
                                                         </td>
                                                         <td><? echo $trans_id; ?></td>
                                                        <td><? echo $_batchno; ?></td>
                                                        ...
                                                        <td>
                                                            <? echo date('d M Y',strtotime($sql_result['created_at']));?>
                                                        </td>
                                                        <td><? echo $sql_result['payee_type']; ?> </td>
                                                        <td><? echo $sql_result['fundDetails']; ?> </td>
                                                        <td><? echo $sql_result['fund_remark']; ?></td>
                                                    </tr>         
                                                <? } } 	}
                                                ?>
                                                </tbody>
