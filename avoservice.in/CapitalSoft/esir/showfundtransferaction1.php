...
if($_SESSION['username']){ 
    ...
    if(isset($_POST['reqs'])){
        ...
        for($x=0;$x<count($app);$x++){ 
            $req_id = $app[$x];
            $currentstatus_sql="select current_status from mis_fund_transfer WHERE req_id = ".$req_id;
            ...
            $updatesql = "";
            if($_currentstatus==1){
              $updatesql = "update mis_fund_transfer SET current_status = 2, batch_no= '".$batch_no."',action_by= '".$action_by."' WHERE req_id = ".$req_id; 
            }
            if($_currentstatus==2){
              $updatesql = "update mis_fund_transfer SET current_status = 3,action_by= '".$action_by."' WHERE req_id = ".$req_id;   
            }
            if($_currentstatus==3){
              $updatesql = "update mis_fund_transfer SET current_status = 4,action_by= '".$action_by."' WHERE req_id = ".$req_id;   
            }
            if($updatesql!=""){
  ...
  <script>
     ...
     window.location.href="showfundtransfer1.php";
 </script>
    <?
}else{
