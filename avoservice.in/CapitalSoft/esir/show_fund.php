                                        ...
                                        <form action="<? echo $_SERVER['PHP_SELF']; ?>" method="POST">
                                            <div class="row">
                                                 ...
                                                 <div class="col-md-3">
                                                     ...
                                                     <input type="date" name="fromdt" class="form-control" value="<? if(isset($_POST['fromdt'])){ echo  $_POST['fromdt']; }else{ echo '2022-12-12' ; } ?>">    
                                                 </div>
                                                 <div class="col-md-3">
                                                 ...
                                                 <div class="col-md-2">
                                                     <label>Beneficiary</label>
                                                      <select id="multiselect_beneficiary" class="form-control" name="beneficiary_name[]" multiple="multiple">
                                                        <?
                                                        $i = 0;
                                                        $ben_sql = mysqli_query($con,"select beneficiary_name from rnm_fund GROUP BY beneficiary_name");
                                                           while($ben_sql_result = mysqli_fetch_assoc($ben_sql)){ ?>
                                                              <option value="<? echo $ben_sql_result['beneficiary_name']; ?>" 
                                                              <? if(isset($_POST['beneficiary_name'])) { if(in_array($ben_sql_result['beneficiary_name'],$_POST['beneficiary_name'])){ echo 'selected'; } } ?>>
                                                           <? echo $ben_sql_result['beneficiary_name']; ?>
                                                                </option> 
                                                           <?
                                                           $i++;
                                                           } ?>
...
if(isset($_POST['beneficiary_name']) && $_POST['beneficiary_name'] != ''){
    
    $beneficiary=json_encode($_POST['beneficiary_name']);
    $beneficiary=str_replace( array('[',']','"') , ''  , $beneficiary);
    $ben_arr=explode(',',$beneficiary);
    $beneficiary = "'" . implode ( "', '", $ben_arr )."'";
    $statement .=" and a.beneficiary_name in($beneficiary)" ;
}
    ...
    if(val==0){
    	...
    	$(document).ready(function() {
              $('#multiselect').multiselect({
                ...
                $('#multiselect_beneficiary').multiselect({
                buttonWidth : '100%',
                includeSelectAllOption : true,
            		nonSelectedText: 'Select an Option'
              });
