...
$contents='';
   ...
   $error_array = array();
    ...
    if(isset($rowData[$i][0][3]))
    { ?>
        <script>
            alert("Error!! Please Upload Correct Excel Format!!");
            window.location.href="mis_call_bulk_close.php";
        </script>  
    <? }else {
        

      
      $check = mysqli_query($con,"select ticket_id from mis_details where ticket_id = '".$ticketid."'");
      $check_res = mysqli_fetch_assoc($check);
      $_ticket_id = $check_res['ticket_id'];
      
      if($ticketid != $_ticket_id){
          echo "Ticket ID ".$ticketid." not Exist!!</br>";
      } else 
        if($ticketid){
               ...
               // echo $misid;die;
                $update = mysqli_query($con,"update mis_details set status='".$status."' where id='".$misid."' ");
                // var_dump($update);
                if($update)
                {
									...
									<table class="table table-bordered table-striped" id="data_table" style="width:100%;">
										...
										<tbody>
											<?
                                                ...
                                                $sqldata = mysqli_query($con,"select * from mis_details where id='".$misid."' ");
