...
<!--<script src="https://code.jquery.com/jquery-3.5.1.js"></script>-->
 ...
 <form id="form" action="show_fundtransfer_previewaction.php" method="POST">
                                                    ...
                                                    $total = 0;
                                                        ...
                                                        if(!empty($app)){
                                                            ...
                                                            for($x=0;$x<count($app);$x++){  
                                                               ...
                                                               /* $sql="select t1.beneficiary_name, t1.account_number, t1.ifsc_code, sum( t1.approved_amt ), t2.fundDetails, t1.id
                                                                       from mis_fund_transfer_test as t1, rnm_fund_test as t2 where t1.req_id = t2.id and t1.status=2 and t1.id=".$_id."
                                                                       group by concat(t2.fundDetails, '-', t1.account_number) order by t1.id desc"; */
                                                                $sql = "select beneficiary_name, account_number, ifsc_code, approved_amt, fundDetails, id, fund_remark, trans_id
                                                                       from mis_fund_transfer where id=".$_id." order by id desc";
                                                                ...
                                                                $_acno = $row[1];           
                                                                $_fd = $row[4];   
                                                                $_transactionid = $row[7];
                                                                $_sqlamt = "select approved_amt
                                                                       from mis_fund_transfer where trans_id='".$_transactionid."' and account_number='".$_acno."' and fundDetails='".$_fd."' order by id desc";       
         ...
         <button class="btn btn-danger"><a href="show_fundtransfer_detail.php">Cancel</a></button>       
