...
if($_SESSION['username']){ 
  ...
  /*  $app=$_POST['apps'];
    ...
    include('config.php');
    $app = array();
    $mainsql="select req_id from mis_fund_transfer where status=3 and current_status!=1 and current_status!=2";
    $date1 = "";
    $date2 = "";
    $_action = 2;
            ...
            if($_POST['status']=="all"){
                ...
                if($_POST['status']!=0){
                  ...
                  $mainsql .= " and current_status='".$_POST['status']."'";
                    ...
                    $_action = 0;
                  $mainsql="select req_id from mis_fund_transfer where current_status=0";
                }
            }
        }
    ...
    while($maintabledata=mysqli_fetch_array($maintable)){
       ...
       // $mainsql .=" and transferred_date >= '".$date1."' and transferred_date <= '".$date2."'";
          $misrequestsql .=" and CAST(a.created_at AS DATE) >= '".$date1."' and CAST(a.created_at AS DATE) <= '".$date2."'";
        }
        $misrequestsql .=" order by id desc limit 1"; 
        $misrequestsqltable=mysqli_query($con,$misrequestsql);   
        if(is_array($misrequestsqltable)){
