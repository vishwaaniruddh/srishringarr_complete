   ...
   for($i = 1; $i<=$row; $i++){
      ...
      $atmid = $rowData[$i][0][3];
        ...
        if($atmid){
                ...
                // $pincode = $rowData[$i][0][15];
                // $live_dt = $rowData[$i][0][16];
                // $live_date = date('Y-m-d', PHPExcel_Shared_Date::ExcelToPHP($live_dt));
                $errorcount=0;
                ...
                $citycount = 0;
                $citysql = "select id from mis_city where city = '".$city."' ";
                $cityquery = mysqli_query($con,$citysql);
                $citysqlresult = mysqli_num_rows($cityquery);
                 if($citysqlresult>0)
                 {
                    $citycount = 1;
                 }
                 else{
                    //  $sentence = "City ".$city." not in City Master";
                    // array_push($error_array,$sentence);
                    $citycount = 1 ; 
                    
                    $zoneSql = mysqli_query($con,"select * from newzone where zone_name='".$zone."'");
                    $zonesqlResult = mysqli_fetch_assoc($zoneSql);
                    $zoneid = $zonesqlResult['id'];
                    
                    
                    $stateSql = mysqli_query($con,"select * from state where state='".$state."'");
                    $statesqlResult = mysqli_fetch_assoc($stateSql);
                    $stateid = $statesqlResult['state_id'];
                    
                    
                    mysqli_query($con,"insert into mis_city(city,zone_id,state_id) 
                    values('".$city."','".$zoneid."','".$stateid."')");
              ...
              if($branchcount==1 && $zonecount ==1 && $citycount ==1 && $errorcount==0){
                  ...
                  if($check_sql_resulr = mysqli_fetch_assoc($check_sql)){
                     ...
                     $sql = "insert into mis_newsitetest_success(activity,customer,bank,atmid,atmid2,atmid3,trackerno,address,city,state,zone,branch,bm_name,bm_number,status,engineer_user_id,created_by,created_at) 
                    values('".$activity."','".$customer."','".$bank."','".$atmid."','".$atmid2."','".$atmid3."','".$trackerno."','".$address."','".$city."','".$state."','".$zone."','".$branch."','".$bm_name."','".$bm_number."','0','".$engineer."','".$userid."','".$created_at."')";
                    ...
                    if(mysqli_query($con,$sql)){ 
                      ...
                      $sql = "insert into mis_newsitetest_success(activity,customer,bank,atmid,atmid2,atmid3,trackerno,address,city,state,zone,branch,bm_name,bm_number,status,engineer_user_id,created_by,created_at) 
                        ...
                        $sql = "insert into mis_newsitetest_err(activity,customer,bank,atmid,atmid2,atmid3,trackerno,address,city,state,zone,branch,bm_name,bm_number,status,engineer_user_id,created_by,created_at,remark) 
                        values('".$activity."','".$customer."','".$bank."','".$atmid."','".$atmid2."','".$atmid3."','".$trackerno."','".$address."','".$city."','".$state."','".$zone."','".$branch."','".$bm_name."','".$bm_number."','0','".$engineer."','".$userid."','".$created_at."','".$sentence."')";
...
while($sql_result = mysqli_fetch_assoc($sql)){
    ...
    <tr>
        ...
        <!--<td><? echo $sql_result['pincode'] ; ?></td>-->
        <!--<td><? echo $sql_result['live_date'] ; ?></td>-->
    </tr>
    <? } ?>
    </tbody>
    </table>
