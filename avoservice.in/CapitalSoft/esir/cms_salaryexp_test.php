...
// rename the sheet
$objSheet->setTitle('MIS Salary Fund Transfer Excel');
...
$objSheet->setCellValue('U1', 'Email_Body');
...
$mnth=date('M-Y');
                                                	...
                                                	for($x=0;$x<count($app);$x++){  
                                                    	...
                                                    	$sql="select * from mis_staff_salary where id='".$app[$x]."'";
                                                	    $view = 0; 
                                                        $table=mysqli_query($con,$sql);    
                                                        $rowdata=mysqli_fetch_row($table);
                                                        
                                                        $req_amt_data=mysqli_query($con,"select amount from mis_salary_fund_transfer where req_id='".$app[$x]."' order by id desc");
                                                            $req_row_amt = mysqli_fetch_row($req_amt_data);
                                                            $_customer_total_amt = $_customer_total_amt + $req_row_amt[0];
                                                            $view = 1;
                                                        /*
                                                        if(!in_array($rowdata[10],$accountno_array)){
                                                           $accs=mysqli_query($con,"select * from mis_staff_salary where account_number='".$rowdata[10]."'"); 
                                                           
                                                           while($accr=mysqli_fetch_array($accs)){
                                                               if(in_array($accr[0],$app)){
                                                                   $req_amt_data=mysqli_query($con,"select amount from mis_salary_fund_requests where req_id=".$accr[0]." order by id desc");
                                                                   $req_row_amt = mysqli_fetch_row($req_amt_data);
                                                                     $_customer_total_amt = $_customer_total_amt + $req_row_amt[0];
                                                            
                                                               }
                                                           }
                                                           array_push($accountno_array,$rowdata[10]);
                                                           $view = 1;
                                                        } */
...
$objSheet->setCellValueByColumnAndRow(20, $row, $rowdata[13]);
 $row++;
$srn++;
}
}
