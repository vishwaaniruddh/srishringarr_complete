...
xmlhttp.open("get","servicesearch1_test.php?val="+txt+"&what="+what+"&calltype="+call,false);
...
<table border="0" cols="res">
  ...
  echo  "select max(alert_id) from alert where atm_id='".$_POST['ref']."' and date(entry_date) ='".$cdate."'";
        $cdate=date('Y-m-d');
        ...
        <td height="35" colspan="4"><input type="submit" value="submit" class="readbutton" id="submit" /></td>
               ...
               else
               {
                 //  if($con1){echo 1; } else { echo 0; } 
                    ServiceForm(NULL,$_SESSION['user'],$con1); 
        		} 
         ?>
