...
//	echo $ab[1]." ".date('Y-m-d', strtotime($ab[9]))." ".$x."<br>";
	...
	$state=mysqli_query($con1,"select * from `avo_branch` where `name`='".$ab[7]."'");
	if(mysqli_num_rows($state)==0)
	{
...
if(!in_array($ab[28],$citycat))
	{
	$contents.="\n ".preg_replace('/\s+/', ' ', $ab[1])." \t ".preg_replace('/\s+/', ' ', $ab[2])." \t ".preg_replace('/\s+/', ' ',$ab[3] )." \t ".preg_replace('/\s+/', ' ',$ab[4] )." \t ".preg_replace('/\s+/', ' ',$ab[5] )." \t ".preg_replace('/\s+/', ' ',$ab[6] )." \t ".preg_replace('/\s+/', ' ',$ab[7] )." \t ".preg_replace('/\s+/', ' ',$ab[8] )." \t ".preg_replace('/\s+/', ' ',$ab[9] )." \t ".preg_replace('/\s+/', ' ',$ab[10] )." \t ".preg_replace('/\s+/', ' ',$ab[11] )." \t ".preg_replace('/\s+/', ' ',$ab[12] )." \t ".preg_replace('/\s+/', ' ',$ab[13] )." \t ".preg_replace('/\s+/', ' ',$ab[14] )." \t ".preg_replace('/\s+/', ' ',$ab[15] )." \t ".preg_replace('/\s+/', ' ',$ab[16] )." \t ".preg_replace('/\s+/', ' ',$ab[17] )." \t ".preg_replace('/\s+/', ' ',$ab[18] )." \t ".preg_replace('/\s+/', ' ',$ab[19] )." \t ".preg_replace('/\s+/', ' ',$ab[20] )." \t ".preg_replace('/\s+/', ' ',$ab[21])."\t ".preg_replace('/\s+/', ' ',$ab[22] )." \t ".preg_replace('/\s+/', ' ',$ab[23] )." \t ".preg_replace('/\s+/', ' ',$ab[24])."\t Invalid city category \t".$ab[26];
	}
	else
	{
	$qry=mysqli_query($con1,"select id from avo_branch where name='".$ab[7]."'");
	$rr=mysqli_fetch_row($qry);
	 $result= mysqli_query($con1,"INSERT INTO  Amc(po,cid,atmid,bankname,area,pincode,city,address,Refid,state,amc_st_date,amc_ex_date,cat,branch) VALUES('$po2','$cust','$ab[1]','$ab[2]','$ab[3]','$ab[4]','$ab[5]','".addslashes($ab[8])."','$ab[9]','$ab[6]','".$start."','".$end."','".$ab[28]."','".$ab[7]."')");
	// echo "INSERT INTO  Amc(po,cid,atmid,bankname,area,pincode,city,address,Refid,state,amc_st_date,amc_ex_date,cat,branch) VALUES('$po2','$cust','$ab[1]','$ab[2]','$ab[3]','$ab[4]','$ab[5]','".addslashes($ab[8])."','$ab[9]','$ab[6]','".$start."','".$end."','".$ab[28]."','".$ab[7]."')";
$id=mysqli_insert_id();
