...
<?php
/*if($_SESSION['user']=='Sneha'){
?>
<div align="center"><a href="newalert.php">New Installation</a></div><br />
<?php } */
  
   
       include("form/forms.php");
       ?>
       <!-- Enter Atm ID : <input type="text" name='re' id="re" value="<?php if(isset($_POST['ref'])){ $r=	explode("####",$_POST['ref']); echo $r[0];}   ?>">&nbsp;&nbsp;<input type="button" name="refsub" value="Get Detail" onClick="GetRef('re')" >-->
        <?php
      ...
      $service=new Service();
       if($service->Process())        
       {
    //  $delqry=mysqli_query($con1,"SELECT engineer,count(*) as cnt  FROM `alert_delegation` WHERE `atm`='".$_POST['ref']."' group by engineer order by cnt desc");
      ...
      $branch_id=$bidrow[0];
       while($delrow=mysqli_fetch_row($delqry))
       {
        ...
        {  // delegate
         ...
         $tab=mysqli_query($con1,"update alert set status='Delegated',call_status='1',eta='".$etdt."' where alert_id='".$req."'");
...
<option value="w2pcb"> W2PCB</option>
</select>
</td>
</tr>
