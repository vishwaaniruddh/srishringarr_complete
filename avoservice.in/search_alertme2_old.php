...
if($_POST['bravo']=='all')
	{
 	if(isset($_POST['branch_avo']) && $_POST['branch_avo']!='')
  	{ $stt=$_POST['branch_avo'];
   	$sql.="Select * from alert190221 where branch_id ='".$stt."' ";
  	} 
  	else
   	$sql.="Select * from alert190221 where 1 and branch_id <>''";
}else{
 	if(isset($_POST['branch_avo']) && $_POST['branch_avo']!='')
  	{ $stt=$_POST['branch_avo'];
   	$sql.="Select * from alert190221 where branch_id ='".$stt."' ";
	...
	$sql.="Select * from alert190221 where branch_id='".$br1."' ";
	}
...
{
$eng=$_POST['eng'];
$eng_alert=mysqli_query($con1,"select alert_id from alert_delegation190221 where engineer='".$eng."'  and `call_close_status`='0' ");
$all_alid=array();
...
$sql.=" and alert_id not in (select alert_id from alert_delegation190221 )";
}
...
while($row= mysqli_fetch_row($table))
{
	...
	$tab=mysqli_query($con1,"select feedback,standby,feed_date from eng_feedback190221 where alert_id='".$row[0]."' order by feed_date DESC limit 1");
...
$fetchengid=mysqli_query($con1,"Select e.engg_name,e.phone_no1,e.engg_id from area_engg e,alert_delegation190221 d where e.engg_id=d.engineer and alert_id='".$row[0]."'");
...
?>
<a href="javascript:void(0);" onclick="newwin('masteralert_old.php?id=<?php echo $row[0] ?>','display',700,700)" class="update">
...
/*$qrytime=mysqli_query($con1,"select update_time,up from alert_updates190221 where `alert_id`='".$row[0]."' order by id DESC LIMIT 1");
...
//===For Service and temp  call============================================================
 ...
 $sql=mysqli_query($con1,"select revise_eta,revise_update from alert_progress190221 where alert_id='".$row[0]."' order by pro_id desc limit 1");
