...
$sql="Select * from atm where active='Y'";
 ...
 $sql="Select * from atm where active='Y' and branch_id in(".$br.") ";	
...
$map_qry=mysqli_query($con1,"select site_id from engg_site_mapping_warr where (engg_id !='' or engg_id !=0) ");

else if($status =='unmapp')
$map_qry=mysqli_query($con1,"select site_id from engg_site_mapping_warr where (engg_id ='' or engg_id is NULL) ");
$all_alid=array();
...
$sql.=" and track_id in ($alert_string)";
...
<!---state-->
<td width="95"><?php echo $row[6] ?></td>
<td width="70"><?php echo $row[9] ?></td>
<td width="70"><?php echo $row[15] ?></td>
<td width="70"><?php echo $bran[0] ?></td>
<?
//echo "select engg_id from engg_site_mapping_warr where site_id='$row[0]'";
...
<? $qrybr=mysqli_query($con1,"select engg_id, engg_name,city from area_engg where area='$row[7]' and status=1");
...
<td width="20">		
<input type="button" name="submission" value="submit" onclick="setchange(<?php echo $mapid; ?>)" />
...
?> 
<form name="frm" method="post" action="export_warr_mapp.php" target="_new">
