...
$alert=mysqli_query($con1,"select a.createdby,a.cust_id,a.atm_id,a.bank_name,a.address,a.state1,a.problem,a.entry_date,a.alert_type,a.assetstatus,c.cust_name from alerts_14082019 a,customer c where a.alert_id='".$id."' and a.cust_id=c.cust_id");
...
<tr><th valign="top">Address</th><td valign="top"><?php echo nl2br($alertro[4]); ?></td><th valign="top">Problem</td><td valign="top"><?php echo nl2br($alertro[6]); ?></td></tr>
  ...
  <th>Call Delegated to</th>
    ...
    $del=mysqli_query($con1,"select engg_name from area_engg where engg_id=(select engineer from `alert_delegation_14082019` where alert_id='".$id."')");
    $delro=mysqli_fetch_row($del);
    $eng=$delro[0];
    
    $del2=mysqli_query($con1,"select engg_name from area_engg where engg_id=(select eng_new from alert_delegation_14082019 where alert_id='".$id."')");
...
$tab=mysqli_query($con1,"select * from eng_feedback_14082019  where alert_id='".$id."' order by feed_date DESC");
