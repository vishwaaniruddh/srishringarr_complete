...
$nodes = 'https://avoservice.in/mail_test/sendmailformt.php';

$getcall = 'http://avoservice.in/get_calllog_mail.php';
...
if ($inbox) {
    ...
    if ($unseenMessages) {
        ...
        foreach ($unseenMessages as $messageNumber) {
            ...
            $siteID = extractValue($emailBody, 'Track_id');
        ...
        //   echo "<pre>"; print_r($siteID); echo "</pre>";
                ...
                if(strlen($contactNo) >10 ){$contactNo =''; }
       
        $errcnt=0;
            
            if (isset($siteID) && empty($siteID)) {
                $error1 = "You must provide Either Site/sol/ATM ID or Invoice Number";
            $errcnt++;
            } else { $error1 = "OK";}
            if($problemReported == ''){
            ...
            if($contactNo == ''){
               $error4 = "You must provide only 10 Digits of One Contact Person Number";
               $errcnt++;
            } else { $error4 = "OK";}
 ...
 //  geth the From email======
       ...
       // $toids=getemailtoRemove($toEmails,$emailToRemove);
                foreach ($toEmails as $key => $email) {
                   if ($email === $emailToRemove) {
                       unset($toEmails[$key]); 
                    } 
                }
             foreach ($senderEmail as $key => $email) {
                   if ($email === $emailToRemove) {
                       unset($senderEmail[$key]); 
                    } 
                }
...
<tr> <td style='text-align: center;'><b>Details</b><br><font color='red'> Do not Modify / Change the Text  below </font></td>
<td style='text-align: center;'><b><font color='red'>Required</font></b><br> Provide these details only </td> </tr>";
$reslt.="<tr><td>Track_id</td> <td>".$error1."</td> </tr>";
...
$reslt.="</table></body></html>";
	 ...
	 $msgtitle = "<font color='blue'>Switching <font color='red'>AVO</font> Electro Power Limited</font><br><br> Hi, This is AVO auto Call_log Facility. Are you trying to log the Call?. If Yes, Provide the details as per below format.<font color='red'>Don't Change the details field</font>. For Any other support / updates, Contact AVO Branches.<br><br>";
        ...
        $alertarray = array(
                ...
                $result = file_get_contents($getcall, false, $alertdata);      //echo $result1;   
        
        
         } else {
             $reason = "<br>As per your mail we are able to read the details as above. Not sufficient to proceed to log the call.<br>";
             
             $repl= "<br><b> We request you to kindly send a separate mail which contains the required detail</b><br> <font color='red' size='4'> DO NOT REPLY TO THIS. INITIATE NEW MAIL</font>";
             $data = array(
                 'subject' => $subject,
                 'to' => $to,
...
function getemailtoRemove($removemail,$emailToRemove)
{
    foreach ($removemail as $key => $email) {
                   if ($email === $emailToRemove) {
                       unset($removemail[$key]); 
                    } 
                } return $email;
}
