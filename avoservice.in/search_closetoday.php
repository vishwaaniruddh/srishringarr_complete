...
$sql.=" and state like ('".$_POST['state']."')";
	 ...
	 $sql.=" and state in ($br2)";
	 if(isset($_POST['state']) && $_POST['state']!='')
     $sql.=" and state like ('".$_POST['state']."')";
	}
	if(isset($_POST['fromdt']) && $_POST['fromdt']!='')
	$date=date('Y-m-d',strtotime(str_replace('/','-',$_POST['fromdt'])));
	else
	$date=date('Y-m-d');
$sql.=" and close_date like ('".$date."%')";	
...
<td  valign="top"><?php echo wordwrap($row[25] ,10,"<br />\n",TRUE) ; ?></td>
<td  valign="top"><?php echo wordwrap($row[30],8,"<br />\n",TRUE); ?></td>
<td  valign="top"><?php echo wordwrap($custrow[0],10,"<br />\n",TRUE); ?></td>
<td  valign="top"><?php // echo $row[17]." ".$row[2];
  if($row[17]=='new' || $row[17]=='new temp'){ echo wordwrap($row[2] ,8,"<br />\n",TRUE);} else {  
...
echo "<p style='color:black; font-weight:bold'>"."<br>".wordwrap(date('d/m/Y h:i:s a',strtotime($qrytime1[0])),12,"<br />\n",TRUE) ."</p>";
}
 ...
 <?php /*if($row[15]!='Done' ){ 
  ?>
 <?php if($row[17] != 'new' and $row[17] != 'service'){ ?><br><a href="notify.php?req=<?php echo $row[0]?>&br=<?php echo $br ?>&type=wait">Standby Close</a><?php } ?>

<?php if($row[17] != 'service'){ ?> <br><a href="notify.php?req=<?php echo $row[0]?>&br=<?php echo $br ?>&type=close" style="text-decoration:none; color:#FFFFFF">Permanent Close</a>  <?php } ?>
 <?php
 }*/
...
//echo $row[16];
?>
 <?php if($row[17] != 'new'){ ?><br><a href="notify.php?req=<?php echo $row[0]?>&br=<?php echo $br ?>&type=wait">Standby Close</a><?php } ?>
