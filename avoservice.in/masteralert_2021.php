...
$alert=mysqli_query($con1,"select a.createdby,a.cust_id,a.atm_id,a.bank_name,a.address,a.state1,a.problem,a.entry_date,a.alert_type,a.assetstatus,c.cust_name from alert160222 a,customer c where a.alert_id='".$id."' and a.cust_id=c.cust_id");
...
if($alertro[8]=='service' &&  $alertro[9] ==  'amc')
	...
	elseif($alertro[8]=='service' or 'new' &&  $alertro[9] == 'site')
	...
	$atmidd=$alertro[2];
        ...
        $rejsta=mysqli_query($con1,"select * from `siteproblem` where `alertid`='".$id."'");
		if(mysqli_num_rows($rejsta)>0){
	 	$rejsta1=mysqli_fetch_row($rejsta);
	 	if($rejsta1[2]==0)$ptype=$rejsta1[3];
	 	else{
...
?>
<tr><th>Client</th><td><?php echo $alertro[10]; ?></td><th>Site/Sol/ATM ID</th><td><?php echo $atmidd; ?></td></tr>
<tr><th>Docket No.</th><td><?php echo $alertro[0]; ?></td><th>End User</th><td><?php echo $alertro[3]; ?></td></tr>
<tr><th valign="top">Address</th><td valign="top"><?php echo nl2br($alertro[4]); ?></td><th valign="top">Problem</td><td valign="top"><?php echo nl2br($alertro[6])."<br>Problem Type :".$ptype; ?></td></tr>
  <tr>
  <th>Call Delegated to</th>
    ...
    $del=mysqli_query($con1,"select engg_name from area_engg where engg_id=(select engineer from alert_delegation_2021 where alert_id='".$id."')");
    $delro=mysqli_fetch_row($del);
    $eng=$delro[0];
    $del2=mysqli_query($con1,"select engg_name from area_engg where engg_id=(select eng_new from alert_redelegation where alert_id='".$id."')");
...
$tab=mysqli_query($con1,"select * from eng_feedback160222  where alert_id='".$id."' order by feed_date DESC");
