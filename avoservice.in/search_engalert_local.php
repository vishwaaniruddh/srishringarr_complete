...
<?php
$qry="select * from alertlocal where 1 ";
...
{
$eng=$_POST['eng'];
$qry.=" and alert_id in (select alert_id from alert_delegation where engineer='".$eng."' )";
}
...
*/
$count=$count+1;
//echo "select engg_name from area_engg where engg_id=(select engineer from alert_delegation where alert_id='".$row2[0]."' order by id DESC limit 1)";
$engq=mysqli_query($con1,"select engg_name from area_engg where engg_id=(select engineer from alert_delegationlocal where alert_id='".$row2[0]."' order by id DESC limit 1)");	
$engqr=mysqli_fetch_row($engq);
$engname=$engqr[0];
$cust_qry=mysqli_query($con1,"select cust_id from local_site where track_id='".$row2[1]."'");
...
<td>
<?php if($row2[15]!='Done') { ?>
<input type="button" value="Done" class="readbutton" onclick="javascript:location.href='eng_feedbacklocal.php?alert=<?php echo $row2[0]; ?>&eng_id=<?php echo $row[0]; ?>'"/>
<?php } else { ?>
