...
<? if($_SESSION['designation']==4){
  ...
  $eng=mysqli_query($con1,"select engg_id, engg_name from area_engg where loginid=(select srno from login where username='".$_SESSION['user']."' ") ;
  
  while($engr=mysqli_fetch_array($eng)){ ?>
  <th width="77"><select name="name" id="name">
  <option value="<?php echo $engr[1]; ?>"><?php echo $engr[1]; ?></option>
