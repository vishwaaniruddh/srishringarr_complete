...
if(isset($_POST['cmdasset']))
{
	...
	//echo "select po,cid,servicetype from Amc where amcid='".$_POST['id']."'";
		...
		if($_POST['type']=='amc')
		$qry3=mysqli_query($con1,"Insert into amcassets(`assets_name`,`assetspecid`,`amcid`,`amcpoid`,`siteid`,`quantity`) Values('".$qryrow4[0]."','".$_POST['spec']."','".$qry2row[1]."','".$qry2row[0]."','".$_POST['id']."','".$_POST['quan']."')");
		if($_POST['type']=='amc')
		{
		//echo "hi".$qry2row[2];
...
<tr>
<td>Start Date:</td><td><input type="text" name="adate" id="adate" onclick="displayDatePicker('adate');" value="<?php echo date('d/m/Y'); ?>" /></td>
</tr>
