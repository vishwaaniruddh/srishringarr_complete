...
<?php
$qryaspec = mysqli_query($con1, "Select * from assets_specification where assets_id='" . $row[1] . "' ");
                while ($qryaspec1 = mysqli_fetch_row($qryaspec)) {
                    ?>
<option value="<?php echo $qryaspec1[0]; ?>" <?php if ($qryaspec1[0] == $atmrow[2]) {
                        echo "selected";
                    }
                    ?>><?php echo $qryaspec1[2]; ?></option>
<?php
}
                ?>
...
<option value="0">select</option>
<?php
$qryspec = mysqli_query($con1, "Select * from assets_specification where assets_id='" . $ro[1] . "' ");
                while ($rowspec = mysqli_fetch_row($qryspec)) {
                    ?>
<option value="<?php echo $rowspec[0]; ?>" <?php if ($rowspec[0] == $row[4]) {
                        echo "selected";
                    }
                    ?>><?php echo $rowspec[2]; ?></option>
<?php
}
                ?>
...
<option value="0">Select</option>
<?php
$branches = mysqli_query($con1, "select * from avo_branch");
        while ($branches1 = mysqli_fetch_row($branches)) {
            ?>
