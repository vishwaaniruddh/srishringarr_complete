...
if ($type == 'amc') {
    ...
    $qry = mysqli_query($con1, "SELECT `cid`,`bankname`,`state`,`city`,`address`,`pincode`,`area`,`servicetype` FROM `Amc` WHERE `amcid`='$atm'");
//echo "SELECT `cid`,`bankname`,`state`,`city`,`address`,`pincode`,`area`,`servicetype` FROM `Amc` WHERE `ref_id`='$atm'";;
} elseif ($type == 'site') {
    $qry = mysqli_query($con1, "SELECT `cust_id`,`bank_name`,`state`,`city`,`address`,`pincode`,`area`,`servicetype` FROM `atm` WHERE `track_id`='$atm'");
