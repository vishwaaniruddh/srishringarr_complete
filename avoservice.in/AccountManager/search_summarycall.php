...
<td  valign="top">&nbsp;<?php $tot += $cnt + $row[1];
            echo $row[1];?></td>
</tr>
<?php
}
...
//echo "Select * from alert where state in (".$br1.") order by alert_id DESC";
    ...
    if (mysqli_num_rows($table) > 0) {
        $cnt2 = 0;
        while ($row = mysqli_fetch_row($table)) {
            $sqlx = mysqli_query($con1, 'select cust_name from customer where cust_id=' . $row[0]);
            $rowx = mysqli_fetch_row($sqlx);
            ?>
<tr>
<td  valign="top"><?php echo $rowx[0]; ?></td>
<td  valign="top">&nbsp;<?php $tot2 += $cnt2 + $row[1];
            echo $row[1];?></td>
</tr>
<?php
}
