...
$po = $_GET['po'];
$callid = $_GET['callid'];
$my1 = mysqli_query($con1, "select type,po from pending_installations where id='" . $callid . "'");
...
if ($myrow[0] == "sales") {
    $qryasst = mysqli_query($con1, "Select assets_name,assets_spec,valid,quantity from site_assets where callid='" . $callid . "'");
} else {
    $qryasst = mysqli_query($con1, "Select assets_name,assetspecid,valid,quantity from amcassets where callid='" . $callid . "'");
}
...
while ($asstres = mysqli_fetch_row($qryasst)) {
    $qryasstid = mysqli_query($con1, "Select name from assets_specification where ass_spc_id='" . $asstres[1] . "' ");
