...
$prob = mysqli_real_escape_string($_POST['prob']);
$cname = mysqli_real_escape_string($_POST['cname']);
$cphone = mysqli_real_escape_string($_POST['cphone']);
$cemail = mysqli_real_escape_string($_POST['cemail']);
$ccmail = mysqli_real_escape_string($_POST['ccemail']);
...
$bank_name = mysqli_real_escape_string($demo_atm_sql_result['bank_name']);
$area = mysqli_real_escape_string($demo_atm_sql_result['area']);
$pincode = mysqli_real_escape_string($demo_atm_sql_result['pincode']);
$city = mysqli_real_escape_string($demo_atm_sql_result['city']);

$address = mysqli_real_escape_string($demo_atm_sql_result['address']);
...
if ($check_atm_sql_result) {
    ...
    mysqli_query($con1, "insert into atm (atm_id,cust_id,bank_name,area,pincode,city,branch_id,start_date,address,po,servicetype,podate,expdt,state1,service_date1,service_date2,service_date3,so_id, site_type) values('" . $atm_id . "','" . $custid . "','" . $bank_name . "','" . $area . "','" . $pincode . "','" . $city . "','" . $branch_id . "','" . $start_date . "','" . $address . "','" . $po . "','" . $servicetype . "','" . $po_date . "','" . $expdt . "','" . $state . "','" . $service_date1 . "','" . $service_date2 . "','" . $service_date3 . "','" . $so_id . "', 'site_del')");

    $track_id = mysqli_insert_id();
...
$sub = mysqli_real_escape_string($_POST['sub']);
$doc = mysqli_real_escape_string($_POST['doc']);
...
$last_alert_id = mysqli_insert_id();

mysqli_query($con1, "update so_order set status=2, call_status=2, alert_id ='" . $last_alert_id . "' where id='" . $so_order_id . "'");
...
while ($asset = mysqli_fetch_assoc($sales_order_qry)) {
    ...
    mysqli_query($con1, "INSERT INTO site_assets (cust_id, po, assets_name,assets_spec, valid, quantity, atmid, serialno, type, rate, status, callid,so_id,atm_trackid, start_date, po_date, alert_id) VALUES ('" . $custid . "', '" . $po . "', '" . $product . "','" . $specs . "', '" . $valid . "', '" . $qty . "', '" . $track_id . "', '" . $serialno . "', 'NEW', '', '1', '0','" . $so_id . "','" . $track_id . "','" . $start_date . "','" . $po_date . "','" . $last_alert_id . "')");
    $i++;
}
...
for ($i = 0; $i < count($asset); $i++) {
    ...
    $qryst = mysqli_query($con1, "INSERT INTO `alert_assets` (`id`, `alert_id`, `po`, `assets`, `qty`, `valid`,`pm`) VALUES (NULL, '" . $last_alert_id . "', '" . $po . "', '" . $asstre . "', '" . $qtyfinal . "', '" . $valid . "','" . $_POST['pm'] . "')");
}
