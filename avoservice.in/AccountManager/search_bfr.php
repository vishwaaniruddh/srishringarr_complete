...
// ATM Id Select======
        ...
        $alrtqr = mysqli_query($con1, "select atm_id,assetstatus from alert where createdby='" . $row[1] . "'");
        $alertr = mysqli_fetch_row($alrtqr);

        if ($alertr[1] == 'site') {
            $atmry = mysqli_query($con1, "select atm_id from atm where track_id='" . $alertr[0] . "'");
        } elseif ($alertr[1] == 'amc') {
            $atmry = mysqli_query($con1, "select atmid from Amc where amcid='" . $alertr[0] . "'");
        }

        $atmidrow = mysqli_fetch_row($atmry);
        $atmid = $atmidrow[0];
