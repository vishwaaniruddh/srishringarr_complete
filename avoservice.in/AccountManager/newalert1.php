...
$my1 = mysqli_query($con1, "select type,po,contactno,contactperson,new_email from pending_installations where id='" . $callid . "'");
$myrow = mysqli_fetch_row($my1);
...
while ($sqlst1 = mysqli_fetch_row($sqlst)) {
    ?>
<option value="<?php echo $sqlst1[1]; ?>" <?php if ($sqlst1[1] == $resatm[0]) {
        echo "selected";
    }
...
while ($sqlbr1 = mysqli_fetch_row($sqlbr)) {
    ?>
<option value="<?php echo $sqlbr1[0]; ?>" <?php if ($sqlbr1[0] == $resatm[2]) {
        echo "selected";
    }
