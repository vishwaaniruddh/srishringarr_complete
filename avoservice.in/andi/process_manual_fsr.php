    ...
    if($size_err==0){
        ...
        //foreach($_FILES as $k => $v){
           ...
           // $target_filedir =  "../visituploadapp/".$visit_id.'/'; 
            $target_filedir = "../manual_fsr_1123/" . $alert_id . '/';
           // $target_filedir = "../manual_fsr_test/"; 
            ...
            if (move_uploaded_file($_FILES["image"]["tmp_name"][$name], $target_file)) {
               ...
               // $compress_images = $target_file;
                ...
                //============Upload time record=======
            $qry= mysqli_query($con1,"Select engineer from alert_delegation where alert_id='".$alert_id."'");
            $engrow=mysqli_fetch_row($qry);
            $engg=$engrow[0];
            $updt=mysqli_query($con1,"insert into fsr_upload_time set alert_id='".$alert_id."', created_by='".$engg."', upload_time='".$date."'");    
