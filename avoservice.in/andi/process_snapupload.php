    ...
    if($size_err==0){
        ...
        //foreach($_FILES as $k => $v){
           ...
           // $target_filedir =  "../visituploadapp/".$visit_id.'/'; 
            $target_filedir = "../inst_snaps_new_test/"; 
            ...
            if (move_uploaded_file($_FILES["image"]["tmp_name"][$name], $target_file)) {
               ...
               // $compress_images = $target_file;
                ...
                $snapqry=mysqli_query($con1,"select * from snap_inst_test where alert_id='".$alert_id."'");
                if(mysqli_num_rows==0){
                    if($cnt==1){
                        $inst=mysqli_query($con1,"Insert into snap_inst_test set alert_id='".$alert_id."', full_product='".$compress_images."',created_at='".$date."' ");
                        
                        if ($inst){
                        $alrt=mysqli_query($con1,"UPDATE alert_test set snap_file='Done' where alert_id='".$alert_id."'");	    
                        } 
                    }
                } 
                
                if($cnt==2){ 
                   $inst=mysqli_query($con1,"UPDATE snap_inst_test set front_panel='".$compress_images."' where alert_id='".$alert_id."'");	
                }
                else if($cnt==3){ 
                   $inst=mysqli_query($con1,"UPDATE snap_inst_test set buyback='".$compress_images."' where alert_id='".$alert_id."'");	
                }
                else if($cnt==4){ 
                   $inst=mysqli_query($con1,"UPDATE snap_inst_test set input_volt='".$compress_images."' where alert_id='".$alert_id."'");	
                }
                else if($cnt==5){ 
                   $inst=mysqli_query($con1,"UPDATE snap_inst_test set output_volt='".$compress_images."' where alert_id='".$alert_id."'");	
                }
                else if($cnt==6){ 
                   $inst=mysqli_query($con1,"UPDATE snap_inst_test set earth_volt='".$compress_images."' where alert_id='".$alert_id."'");	
                }
            } else {
                $err = $err + 1;
