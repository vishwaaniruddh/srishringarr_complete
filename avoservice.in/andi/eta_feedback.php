...
//$engqry=mysqli_query($conapp,"select engg_name,engg_id from area_engg where loginid='".$eng_id."'");
...
if(isset($_GET['etatime']))
$etatime =$_GET['etatime'];
$lat='';
$lng='';
$address='';
...
$sql=mysqli_query($conapp,"Insert into eng_feedback(`engineer`,`alert_id`,`feedback`,`feed_date`,`standby`,`lat`,`lng`,`fromplace`) Values('".$login_id."','".$alert."','".$st."','".$cdate."','','".$lat."','".$lng."','".$address."')");
...
mysqli_query($conapp,"insert into eta_feedback(id,eta_time,created_dt,updt_by) values('".$alert."','".$etatime."' ,'".$cdate."' ,'".$login_id."')");
