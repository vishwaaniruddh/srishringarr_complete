    ...
    if($size_err==0){
        ...
        $err_cd = 0;
            ...
            if (!file_exists($target_filedir)) {
                    ...
                    $path = $_FILES["full_product"]["name"][0];
                    $ext = pathinfo($path, PATHINFO_EXTENSION);
                   // $filename = time() . "_" . $name . "." . $ext;
                  $filename = "fullprod.".$ext;
                    $target_file = $target_filedir . $filename;
                     if (move_uploaded_file($_FILES["full_product"]["tmp_name"][0], $target_file)) {
                        $noerr_cd = $noerr_cd + 1;
                        $str = $target_file;
  ...
  //================Front Panel============
        ...
        $err_fp = 0;
            ...
            if (!file_exists($target_filedir)) {
                    ...
                    $path = $_FILES["front_panel"]["name"][0];
                    $ext = pathinfo($path, PATHINFO_EXTENSION);
                   // $filename = time() . "_" . $name . "." . $ext;
                  $filename = "frontpanel.".$ext;
                    $target_file = $target_filedir . $filename;
                    
                     if (move_uploaded_file($_FILES["front_panel"]["tmp_name"][0], $target_file)) {
                        $noerr_fp = $noerr_fp + 1;
                        $str = $target_file;
                        $mod_target_file = ltrim($str,"../");
                        $compress_images = $mod_target_file;
...
//==============Buyback==============
        ...
        $totalfiles_b = $totalfiles_b + 1;
            ...
            $path = $_FILES["buyback"]["name"][0];
                    $ext = pathinfo($path, PATHINFO_EXTENSION);
                   // $filename = time() . "_" . $name . "." . $ext;
                  $filename = "buyback.".$ext;
                    $target_file = $target_filedir . $filename;
                    
                     if (move_uploaded_file($_FILES["buyback"]["tmp_name"][0], $target_file)) {
                        $noerr_b = $noerr_b + 1;
                        $str = $target_file;
                        $mod_target_file = ltrim($str,"../");
                        $compress_images = $mod_target_file;
 ...
 //=========Input Volt==========
        ...
        $err_iv = 0;
            ...
            $totalfiles_iv = $totalfiles_iv + 1;
                    ...
                    $path = $_FILES["input_volt"]["name"][0];
                    $ext = pathinfo($path, PATHINFO_EXTENSION);
                   // $filename = time() . "_" . $name . "." . $ext;
                  $filename = "input_volt.".$ext;
                    $target_file = $target_filedir . $filename;
                    
                     if (move_uploaded_file($_FILES["input_volt"]["tmp_name"][0], $target_file)) {
                        $noerr_iv = $noerr_iv + 1;
                        $str = $target_file;
                        $mod_target_file = ltrim($str,"../");
                        $compress_images = $mod_target_file;
 ...
 $totalfiles_ov = 0;
        ...
        $err_ov = 0;
           ...
           $path = $_FILES["output_volt"]["name"][0];
                    $ext = pathinfo($path, PATHINFO_EXTENSION);
                   // $filename = time() . "_" . $name . "." . $ext;
                  $filename = "output_volt.".$ext;
                    $target_file = $target_filedir . $filename;
                    
                     if (move_uploaded_file($_FILES["output_volt"]["tmp_name"][0], $target_file)) {
                        $noerr_ov = $noerr_ov + 1;
                        $str = $target_file;
                        $mod_target_file = ltrim($str,"../");
                        $compress_images = $mod_target_file;
    ...
    $inst = mysqli_query($con1, "UPDATE snap_inst set output_volt='" . $compress_images . "' where alert_id='" . $alert_id . "'");
        ...
        $path = $_FILES["earth_volt"]["name"][0];
                    $ext = pathinfo($path, PATHINFO_EXTENSION);
                   // $filename = time() . "_" . $name . "." . $ext;
                  $filename = "earth_volt.".$ext;
                    $target_file = $target_filedir . $filename;
                    
                     if (move_uploaded_file($_FILES["earth_volt"]["tmp_name"][0], $target_file)) {
                        $noerr_ev = $noerr_ev + 1;
                        $str = $target_file;
                        $mod_target_file = ltrim($str,"../");
                        $compress_images = $mod_target_file;
