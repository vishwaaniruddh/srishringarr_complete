...
?>
<table width="60%" border="1" cellpadding="2" cellspacing="0" style="margin-top:5px;" class=""  id="call_summary1" >
<?php 
...
// echo $instqry;  
 ...
 $pmqry="SELECT count(*) as count FROM `alert` where branch_id='".$row[0]."' and (alert_type='temp_pm' or alert_type='pm' or alert_type='PM') and close_date Between STR_TO_DATE('$fromdt','%d/%m/%Y') AND STR_TO_DATE('$todt','%d/%m/%Y') + INTERVAL 1 DAY group by branch_id ";
...
<th rowspan="3" width="10%">Engineer Name (click on name for details)</th>
<th rowspan="3" width="6%">Branch</th>
<th rowspan="3" width="6%">Employee Code</th>
<th rowspan="3" width="6%">Designation</th>
<th rowspan="3" width="6%">Date of Join</th>
<th rowspan="3" width="6%">Present As per HR portal</th>
...
while($row= mysqli_fetch_row($table))
{$pts=0;
$prescnt=0; $fieldcnt=0;$inhousecnt=0;$cntsc=0;$cntins=0;$cntpm=0;$derecnt=0;
 ...
 $present="SELECT count(*) FROM `Attendance` where empcode='".$row[5]."' and attendance='p' and date between STR_TO_DATE('$fromdt','%d/%m/%Y') AND STR_TO_DATE('$todt','%d/%m/%Y') ";
 //echo $present;
 
 $pres=mysqli_query($con1,$present);
$presrow=mysqli_fetch_row($pres); 
$prescnt=$presrow[0]; 

 //============= while loop for open call =========================

//$serviceqry= "SELECT distinct(a.alert_id), a.alert_type, b.engineer FROM alert a, alert_delegation b where a.alert_id=b.alert_id and b.engineer='".$row[0]."' and (a.status='Done' or a.call_status='Done') and b.status !=2 and a.close_date Between STR_TO_DATE('$fromdt','%d/%m/%Y') AND STR_TO_DATE('$todt','%d/%m/%Y') + INTERVAL 1 DAY ";

$serviceqry= "SELECT distinct(a.alert_id), a.alert_type, b.engineer FROM alert a, alert_delegation b where a.alert_id=b.alert_id and b.engineer='".$row[0]."' and (a.status='Done' or a.call_status='Done') and a.close_date Between STR_TO_DATE('$fromdt','%d/%m/%Y') AND STR_TO_DATE('$todt','%d/%m/%Y') + INTERVAL 1 DAY ";
...
$inhouseqry="SELECT count(*) as count FROM `eng_mis` where eng_id in(select engg_id from area_engg where area ='".$row[0]."') and  type='In House' and mis_date Between STR_TO_DATE('$fromdt','%d/%m/%Y') AND STR_TO_DATE('$todt','%d/%m/%Y')";
    ...
    if($ser[1]=='service' or $ser[1]=='new temp')$cntsc++; 
   // else if ($ser[1]=='service' or $ser[1]=='new temp') $cntsc++;  
    else if ($ser[1]=='new') $cntins++; 
    else if ($ser[1]=='dere' or $ser[1]=='temp_dere') $derecnt++;
    else if ($ser[1]=='pm' or $ser[1]=='temp_pm' or $ser[1]=='PM') $cntpm++;
...
<!--======= Designation====-->  
 ...
 <!--======= Attendance====-->  
 <td  valign="top"><?php echo $prescnt;  ?></td>
