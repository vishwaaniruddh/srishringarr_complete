...
$sql=mysqli_query($con1,"select * from so_order where id='".$_GET['id']."'");
$row=mysqli_fetch_row($sql);

$sql1=mysqli_query($con1,"select * from new_sales_order where so_trackid='".$row[1]."'");
$row1=mysqli_fetch_row($sql1);
?>
<html>
<head>
<title>Edit-<?php echo $_SESSION['user']; ?></title>
...
<body>
<form name="frm1" action="new_editprocessSO.php" method="post" enctype="multipart/form-data" >
...
<tr>
  ...
  <td>
  <input type="text" name="crndate" id="crndate" value="<?php if($row[16]!='0000-00-00' and $row[16]!='')echo date('d/m/Y',strtotime($row[16])); ?>" onclick="displayDatePicker('crndate');"/>
  </td>
</tr>
<tr><td>&nbsp;</td><td>&nbsp;</td></tr>
<tr>
  ...
  <td>
  <input type="text" name="crnamt" id="crnamt" value="<?php echo $row[17]; ?>" />
  </td>
</tr>
<tr><td>&nbsp;</td><td>&nbsp;</td></tr>
<tr>
