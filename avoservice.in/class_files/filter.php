...
class filter{
	function filter_by($server_name,$db_username,$db_password,$db_name,$arry_final_columns,$table,$array_filtbycol,$array_filtbyval,$order_by_col,$order_by_a_d){
	...
	$result=mysql_query("select $final_col_str from $table where $where_str");
		$con_obj->close_connection();
	}
	else if($order_by_a_d=="a"){
		$result=mysql_query("select $final_col_str from $table where $where_str order by $order_by_col asc");
			$con_obj->close_connection();
		}
	else if($order_by_a_d=="d"){
		$result=mysql_query("select $final_col_str from $table where $where_str order by $order_by_col desc");
			$con_obj->close_connection();
		}
		else return false;
	return $result;
