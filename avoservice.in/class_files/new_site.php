...
class new_site{
	function drop_down($server_name,$db_username,$db_password,$db_name,$sel_id,$sel_table,$ref_col,$sel_value,$show_table,$show_col){
		include_once('database_connection.php');
		$con_obj=new database_connection();
		$con_obj->open_connection($server_name,$db_username,$db_password,$db_name);
		$ref_table=mysql_query("select $sel_id from $sel_table where $ref_col='$sel_value'");
		$ref_id=mysql_fetch_row($ref_table);
		$show_table=mysql_query("select $show_col from $show_table where $sel_id='$ref_id[0]'");
		$con_obj->close_connection();
		return $show_table;
		}
		
		
	function create_new($atm_id,$cust_id,$bank_name,$pincode,$city,$state,$start_date,$address){
		include_once('database_connection.php');
		$con_obj=new database_connection();
		$con_obj->open_connection("localhost","satyavan_acc","Myaccounts123*","satyavan_accounts");
		$area_table=mysql_query("select area from area where pincode = '$pincode'");
		$con_obj->close_connection();
		$area=mysql_fetch_row($area_table);
