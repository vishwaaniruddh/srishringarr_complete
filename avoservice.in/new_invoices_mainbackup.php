...
global $con2;
    ...
    $sqlpo = mysqli_query($con1,"select * from purchase_order where id='".$po."'");
    $po_result = mysqli_fetch_assoc($sqlpo);
    $person_id = $sql_result['$po_result'];
...
<td valign="top">&nbsp;<?php echo $po_result['delivery_mode']; ?> </td>
    ...
    function setSubmit(id)
{
 ...
 $.ajax({
  ...
  <input type="hidden" name="qr" value="<?php echo $sqlqry; ?>" readonly>    
  
    
    
    <input type="submit" name="cmdsub" value="Export" > <span>(You can Export MAX 2000 Records at one Time.)</span>
    </form>
