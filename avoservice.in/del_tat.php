...
<script>
  function opennew(eid)
  { //alert(eid);
   var fromdt=document.getElementById('fromdt').value;//alert(fromdt);
   var todt=document.getElementById('todt').value;//alert(todt);
   window.open("eng_calldetails.php?eid="+eid+"&fromdt="+fromdt+"&todt="+todt, "_blank", "toolbar=yes, scrollbars=yes, resizable=yes, top=200, left=200, width=800, height=600");
  }
</script>
<script>
///////////////////////////////search 
...
document.getElementById("search").innerHTML ="<center><img src=loader.gif></center>";
		  ...
		  var calltype=document.getElementById('calltype').value;//alert(calltype);	 		  
			 if(a!="Loading"){
				var calltype=document.getElementById('calltype').value;//alert(calltype);	 			 	
			 	var fromdt=document.getElementById('fromdt').value;//alert(fromdt);
			 	var todt=document.getElementById('todt').value;//alert(todt);
				var branch_avo=document.getElementById('branch_avo').value;
 	...
 	//alert(br);
			...
			if(a!="Loading"){ 
   pmeters = 'calltype='+calltype+'&Page='+b+'&perpg='+ppg+'&fromdt='+fromdt+'&todt='+todt+'&branch_avo='+branch_avo;
			//alert(pmeters);
			}
			else
			{
...
//alert(calltype);
if(calltype=="brsummary")
{
document.getElementById('branch_avo').style.display="none";
}
else
{
document.getElementById('branch_avo').style.display="";
}
...
<body onLoad="funchange();searchById('Loading','1','')">
...
<h2 class="h2color">Delegation & ETA Timeslot</h2>

<div >
<button id="myButtonControlID" onClick="tableToExcel('callsummary', 'CLOSED SERVICE CALL TAT ANALYSIS')">Export</button>
	<br />
<table cellpadding="" cellspacing="0" >
  <tr>
  		
    	<th width="77" colspan="">    		    
    			<select name="calltype" id="calltype" onchange="funchange();searchById('Listing','1','');">
...
<div id="br">     	
     ...
     <input type="text" name="fromdt" id="fromdt" readonly="readonly" onclick="displayDatePicker('fromdt');" placeholder="From Alert Date"/></th>
     
     	<!--To Date-->
   		<th width="75" colspan="2">
   		<input type="text" name="todt" id="todt" readonly="readonly" onclick="displayDatePicker('todt');" placeholder="To Alert Date"/></th>
