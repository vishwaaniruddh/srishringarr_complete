...
//alert(br);
		...
		// alert(calltype);
			...
			var url = 'search_engalert_local.php'; 
...
$sql1=mysqli_query($con1,"Select * from alert_delegationlocal where engineer='".$row2[0]."' and alert_id in (select alert_id from alertlocal where call_status<>'Done')");
...
while($row1=mysqli_fetch_row($sql1)) {
	$atmrow='';
	//echo "select * from local_site where track_id='".$row1[2]."'";
	$cust_qry=mysqli_query($con1,"select * from local_site where track_id='".$row1[2]."'");
	$cust=mysqli_fetch_row($cust_qry);
	
	//echo "select * from alert where alert_id='".$row1[3]."'";
	//echo "<br>select * from alert where alert_id='".$row1[3]."' where call_status<>'Done'";
$sql2=mysqli_query($con1,"select * from alertlocal where alert_id='".$row1[3]."'and call_status<>'Done'");	//$sql2=$sel_obj->select_rows('localhost','satyavan_acc','Myaccounts123*','satyavan_accounts',array("*"),"alert","alert_id",$row1[0],array(""),"y","","");
...
<td><?php echo $cust[1]; ?></td>
<td><?php echo $cust[2]; ?></td>
...
<td><?php
 ...
 $sql3=mysqli_query($con1,"select assets,qty from alert_assetslocal where alert_id='$row2[0]'");
       while($row3=mysqli_fetch_row($sql3))
       echo $row3[0]."($row3[1])".", ";}
       ?></td>
<td>
<?php if($row2[15]!='Done') { ?>
<input type="button" value="Done" class="readbutton" onclick="javascript:location.href='eng_feedbacklocal.php?alert=<?php echo $row1[3]; ?>&eng_id=<?php echo $row[0]; ?>'"/>
<?php } else { ?>
