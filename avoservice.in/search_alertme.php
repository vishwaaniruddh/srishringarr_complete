<html><body>nf vjfbvj,nv</body><html>
<?php
session_start();
...
$sql.=" and (alert_id not in (select alert_id from alert_delegation) or alert_id in (select alert_id from alert_delegation where engineer='0'))";
}
...
<td width="90" valign="top">&nbsp;<?php echo $row[25]; ?></td>
...
<td width="72" valign="top">&nbsp;<?php  echo $row[17]." ".$row[2];
  if($row[17]=='new' || $row[17]=='new temp' || $row[17]=='temp_pm'){ echo $row[2]; } else {  
...
echo $getoldname[0]."<br>".$getoldname[1];
  ?></td>
  <td width="75" valign="top">&nbsp;
<?php 
...
<?php if($row[17] != 'new'){ ?> <br><a href="notify.php?req=<?php echo $row[0]?>&br=<?php echo $br ?>&type=wait&ctp=<?php if($row[17]=='new'){ echo "new";}else{ echo "service"; } ?>">Standby Close</a><?php } ?>

<br><a href="notify.php?req=<?php echo $row[0]?>&br=<?php echo $br ?>&type=close&ctp=<?php if($row[17]=='new'){ echo "new";}else{ echo "service"; } ?>" style="text-decoration:none; color:#FFFFFF">Permanent Close</a>
 <?php
 }
