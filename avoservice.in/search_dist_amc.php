...
$sql.=" and amcid in(select site_id from engg_site_mapping where engg_id ='".$engg."')";
}
...
$qry1=mysql_query("select cust_name from customer where cust_id='$row[1]'");
$crow=mysql_fetch_row($qry1);
$qrybr=mysql_query("select name from avo_branch where id='$row[8]'");
...
<?
$qryt1=mysql_query("select engg_id from engg_site_mapping where site_id='".$row[0]."'");
...
?> 
<form name="frm" method="post" action="export_dist_calc.php" target="_new">
