...
if (mysqli_num_rows($table) > 0) {
    while ($row = mysqli_fetch_row($table)) {
        ...
        $qry1 = mysqli_query($conc,"select * from customer where cust_id='$row[2]'");
...
<td width="75"><?php
        ...
        $qry2me = mysqli_query($conc,"select * from site_assets where cust_id='$row[2]' and po='$row[11]' and atmid='" . $row[0] . "'");
        while ($detailme = mysqli_fetch_row($qry2me)) {
/*echo "select * from assets_specification where ass_spc_id='$detail[4]'";
$qry3=mysqli_query($conc,"select * from assets_specification where ass_spc_id='$detail[4]'");
