...
$alert = mysqli_query($concs,"select a.createdby,a.cust_id,a.atm_id,a.bank_name,a.address,a.state1,a.problem,a.entry_date,a.alert_type,a.assetstatus,c.cust_name from alert a,customer c where a.alert_id='" . $id . "' and a.cust_id=c.cust_id");
$alertro = mysqli_fetch_row($alert);
if ($alertro[8] == 'service' && $alertro[9] == 'amc') {
    $atm = mysqli_query($concs,"select atmid from Amc where amcid='" . $alertro[2] . "'");
}

if ($alertro[8] == 'service' && $alertro[9] == 'site') {
    $atm = mysqli_query($concs,"select atm_id from atm where track_id='" . $alertro[2] . "'");
}
...
$del = mysqli_query($concs,"select engg_name from area_engg where engg_id=(select engineer from alert_delegation where alert_id='" . $id . "')");
$delro = mysqli_fetch_row($del);
$eng = $delro[0];
$del2 = mysqli_query($concs,"select engg_name from area_engg where engg_id=(select eng_new from alert_redelegation where alert_id='" . $id . "')");
...
$tab = mysqli_query($concs,"select * from eng_feedback  where alert_id='" . $id . "' order by feed_date DESC");
while ($row = mysqli_fetch_row($tab)) {

    $qry = mysqli_query($concs,"select * from login where srno='" . $row[2] . "'");
