...
//echo $atmid."<br>".$qry2;
$flag = 0;
$qry = mysqli_query($atmid);
if (mysqli_num_rows($qry) > 0) {
    $flag = 1;
//echo "No result found<br>";
    while ($row = mysqli_fetch_array($qry)) {
        ?>
<a href="service_test.php?id=<?php echo $row[2]; ?>&type=amc"><?php	echo $row[0] . "-" . $row[3] . "(Amc Site)"; ?></a></br>
<?php	echo $row[1]; ?><br><br>
    <?php
}}
//echo $qry2;
$qr = mysqli_query($qry2);
if (mysqli_num_rows($qr) > 0) {
    $flag = 1;
    while ($row2 = mysqli_fetch_array($qr)) {
        ?>
<a href="service_test.php?id=<?php echo $row2[2]; ?>&type=site"><?php	echo $row2[0] . "-" . $row[3] . "(New Site)"; ?></a></br>
