...
<div id="header">
<form action="process_delegation.php" method="post" name="form" onsubmit="return validate(this)";>
<table border="1">
<?php
...
$logcal_time=mysqli_query($concs,"select `entry_date` from `alert` where `alert_id`='".$_GET['req']."' ");
...
$alert=mysqli_query($concs,"select a.createdby,a.cust_id,a.atm_id,a.bank_name,a.address,a.state1,a.problem,a.entry_date,a.alert_type,a.assetstatus,c.cust_name,a.branch_id,a.custdoctno from alert a,customer c where a.alert_id='".$req."' and a.cust_id=c.cust_id");
...
if($alertro[9] ==  'amc')
   { $atm=mysqli_query($concs,"select atmid, latitude, longitude from Amc where amcid='".$alertro[2]."'");
   // echo "select atmid, latitude, longitude from Amc where amcid='".$alertro[2]."'";
   }
if($alertro[9] == 'site'){
$atm=mysqli_query($concs,"select atm_id, latitude, longitude from atm where track_id='".$alertro[2]."'");
...
<?php
$branch_avo=mysqli_query($concs,"select * from `avo_branch`");
...
<option value="0">select</option>
    <?php
$sql="select engg_id,engg_name,area,city,engg_desgn, latitude, longitude from area_engg where status=2 order by engg_name ASC";
...
$eng=$row[0];
?>
    <option value="<?php echo $eng; ?>"> <?php echo $row[1]."  (".$row[4]."-".$r2[0].")"; ?></option>
    <?php
}
?>
  </select>
