...
if ($stat == 1 || $tmcnt >= 1) {
    echo "Sorry you cannot log the call. Either Call already in OPEN or It seems Site Issue Repeated within 30 Days. Please contact Technical team / help desk of AVO for assistance.";} else if ($stat == 1 || $rcnt >= 5) {
    ...
    if (mysqli_num_rows($chksql) == 0) {
        $sql = mysqli_query($conc,"INSERT INTO `alert` (`alert_id`, `cust_id`,`atm_id`, `bank_name`, `area`, `address`, `city`, `branch_id`, `pincode`, `problem`, `entry_date`, `alert_date`, `caller_name`, `caller_phone`, `caller_email`, `status`, `call_status`, `alert_type`, `close_date`, `standby`, `po`, `assetstatus`, `appby`, `appref`,`state1`,`createdby`,`subject`,`custdoctno`,`ccmail`,`whatsapp`) VALUES (NULL, '" . $_POST['cust'] . "','" . $_POST['ref'] . "' , '" . $_POST['bank'] . "', '" . $_POST['area'] . "', '" . $_POST['add'] . "', '" . $_POST['city'] . "', '" . $_POST['branch'] . "', '" . $_POST['pin'] . "', '" . $_POST['prob'] . "', '" . $dt . "', '" . $_POST['alertdt'] . "', '" . $_POST['cname'] . "', '" . $_POST['cphone'] . "', '" . $_POST['cemail'] . "', 'Pending', 'Pending', 'service', '', '', '" . $_POST['po'] . "','" . $_POST['type'] . "', '" . $_POST['appby'] . "', '" . $_POST['how'] . "','" . $_POST['state'] . "','" . $qry2ro[0] . "_" . date("ymd") . $num3 . "','" . $_POST['sub'] . "','" . $_POST['docket'] . "','" . $ccm . "' ,'" . $wnatsno . "')");
...
//print_r($cc);
        $subject = mysqli_real_escape_string($_POST['sub']);
//echo "<br>";
...
//echo $tbl."<br>";
        ...
        if ($alertr[21] == 'site') {$asstatus = "Warranty";} elseif ($alertr[21] == 'amc') {$asstatus = "AMC";} else {
            $asstatus = "PCB";
        }

        if ($alertr[17] == 'new') {$calltp = "New Installation";} elseif ($alertr[17] == 'service' || $alertr[17] == 'new temp') {$calltp = "Service Call";} elseif ($alertr[17] == 'pm' || $alertr[17] == 'temp_pm') {$calltp = "PM Call";} elseif ($alertr[17] == 'dere' || $alertr[17] == 'temp_dere') {$calltp = "De-Re Installation";}
