...
document.getElementById("search").innerHTML ="<center><img src=loader.gif></center>";
 	...
 	//alert(br);
			...
			if(a!="Loading"){ 
   pmeters = 'br='+br+'&Page='+b+'&perpg='+ppg+'&fromdt='+fromdt+'&todt='+todt+'&calltype='+calltype+'&branch='+branch;
			//alert(pmeters);
			}
			else
			{
...
<h2 class="h2color">Inst Report - Status Summary</h2>
<h4 class="h2color">From 01-08-2021</h4>
...
<table cellpadding="" cellspacing="0" >
  <tr>
       ...
       <?   if ($_SESSION['designation'] !=3) { ?>   
              ...
              <? } ?>
              <option value="engrsummary">Engineer wise Summary</option> 
       ...
       <th width="77" colspan="">
           ...
           <? 
            if ($_SESSION['designation'] !=3) { 
           $brqry=mysqli_query($con1,"select * from avo_branch order by name ASC") ;
            } else 
            $brqry=mysqli_query($con1,"select * from avo_branch where id in('".$_SESSION['branch']."')") ;
           
           ?>
           <option value="">Select Branch</option> 
           <? while($br=mysqli_fetch_row($brqry)) { ?>
