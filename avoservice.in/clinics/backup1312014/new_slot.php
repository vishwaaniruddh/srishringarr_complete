...
{
//echo "hi";
//echo "select * from slot where hospital='".$hos."' and center='".$_POST['center']."'";
$search=mysql_query("select * from slot where hospital='".$hos."' and center='".$_POST['center']."' and app_date='".date('Y-m-d', strtotime($appdate .' +'.$i.' days'))."'");
...
{
$qr=mysql_query("select max(block_id) from slot");
$qrro=mysql_fetch_row($qr);
$blockid=($qrro[0]+1);
$sql="insert into slot (block_id,hospital,app_date,start_time,end_time,center) values ('$blockid','$hos','".date('Y-m-d', strtotime($appdate .' +'.$i.' days'))."','$time','$time1','".$_POST['center']."')";
//echo $sql." <br>";
$res=mysql_query($sql);
if(!$res)
{
...
<td width="239" height="54">
 <input type="text" name="center" id="center" onkeyup="lookup2(this.value,this.id,'centersuggestions','centerautoSuggestionsList','centerref1');" value="Borivali"  />
              <div class="suggestionsBox" id="centersuggestions" style="display: none; position:absolute; left:270px; z-index:10">
