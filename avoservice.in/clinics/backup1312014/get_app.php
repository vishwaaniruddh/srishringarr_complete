...
// echo $row[12];
  if($row[12]=='0' && $row[2]>=date("Y-m-d")){ ?>
   ...
   <?php if($row[2]==$dt && ($row[6]!='w')){ ?> <a href='#' onclick="Changestatus('<?php echo $row[0] ?>','1')"> Appointment </a>
	<?php
	}
	}
    ...
    <a href='#' onclick="Editapp('<?php echo date('d/m/Y',strtotime($row[2]));  ?>','<?php echo $row[0] ?>','<?php echo $row[7]; ?>','<?php echo $row[13]; ?>')"> Edit </a>
    <br /><br />
	<?php if($row[2]==$dt){ ?><a href='#' onclick="Changestatus('<?php echo $row[0] ?>','2')">Visiting Waiting</a><?php } ?>
    <?php }
	elseif($row[12]=='2'){ ?>
	Consulting
    <?php }
