...
$opd=mysql_query("select * from pre-investigation where p_id='$id' order by current_date");
...
<li ><a href="#<?php echo $opd1[0]."o"; ?>"><?php if(isset($opd1[3]) and $opd1[3]!='0000-00-00') echo date('d-m-Y',strtotime($opd1[3])); ?> (Pre Investigation)</a>
		...
		<ul id="issues"><!------------------------------------OPD--------------------------------------------------------------------------------------------------->
             <?php $opd2=mysql_query("select * from pre-investigation where p_id='$id' order by current_date");
