...
xmlhttp.onreadystatechange=function()
  ...
  document.getElementById('netamt').value=xmlhttp.responseText;
...
<script>
function displayDiss()
{
	 packamtt = document.getElementById('packamt').value;
	 disamtt = document.getElementById('disamt').value;
	 
	 //alert(packamtt)
	 netamtt = Number(packamtt)-Number(disamtt);
	 //alert(netamtt);
     document.getElementById('netamt').value=netamtt;
...
<div class="M_page">
         ...
         <form method="post" class="signin" action="update_patient.php" onSubmit="return validate(this)" name="form" onSubmit="popup() enctype="multipart/form-data">
                ...
                <td>Patient ID:</td> <td> <input type="text" readonly="readonly" name="id" id="id" value="<?php echo $id ; ?>" /></td>
...
?>
                  ...
                  <tr>
                   <td><label class="timegiven"> Centre:</label></td>
                    ...
                    <td>
                    <input type="hidden" name="oldcenter" id="oldcenter" value="<?php echo $row[19]; ?>" />
                    <select name="center" id="center">
                   <option value="">Select</option>
                <?php $area1=mysql_query("select * from area ORDER BY name ASC");
				while($area=mysql_fetch_row($area1)){
				?>
                <option value="<?php echo $area[0]; ?>" <?php if(strcasecmp($row[19],$area[0])==0)  echo "selected"; ?>><?php echo $area[0]; ?></option>
                <?php } ?>
 ...
 <option value="married" <?php if($row[71]=='married'){ ?> selected="selected"<?php }  ?>>Married</option>
  <option value="unmarried" <?php if($row[71]=='unmarried'){ ?> selected="selected"<?php }  ?>>Unmarried</option>
                   ...
                   <tr><td><label>Type :</label></td><td><select name="pattype" id="pattype" onchange="showpack('pattype');">
                  ...
                  <?php
                  	...
                  	if($row[9]!='')
                  	{
                  		//echo "select  * from patient  where srno='".$row[9]."'";
                  		$ref_qry=mysql_query("select  * from patient  where srno='".$row[9]."'");
                  		$ref_row=mysql_fetch_array($ref_qry);
                  		echo $ref_row['NAME'];
                  	}
                  	else
                  	{
                ...
                <div id="package" style="display:<?php if($row[0]=='nr'){ ?>none<?php }else{ ?> block <?php } ?>">
                  <table width="1026"><tr><td width="262"><label>Select Package:<?php echo $pac[2]; ?></label>&nbsp;&nbsp;<a href="#" onclick="packagewindow('pack');"><img src="images/add.png" height="15px" width="15px" title="Add New Package" /></a></td><td width="261"><select name="pack" id="pack" onchange="getpackamt(this.id,'packamt');">
   ...
   <option value="<?php echo $packro[0]; ?>"<?php if($packro[1]==$pac[2]){?> selected="selected"<?php } ?>><?php echo $packro[1]; ?></option>
                  <?php
				   ...
				   ?></select></td><td width="108">
                   <label>Package Start Date:</label></td><td width="375">
                   <input type="text" name="stdt" id="stdt" value="<?php if(mysql_num_rows($packr)>0){ echo date("d/m/Y",strtotime($pac[3])); }else{ echo date("d/m/Y");  } ?>"  onClick="displayDatePicker('stdt');"/></td></tr>
                   
                   
                   <tr><td><label>Package Amount :</label></td><td colspan="3"><input type="text" readonly="readonly" name="packamt" id="packamt" value="<?php echo $pagamt=$pac[6]+$pac[7]; ?>" /></td></tr>
                   <tr><td><label>Discount Amount :</label></td><td colspan="3"><input type="text" name="disamt" id="disamt" value="<?php echo $pac[7]; ?>" onkeyup="displayDiss()" /></td></tr>
                   <tr><td><label>Net Amount :</label></td><td colspan="3"><input type="text" readonly="readonly" name="netamt" id="netamt" value="<?php echo $pac[6]; ?>" /></td></tr>
               ...
               <input type="hidden" name="oldno" id="oldno" value="<?php echo $row[6]; ?>" />
