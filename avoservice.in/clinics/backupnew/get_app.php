...
// echo $row[12];
  if($row[12]=='0'){ ?>
    ...
    <br /><br />
   <?php if($row[2]==$dt && ($row[6]!='w')){ ?> <a href='#' onclick="Changestatus('<?php echo $row[0] ?>','1')"> Make Waiting Outside </a>
	<?php
	}
	}
	elseif($row[12]=='1'){ 
    ...
    <a href='#' onclick="Editapp('<?php echo date('d/m/Y',strtotime($row[2]));  ?>','<?php echo $row[0] ?>','<?php echo $row[7]; ?>','<?php echo $row[13]; ?>')"> Edit </a>
    <br /><br />
	<?php if($row[2]==$dt){ ?><a href='#' onclick="Changestatus('<?php echo $row[0] ?>','2')">Send for OPD</a><?php } ?>
    <?php }
	elseif($row[12]=='2'){ ?>
	Inside OPD
    <?php }
...
//echo "Amt".$bal;
	?>
    <a href="#" onClick="window.open('opdpayment.php?appid=<?php echo $row[0];  ?>&amt=<?php //echo $bal; ?>&pid=<?php echo $row[8]; ?>','opdpay','width=600,height=300,left=200,top=100')" style="text-decoration:none; color:#099">Make Payment</a>
    <?php
	}
	else
	{
	...
	<a href="#" onClick="window.open('opdpayment.php?appid=<?php echo $row[0];  ?>&amt=<?php //echo $feero[0]; ?>&pid=<?php echo $row[8]; ?>','opdpay','width=600,height=300,left=200,top=100')" style="text-decoration:none; color:#099">Make Payment</a>
    <?php
	}
	 }
	elseif($row[12]=='5')
    ...
    <td width="51" height="31"><?php if($row[12]=='0'){ ?><a href="javascript:confirm_delete3('<?php echo $row[0]; ?>');"> Delete </a><?php } ?></td>
    </tr>
    </tbody>
...
<input type="text" name="texting" id="texting" onkeyup="lookup(this.value,this.id,'textsuggestions','textautoSuggestionsList','mailref1');" style="background:#fff;border:1px solid #ac0404;width:150px;" value="" placeholder="Email"   />
...
<input type="hidden" name="msg" value="<?php echo $query; ?>" /><br />
