...
$appid=$_POST['appid'];
$amount=$_POST['amount'];
$packidme=$_POST['pkgid'];
//echo "packidme=".$packidme;
if($amount>0)
{
	//echo "Insert into opd_collection(`appid`,`amt`) Values('".$appid."','".$amount."')";
$qry=mysql_query("Insert into opd_collection(`appid`,`amt`,`patientid`,`paymode`,`problems`,`date`,`packid`) Values('".$appid."','".$amount."','".$_POST['pid']."','".$_POST['bank_account']."','".$_POST['prob']."',STR_TO_DATE('".$_POST['Datepay']."','%d/%m/%Y'),'".$packidme."')");
if(!$qry)
echo mysql_error();
//echo "update appoint set presstat='5' where app_real_id='".$appid."'";
$payid=mysql_insert_id();
if($qry)
$qry444=mysql_query("INSERT INTO `paydetails` (`id`, `payid`, `mode`, `chkno`, `bankname`, `chkdate`, `status`,`packid`) VALUES ('', '".$payid."', '".$_POST['bank_account']."', '".$_POST['chqno']."', '".$_POST['bname']."', STR_TO_DATE('".$_POST['Datechq']."','%d/%m/%Y'), '0','".$packidme."')");
if(!$qry)
