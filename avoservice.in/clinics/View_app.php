...
function searchById(Mode,Page) {
 //alert("hi");
		  ...
		  /*if(document.getElementById('searchdate').value=="" && document.getElementById('fname22').value=="" && document.getElementById('cont').value=="")
		  {
			...
			var pmeters = 'mode='+Mode+'&Page='+Page+'&name='+name+'&searchdate='+searchdate+'&cont='+cont+'&hos='+hos+'&ref='+ref+'&dept='+<?php echo $_SESSION['dept']; ?>+'&center='+center;
...
function Changestatus(appid,stat,page)
{
//alert(appid+" "+stat);
var xmlhttp;
if (window.XMLHttpRequest)
...
xmlhttp.open("GET","add_update.php?rem="+rem+"&id="+ld+"&type=o",true);
xmlhttp.send();
...
<legend> <h1><img src="ddmenu/apointment.png" style="width:50px; height:50px;" />
<?php  if(isset($_GET['timestamp'])){ echo  strftime("%B",$_GET['timestamp']); }else{ ?>Today's <?php } ?> Appointments</h1></legend>
   ...
   <td>
	<select name="center" id="center" onchange="searchById('Listing','1');">
		...
		<?php } ?>
          ...
          window.open('app_print.php?name='+name+'&searchdate='+searchdate+'&cont='+cont+'&center='+center, '_BLANK')">Print</button>
