...
<?php
 ...
 $sql.="Select * from new_sales_order where status=2 ";
...
$so_ordqry=mysqli_query($con1,"Select po_id from so_order where inv_date between '".$fromdt."' AND '".$todt."'");
$so_allid=array();
while($so_ord=mysqli_fetch_row($so_ordqry)) {
$so_allid[]=$so_ord[0];
}
$string1 = implode(",",$so_allid);

$sql.=" and so_trackid in($string1)";
...
{
$exec=$_POST['exec'];
$poqry=mysqli_query($con1,"Select id from purchase_order where salesperson='".$exec."'");
$po_allid=array();
while($po=mysqli_fetch_row($poqry)) {
$po_allid[]=$po[0];
}
$string = implode(",",$po_allid);

$sql.=" and po_trackid in($string)";
}
//echo $sql;
...
$poname=mysqli_query($con1,"select po_no, salesperson from `purchase_order` where `id`='".$row[1]."'");
$po_no=mysqli_fetch_row($poname);

global $con2;
    $exe_sql = mysqli_query($con2,"SELECT exe_name FROM salesteam where exe_id='".$po_no[1]."'");
	$exe = mysqli_fetch_assoc($exe_sql)
		?>
...
<td  valign="top" width="200">&nbsp;<?php echo $exe['exe_name']; ?></td>

<!--===Customer Name===-->
<td  valign="top">&nbsp;<?php 
$cuname=mysqli_query($con1,"select `cust_name` from `customer` where `cust_id`='".$row[3]."'");
...
<!--===PO===-->
<td  valign="top">&nbsp;<?php echo $po_no[0]; ?></td>
...
<td  valign="top">
<?php
$branch_qry=mysqli_query($con1,"select * from avo_branch where id='".$row[4]."'");
...
<!--=== Status ===--> 
<td  valign="top">
<? if ($so[19]=='c' ) echo "Invoice Cancelled";
   if ($so[19]=='h' ) echo "Invoice on Hold";
...
if ($so[9]=='0000-00-00') echo "ETA: ".$so[7]; 
else echo $so[9] ;
?>  </td>
...
You Can export Max.2000 records 
</form>
