...
$contents='';
 $contents.="Sr no \t Customer Vertical \t so_trackid \t PO Number \t PO ID  \t Buyer Name \t ATM ID \t Branch \t Delivery Type \t Installation Request \t   Buyback \t Contact Name \t  Contact Phone \t Contact Email \t Success\t  Message \t";
    ...
    for($i = 1; $i<=$row; $i++){
        ...
        $remarks =  $rowData[$i][0][16];
        $product = $rowData[$i][0][17];
        
        $whatsapp = $rowData[$i][0][18];

      //  $so_by  = $rowData[$i][0][21];
      
      $so_by  = "71";
      
      $check_po_sql = mysqli_query($con1,"select * from purchase_order where po_no ='".$po_no."'");
    ...
    for($j = 0 ; $j < $product_count; $j++){
            ...
            $check_productname= mysqli_query($con1,"select * from assets where assets_name='".$product_name."'");
            $product_idrow = mysqli_fetch_assoc($check_asset_sql); 
                    
            $product_id = $product_idrow['assets_id'];
