    ...
    $total_pages = ceil($total_rows / $no_of_records_per_page);
        ...
        <div id="search_filters">
			...
			?>
                ...
                <option value="<?php echo $branch_sql_result['id']; ?>" <?php if( $_POST['avo_branch'] ==$branch_sql_result['id']  ){ echo 'selected'; } ?> >
           ...
           $cust_sql = mysqli_query($con1,"select * from customer where cust_id in($cust)");
            ...
            while($cust_sql_result = mysqli_fetch_assoc($cust_sql)){ ?>
                ...
                <option value="<?php echo $cust_sql_result['cust_id'];?>"  <?php if( $_POST['customer_vertical'] ==$cust_sql_result['cust_id']  ){ echo 'selected'; } ?>  >
...
<!-- <input type="text" name="atm_id" id="atm_id" class="form-control" placeholder="Site/Sol/ATM ID"/> -->
       ...
       if(isset($_POST["invno"]) || isset($_POST["branch_avo"]) || isset($_POST["cust"]) || isset($_POST["fromdt"]) || isset($_POST["todt"]) || isset($_POST["atm_id"]) || isset($_POST["sub_date"]) || isset($_POST["filter"]) || isset($_POST["bank_name"]) || isset($_POST["docketno"])){
...
<?php if ($_SESSION['user']=='masteradmin'){ ?>
...
<td>
    <?php $qryfirst="SELECT Remarks_update FROM `SO_Update` WHERE so_id='".$id."' and remarks_type=1 order by updt_id desc ";
 //   echo $qryfirst;
...
<td>
 ...
 //************===barcode complete if call_status=1 If no UPS proceed========**********       
    ...
    if(mysqli_num_rows($assetqry) !=0 && $prepared_sql_result['call_status'] =='1'){     
...
else if(mysqli_num_rows($assetqry) ==0 or $del_type=='stock_trfr'){
     ...
     }  elseif(mysqli_num_rows($assetqry) !=0 && $prepared_sql_result['call_status'] !='1') { ?>
    ...
    function setSubmit(id)
{
 ...
 $.ajax({
    ...
    <!--<form name="frm" method="POST" action="https://avoservice.in/exportexcel_new.php" style="text-align: center;"> -->
