...
$sql="SELECT a.*,b.* FROM so_order a, new_sales_order_asset b  WHERE a.po_id = b.so_trackid AND a.status=2 and a.inv_value in (0,1) ";

if(isset($_POST['product']) && $_POST['product']!='')
{

$sql .= " AND b.po_product='".$product."'";
}

if(isset($_POST['$specs']) && $_POST['$specs']!='')
{
$sql .=" and b.po_model = '".$specs."'";
}
...
{ 
echo $detailme[3].' Cap:' .get_spec($detailme[4]).' ||<br>Qty:'.$detailme[5].') || Warr:'.$detailme[6].' ||<br>Rate:'.$detailme[7]."</br>";
} 
?>
</td>
<? if($row['status'] == 2){

//echo "select alert_id from site_assets where so_id='".$so_id."'";
...
<form name="frm" method="post" action="export_zerovalue_sales.php" target="_new">
