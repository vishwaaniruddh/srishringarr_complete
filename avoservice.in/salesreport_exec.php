...
document.getElementById("search").innerHTML ="<center><img src=loader.gif></center>";
		 ...
		 var cid=document.getElementById('cid').value;//alert(cid);
			 if(a!="Loading"){
				...
				var exec=document.getElementById('exec').value;//alert(exec);
				
				var branch=document.getElementById('branch').value;//alert(branch);
			  }
			 // alert(br);
			//alert("gg"); 
			var url = 'search_salesrep_exec.php';
 	...
 	//alert(br);
			...
			if(a!="Loading"){ 
   pmeters = 'cid='+cid+'&Page='+b+'&perpg='+ppg+'&fromdt='+fromdt+'&todt='+todt+'&branch='+branch+'&exec='+exec;
			//alert(pmeters);
			}
			else
			{
...
<h2 class="h2color">Salesman Invoiced Report</h2>
...
<?php  } ?></select></th>
   	...
   	<!--Exec Name-->
    	...
    	<th>
        <select name="exec" id="exec" onchange="searchById('Listing','1','');">
...
<?php
    ...
    <option value="<?php echo $exe['exe_id']; ?>"><?php echo $exe['exe_name']; ?></option>
        <?php } ?>
    </select>  </th>   	
     ...
     <input type="text" name="fromdt" id="fromdt" readonly="readonly" onclick="displayDatePicker('fromdt');" placeholder="Inv From Date"/></th>
     
     	<!--To Date-->
   		<th width="75" colspan="2">
   		<input type="text" name="todt" id="todt" readonly="readonly" onclick="displayDatePicker('todt');" placeholder="Inv To Date"/></th>
