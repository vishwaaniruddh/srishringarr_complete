...
//=============================inserting code start here==========================================
		 ...
		 $add=mysql_query("select * from `purchase_order` where `po_no`='".$pordr."'");
		 if(mysql_num_rows($add)>0){}
		 else{
		// echo "<br>insert into`purchase_order` VALUES ('".$pordr."','".$nos."','".$cust."','".$ref."','".$service."',STR_TO_DATE('".$dateget."','%d/%m/%Y')) ";	 
		 $result12=mysql_query("insert into`purchase_order` VALUES ('".$pordr."','".$nos."','".$cust."','".$ref."','".$service."',STR_TO_DATE('".$dateget."','%d/%m/%Y')) ");
	
	if($upsno>0){	
	//echo "<br>insert into `po_assets` (`po_no`,`assets_name`,`specs`,`qty`,`warranty`,`rate`) values('".$pordr."','UPS','".$ups."','".$upsno."','".$upswr."','".$upsrate."')";			   
	 $addasset=mysql_query("insert into `po_assets` (`po_no`,`assets_name`,`specs`,`qty`,`warranty`,`rate`) values('".$pordr."','UPS','".$ups."','".$upsno."','".$upswr."','".$upsrate."')");		    
	 }
	...
	$addasset=mysql_query("insert into `po_assets` (`po_no`,`assets_name`,`specs`,`qty`,`warranty`,`rate`)values('".$pordr."','Battery','".$btry."','".$btryno."','".$btrywr."','".$batteryrate."')");  
		}
	...
	$addasset=mysql_query("insert into `po_assets` (`po_no`,`assets_name`,`specs`,`qty`,`warranty`,`rate`) values('".$pordr."','Isolation Transformer','".$isot."','".$isotno."','".$isotwr."','".$isotrate."')");
	 }
	 if($stabno>0)
	{
	...
	$addasset=mysql_query("insert into `po_assets` (`po_no`,`assets_name`,`specs`,`qty`,`warranty`,`rate`) values('".$pordr."','Stabilizer','".$stab."','".$stabno."','".$stabwr."','".$stabrate."')");
	 }
	 if($avrno>0)
	{
	...
	$addasset=mysql_query("insert into `po_assets` (`po_no`,`assets_name`,`specs`,`qty`,`warranty`,`rate`) values('".$pordr."','AVR','".$avr."','".$avrno."','".$avrwr."','".$avrrate."')");
	 }	
		 }
...
$result= mysql_query("INSERT INTO  atm(atm_id,bank_name,area,pincode,city,branch_id,address,ref_id,po,cust_id,servicetype,podate,state1,cat,pending_status)  VALUES('".$atm."','".$bank."','".$area."','".$pincode."','".$city."','".$branch."','".$addr."','".$ref."','".$pordr."','".$cust."','".$service."','".$dateget."','".$state."','".$cat."',1)");
 //====last inserted auto id from atm get here =====
 $id=mysql_insert_id();
 
 	$ponum=mysql_query("select `po` from `atm` where `po`='".$pordr."'");
	$ponum1=mysql_num_rows($ponum);
...
//===========For UPS============
	...
	$result1= mysql_query("insert into `site_assets`(cust_id,po,assets_name,assets_spec,valid,quantity,atmid,type,rate)values('$cust','$pordr','UPS','".$ups."','".$upswr."','".$upsno."','".$id."','NEW','".$upsrate."')");
	 }
//===========For Battery============	 
	 if($btryno>0)
	{
	...
	$result1= mysql_query("insert into `site_assets`(cust_id,po,assets_name,assets_spec,valid,quantity,atmid,type,rate)values('$cust','$pordr','Battery','".$btry."','".$btrywr."','".$btryno."','".$id."','NEW','".$batteryrate."')");
	 }
//===========For Isolation Transformer============	 
	...
	$result1= mysql_query("insert into `site_assets`(cust_id,po,assets_name,assets_spec,valid,quantity,atmid,type,rate)values('$cust','$pordr','Isolation Transformer','".$isot."','".$isotwr."','".$isotno."','".$id."','NEW','".$isotrate."')");
	 }
//===========For Isolation Stabilizer============	 
	...
	$result1= mysql_query("insert into `site_assets`(cust_id,po,assets_name,assets_spec,valid,quantity,atmid,type,rate)values('$cust','$pordr','Stabilizer','".$stab."','".$stabwr."','".$stabno."','".$id."','NEW','".$stabrate."')");
	 }
//===========For Isolation AVR============	 
	 if($avrno>0)
	{
	...
	$result1= mysql_query("insert into `site_assets`(cust_id,po,assets_name,assets_spec,valid,quantity,atmid,type,rate)values('$cust','$pordr','AVR','".$avr."','".$avrwr."','".$avrno."','".$id."' ,'NEW','".$avrrate."')");
	 }	
...
$result= mysql_query("INSERT INTO Amc(po,cid,atmid,bankname,area,pincode,city, branch,address, Ref_id, servicetype, state1) VALUES ('".$pordr."', '".$cust."', '".$atm."', '".$bank."', '".$area."', '".$pincode."', '".$city."', '".$branch."', '".$addr."', '".$ref."', '".$service."','".$state."')");
...
// echo $dt;
 ...
 //echo "<br>INSERT INTO `satyavan_accounts`.`amcpurchaseorder` (`id`, `cid`, `po`, `startdt`, `expdt`,`amcsiteid`) VALUES (NULL, '".$cust."', '".$pordr."', '".$start."','".date('Y-m-d', strtotime("+12 months $start"))."','".$id."')";
  ...
  $qry=mysql_query("INSERT INTO `amcpurchaseorder` (`id`, `cid`, `po`, `startdt`, `expdt`,`amcsiteid`) VALUES (NULL, '".$cust."', '".$pordr."', '".$start."','".date('Y-m-d', strtotime("+12 months $start"))."','".$id."')");
...
{	
	...
	$q=mysql_query("Insert into servicemonth(`po`,`date`,`type`,`siteid`) Values('".$pordr."','".date("Y-m-d",strtotime("+".$j." months", $today))."','AMC','".$id."')");
	if(!$q)
...
$insert=mysql_query("insert into `amcassets`(amcid,amcpoid,assets_name,assetspecid,quantity,siteid) values('$cust','$pordr','UPS','".$ups."','".$upsno."','".$id."')");
	}
...
$insert=mysql_query("insert into `amcassets`(amcid,amcpoid,assets_name,assetspecid,quantity,siteid) values('$cust','$pordr','Battery','".$btry."','".$btryno."','".$id."')");
	}
...
$insert=mysql_query("insert into `amcassets`(amcid,amcpoid,assets_name,assetspecid,quantity,siteid) values('$cust','$pordr','Isolation Transformer','".$isot."','".$isotno."','".$id."')");
	}
...
$insert=mysql_query("insert into `amcassets`(amcid,amcpoid,assets_name,assetspecid,quantity,siteid) values('$cust','$pordr','Stabilizer','".$stab."','".$stabno."','".$id."')");
	}
	
	 if($avrno>0)
	{
	//echo "<br>insert into `site_assets`(cust_id,po,assets_name,assets_spec,valid,quantity,atmid)values('$cust','$pordr','Stabilizer','".$stab."','".$stabwr."','".$stabno."','".$atm."')";	
	$insert=mysql_query("insert into `amcassets`(amcid,amcpoid,assets_name,assetspecid,quantity,siteid) values('$cust','$pordr','AVR','".$avr."','".$avrno."','".$id."')");
	 }
		if($insert){
