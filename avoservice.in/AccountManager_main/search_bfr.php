...
$alrtqr=mysql_query("select atm_id,assetstatus from alert where createdby='".$row[1]."'");
$alertr=mysql_fetch_row($alrtqr);


if($alertr[1] == 'site')
$atmry=mysql_query("select atm_id from atm where track_id='".$alertr[0]."'");
elseif($alertr[1] == 'amc')
$atmry=mysql_query("select atmid from Amc where amcid='".$alertr[0]."'");
$atmidrow=mysql_fetch_row($atmry);
$atmid=$atmidrow[0];
if($atmid==''){$atmid=$alertr[0];}
?>
<tr>
<td width="50" valign="top">&nbsp;<?php echo $row[4]; ?></td> <!--Cust -->
<td width="50" valign="top">&nbsp;<?php echo $atmid; ?></td> <!--ATM Id -->
...
<?
  $result3=mysql_query("select * from UpdateStatus where brf_id='$row[0]' order by currentdate desc limit 1 ");
$fetchBrf_id=mysql_fetch_array($result3);
