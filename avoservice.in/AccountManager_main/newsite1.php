...
$oldpo=mysql_query("select `po` from `atm` where 1 ");
$oldpo1=mysql_fetch_row($oldpo);

?>
<h2 class="h2color">Add New Site</h2>

<form action="process_newsiteme" method="post" enctype="multipart/form-data"  name="form" id="form" >
<table width="100%">
<tr>
...
<td width="200"><input type="text" name="po" id="po" onblur="getpo();" /></td>
	...
	<input type="radio" name="type" id="type" value="sales" checked="checked" class="type" />Sales Site  	
  	<input type="radio" name="type" id="type2" value="AMC" class="type"/>AMC Site
 </td>
</tr>
...
<tr>
<td width="100" height="35"><b>Ref ID:</b></td>
...
<?php 
//$addpo=mysql_query("select `po` from `atm` where `po`=");
//$addpo1=mysql_num_rows($addpo);
...
<?php
$qry1=mysql_query("Select * from assets_specification where assets_id='1'");
...
<option value="0">select</option>
<?php
$qry1=mysql_query("Select * from assets_specification where assets_id='2'");
while($row=mysql_fetch_row($qry1))
...
<td width="200"><input type="text" name="atm" id="atm" onblur="getallatm(this.value);" /></td>
...
<?php
$state=mysql_query("select * from state order by state ASC");
while($stro=mysql_fetch_array($state))
{
?>
...
<td colspan="4" align="center" height="50"><div id="search"><button type="button" name="mysub" id="mysub" onclick="validate(this.form); " class="readbutton" value=""> Submit </button></div> </td>
</tr>
</table>
</form>
</center>
