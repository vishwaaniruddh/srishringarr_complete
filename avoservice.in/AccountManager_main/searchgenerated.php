...
//echo "Select * from site_assets where atmid = '".$row[0]."' ";
	$qry2me=mysql_query("Select * from site_assets where atmid = '".$row[0]."' ");
...
$atmqry=mysql_query("select so_id from atm where track_id='".$row[2]."'");
$atm=mysql_fetch_row($atmqry);

//echo "select inv_no from so_order where po_id='".$atm[0]."'";
$invqry=mysql_query("select inv_no from so_order where po_id='".$atm[0]."'");
$atm=mysql_fetch_row($invqry);
...
$atmid=mysql_query("select `atm_id` from atm where `track_id`='".$row[2]."'");
if(mysql_num_rows($atmid)==0)
$atmid=mysql_query("select `atmid` from Amc where `amcid`='".$row[2]."'");
$atmid1=mysql_fetch_row($atmid);
echo $atmid1[0];
if(mysql_num_rows($atmid)==0) { echo $row[2] ; }
 ?></td>
<!---customer-->
<td width="95"><?php 
$custname=mysql_query("select `cust_name` from `customer` where `cust_id`='".$row[1]."'");
$custname1=mysql_fetch_row($custname);
echo $custname1[0];
 ?></td>
<!---bank-->
...
<td width="70">
<?php 
$branch=mysql_query("select * from avo_branch where id='".$row[7]."'");
...
<?php
/*$asst=mysql_query("Select assets from alert_assets where alert_id='".$row[0]."'");
while($resasst=mysql_fetch_row($asst))
{
echo $resasst[0]."\n";
}*/
 ?></td>-->
...
$al=mysql_query("select feedback from eng_feedback where alert_id='".$row[0]."' order by id DESC limit 1");
