...
Class CheckLogin
{
	...
	$this->logchk();
	 ...
	 public function logchk()
	 {
		 ...
		 $result = mysqli_query($con1,"select * from login where username='$this->user' and password='$this->pwd' and status=1 ");
if(mysqli_num_rows($result)>0)
   ...
   if($row[3]=='' || $row[3]=='0' || $row[3]=='null')
   $_SESSION['branch']='all';
   else
   $_SESSION['branch']=$row[3];
   
   if($row[4]=='' || $row[4]=='0' || $row[4]=='null')
