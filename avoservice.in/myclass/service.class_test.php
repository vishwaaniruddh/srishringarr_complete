...
Class Service
{
	...
	public function __construct()
	{
		...
		$this->pcb=array();
		$this->assid=array();
		$this->sub=mysqli_real_escape_string($con1, $_POST['sub']); 
...
//echo			$i."- ".$_POST['assets'][$i];
	...
	public function Process($con1)
	{
		if($this->Isvalid() && $this->Execute($con1))
		{
		}
	...
	public function Isvalid()
	{
		...
		if(empty($this->ref))
		$this->errors[]="Site Id Cannot be left Blank";
		if(empty($this->atm))
		$this->errors[]="Site Cannot be left Blank";
		for($i=0;$i<$this->cnt;$i++)
		{
		...
		$this->errors[]="Please select Branch";
    ...
    // include ("config.php");
    	...
    	$dt=date("Y-m-d H:i:s");
//echo "select srno from login where username='".$_SESSION['user']."'"; die;
...
echo "INSERT INTO `alert` (`alert_id`, `cust_id`,`atm_id`, `bank_name`, `area`, `address`, `city`, `branch_id`, `pincode`, `problem`, `entry_date`, `alert_date`, `caller_name`, `caller_phone`, `caller_email`, `status`, `call_status`, `alert_type`,  `po`, `assetstatus`, `appby`, `appref`,`state1`,`createdby`,`subject`,`custdoctno`,`ccmail`) VALUES (NULL, '".$this->cust."','".$this->atm."' , '".$this->bank."', '".$this->area."', '".$this->add."', '".$this->city."', '".$this->branch."', '".$this->pin."', '".$this->prblm."', '".$dt."', '".$aldate."', '".$this->caller."', '".$this->con."', '".$this->email."', 'Pending', 'Pending', '".$this->type_call."',   '".$this->po."','".$this->type."', '".$this->appby."', '".$this->how."','".$this->state."','".$qry2ro[0]."_".date("ymd").$num3."','".$this->sub."','".$this->doc."','".$this->ccm."')";

	$sql = mysqli_query($con1,"INSERT INTO `alert` (`alert_id`, `cust_id`,`atm_id`, `bank_name`, `area`, `address`, `city`, `branch_id`, `pincode`, `problem`, `entry_date`, `alert_date`, `caller_name`, `caller_phone`, `caller_email`, `status`, `call_status`, `alert_type`,  `po`, `assetstatus`, `appby`, `appref`,`state1`,`createdby`,`subject`,`custdoctno`,`ccmail`) VALUES (NULL, '".$this->cust."','".$this->atm."' , '".$this->bank."', '".$this->area."', '".$this->add."', '".$this->city."', '".$this->branch."', '".$this->pin."', '".$this->prblm."', '".$dt."', '".$aldate."', '".$this->caller."', '".$this->con."', '".$this->email."', 'Pending', 'Pending', '".$this->type_call."',   '".$this->po."','".$this->type."', '".$this->appby."', '".$this->how."','".$this->state."','".$qry2ro[0]."_".date("ymd").$num3."','".$this->sub."','".$this->doc."','".$this->ccm."')");
...
//	echo "".mysqli_error($con1);
	...
	for($i=0;$i<count($this->assid);$i++)
		...
		{
		$escem=array();
		$escpple=mysqli_query($con1,"select email,level from esclatingpeople where level='0' and state like '%".$this->state."%' and status=0 order by id ASC");
while($pplro=mysqli_fetch_row($escpple))
$escem[]=$pplro[0];

$esclation=implode(",",$escem);
		$esc=mysqli_query($con1,"INSERT INTO `escalation` (`alertid`, `startdt`, `endtime`, `escalateto`, `type`, `level`, `status`) VALUES ('".$row."', '".$dt."', '".date( "Y-m-d H:i:s", strtotime( "$dt +15 minute" ) )."', '".$esclation."', '0', '1', '0')");
//================		
if($this->type=='site')
	$at=mysqli_query($con1,"select atm_id from atm where track_id='".$this->atm."'");
	elseif($this->type=='amc')
	$at=mysqli_query($con1,"select atmid from Amc where amcid='".$this->atm."'");
...
$headers = "From: <HelpDesk@avoservice.in>\r\n";
  ...
  $qrymail= mysqli_query($con1,"select branch_email from avo_branchmgr_email where branch_id='".$this->branch."'");
  ...
  window.location="service1.php";
   ...
   <?php 
		 ...
		 ?>
         	<script type="text/javascript">
			alert("Sorry ! Service Alert unsuccessfully Please Try Again.);
  			window.location="service1.php";
			</script>
		 <?php
		}
