...
return _validate[e.name](e);return true;}
...
return true;}
e.onkeydown=function(event){event=event||window.event;key=event.keyCode||event.which;var box=document.getElementsByName(this.getAttribute('rel'))[0];if(box&&key==32&&this.className=='combo2'){this.style.display='none';box.style.display='inline';box.value='';setFocus(box);return false;}else{if(key==13&&!e.length)
return false;}
}
}
...
"edit","scrollbars=yes,resizable=0,width="+editors[key][2]+",height="+editors[key][3]);if(_w.opener==null)
...
'button':function(e){e.onclick=function(){return validate(e);}
},
...
'button[aspect=popup]':function(e){e.onclick=function(){if(_w)_w.close();_w=open(document.location+'popup=1',
"edit","Scrollbars=0,resizable=0,width=800,height=600, top=50,left=50");if(_w.opener==null)
_w.opener=self;_w.focus();return false;}
},
'select':function(e){if(e.onfocus==undefined){e.onfocus=function(){save_focus(this);};var c=e.className;if(c=='combo'||c=='combo2'||c=='combo3')
_set_combo_select(e);}
},
...
if(((asp&&asp.indexOf('default')!==-1)&&key==13)||((asp&&asp.indexOf('cancel')!==-1)&&key==27)){if(validate(el)){if(asp.indexOf('process')!==-1)
