...
//===========
$alertng=mysqli_query($con1,"select assetstatus,atm_id from alert where alert_id='".$req."'");

$alertro=mysqli_fetch_row($alertng);

if($alertro[0] !='' or $alertro[0] !=NULL){
if($alertro[0] ==  'amc')
   { $atm=mysqli_query($con1,"select atmid, latitude, longitude from Amc where amcid='".$alertro[1]."'");
   }
if($alertro[0] == 'site'){
$atm=mysqli_query($con1,"select atm_id, latitude, longitude from atm where track_id='".$alertro[1]."'");
...
$oldeng=mysqli_query($con1,"select engineer from alert_delegation where alert_id='".$req."'");
$getold=mysqli_fetch_row($oldeng);

//echo $getold[0]."Hiiiiiiiiiiii";
...
if($_GET['br']=='all')
	...
	$sql="select engg_id,engg_name,area,city,engg_desgn, latitude, longitude from area_engg where status=1 order by engg_name ASC";
		...
		$sql="select engg_id,engg_name,area,city,engg_desgn, latitude, longitude from area_engg where current_area in(".$br.") and status=1";
...
$q2=mysqli_query($con1,"select city,state_id from cities where city_id='".$row[3]."'");
$r2=mysqli_fetch_row($q2);

$q=mysqli_query($con1,"select state from state where branch_id='".$row[2]."' and state_id='".$r2[1]."'");
$r=mysqli_fetch_row($q);

$eng=$row[0].",".$dis;

?>
<option value="<?php echo $eng; ?>" <?php if($row[0]==$getold[0]){ echo "selected"; } ?>><?php echo $row[1]."/ Distance: ".$dis." (".$row[4]."-".$r2[0].")"; ?></option>
