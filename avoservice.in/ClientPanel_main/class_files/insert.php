...
class insert{
	...
	function insert_into($server_name,$db_username,$db_password,$db_name,$table_name,$arr_columns,$arr_values){
		...
		if($string_values){
		if($string_columns=='*'){				
			$bool=mysql_query("insert into $table_name() values ($string_values)");
			$con_obj->close_connection();
			if($bool)return true;
			else return false;
		}
		else{
			$bool=mysql_query("insert into $table_name($string_columns) values ($string_values)");
			$con_obj->close_connection();
			if($bool)return true;
			else return false;
		}
