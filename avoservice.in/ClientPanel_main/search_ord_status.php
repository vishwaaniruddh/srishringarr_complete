...
$atmqry= mysql_query("Select bank_name, address, so_date from demo_atm where so_id='".$row[0]."'");
$atm= mysql_fetch_row($atmqry);

//=========SO Order ======
$soqry=mysql_query("Select * from so_order where po_id='".$row[0]."'");
$so= mysql_fetch_row($soqry);

//=====Call Log & status==== 

$alertqry=mysql_query("Select entry_date, createdby, close_date from alert where alert_id='".$so[24]."'");

$alert= mysql_fetch_row($alertqry);
...
<!--===Customer Name===-->
<td  valign="top">&nbsp;<?php 
$cuname=mysql_query("select `cust_name` from `customer` where `cust_id`='".$row[3]."'");
$cuname1=mysql_fetch_row($cuname);
echo $cuname1[0]; ?></td>
...
<!--===PO===-->
<td  valign="top">&nbsp;
<?php $poname=mysql_query("select `po_no` from `purchase_order` where `id`='".$row[1]."'");
$po_no=mysql_fetch_row($poname);
echo $po_no[0]; ?></td>
...
<td  valign="top">
<?php
$branch_qry=mysql_query("select * from avo_branch where id='".$row[4]."'");
$branch_row=mysql_fetch_array($branch_qry);
...
<? 
if ($row[14]!='2'  || $so[19]!='2' ){
$soupdate= mysql_query("Select Remarks_Update from SO_Update where so_id= '".$row[0]."' Order by updt_id DESC LIMIT 1");

//echo "Select Remarks_update from SO_Update where so_id= '".$row[0]."' Order by updt_id DESC LIMIT 1";

$update=mysql_fetch_row($soupdate);
echo $update[0];  }

else if ($so[19]=='2'){
$call= mysql_query("Select feedback from eng_feedback where alert_id= '".$so[24]."' order by id DESC LIMIT 1");
$update1=mysql_fetch_row($call);
echo $update1[0];  }
