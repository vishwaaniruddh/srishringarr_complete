...
<?php
       ...
       {
               $dt=date("Y-m-d H:i:s");
     $qry2=mysql_query("select cust_name from customer where cust_id='".$POST['cust']."'");
...
$res=mysql_query("SELECT * FROM amcassets where siteid='".$_POST['ref']."'");
...
$qry=mysql_query("select * from amcpurchaseorder where amcsiteid='".$_POST['ref']."'");
...
$qrystate=mysql_query("select * from avo_branch");
while($qrystate1=mysql_fetch_row($qrystate)){
?>
...
$cl=mysql_query("select client from clienthandle where logid in (select srno from login where username='".$_SESSION['user']."')");
$clnt=array();
while($clntr=mysql_fetch_array($cl))
$clnt[]=$clntr[0];
$client=implode(",",$clnt);
$client=str_replace(",","','",$client);
include ("config.php");
$whatcc=mysql_query("select a.id,a.groupname, b.cust_name from whatsapp_groupname a,customer b where a.cust_id=b.cust_id and a.status=1 and b.cust_name in ('$client') and a.type='service' order by b.cust_name,a.groupname ASC");
...
<?php
while($whatsrow=mysql_fetch_row($whatcc))
{
?>
...
$ccc=mysql_query("select e.email,c.cust_name,e.bank from emailid e,customer c where e.custid=c.cust_id and e.status=0 and c.cust_name in ('$client') order by c.cust_name,e.bank ASC");
if(!$ccc)
echo mysql_error();
?>
...
<?php
while($ccro=mysql_fetch_array($ccc))
{
?>
...
if($ref!='' || $ref!=NULL)
{
$re=mysql_query("select * from Amc where Ref_id='".$ref."' limit 1");
$refid=mysql_fetch_row($re);
}
$qry2=mysql_query("select srno from login where username='".$user."'");
$qry2ro=mysql_fetch_row($qry2);
...
//echo "select e.email,c.cust_name,e.bank from emailid e,customer c where e.custid=c.cust_id and e.status=0 and c.cust_name in ('$client') order by c.cust_name,e.bank ASC";
$ccc=mysql_query("select e.email,c.cust_name,e.bank from emailid e,customer c where e.custid=c.cust_id and e.status=0 and c.cust_name in ('$client') order by c.cust_name,e.bank ASC");
//echo mysql_num_rows($ccc);
/*while($ccro=mysql_fetch_row($ccc))
{
