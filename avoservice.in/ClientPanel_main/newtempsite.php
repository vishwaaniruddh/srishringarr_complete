...
<?php 
        ...
        while($branch1=mysql_fetch_array($selbr2))
        {
        ?>
...
<?php
	...
	<?php
	$stateqry=mysql_query($stqry);
	while($sttro=mysql_fetch_array($stateqry))
	{
	?>
...
$cc=mysql_query("select e.email,c.cust_name,e.bank from emailid e,customer c where e.custid=c.cust_id and e.status=0 and c.cust_name in ('$client') order by c.cust_name,e.bank ASC");
?>
