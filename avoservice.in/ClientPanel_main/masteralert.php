...
$alert=mysql_query("select a.createdby,a.cust_id,a.atm_id,a.bank_name,a.address,a.state1,a.problem,a.entry_date,a.alert_type,a.assetstatus,c.cust_name from alert a,customer c where a.alert_id='".$id."' and a.cust_id=c.cust_id");
$alertro=mysql_fetch_row($alert);
if($alertro[8]=='service' &&  $alertro[9] ==  'amc')
    $atm=mysql_query("select atmid from Amc where amcid='".$alertro[2]."'");
	if($alertro[8]=='service' &&  $alertro[9] == 'site')
	$atm=mysql_query("select atm_id from atm where track_id='".$alertro[2]."'");
	
	$atmro=mysql_fetch_row($atm);
?>
...
<tr><th valign="top">Address</th><td valign="top"><?php echo nl2br($alertro[4]); ?></td><th valign="top">Problem</td><td valign="top"><?php echo nl2br($alertro[6]); ?></td></tr>
  ...
  <th>Call Delegated to</th>
    ...
    $del=mysql_query("select engg_name from area_engg where engg_id=(select engineer from alert_delegation where alert_id='".$id."')");
    $delro=mysql_fetch_row($del);
    $eng=$delro[0];
    $del2=mysql_query("select engg_name from area_engg where engg_id=(select eng_new from alert_redelegation where alert_id='".$id."')");
    if(mysql_num_rows($del2)>0){
    $delro2=mysql_fetch_row($del2);
    $eng=$delro[0];
    }
    echo $eng;
    ?>
