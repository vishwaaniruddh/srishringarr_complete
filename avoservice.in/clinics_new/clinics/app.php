...
<div id="" class="login-popup">
         ...
         <form method="post" class="signin" action="processapp.php" onSubmit="return appvalidate(this)" name="appform">
            	...
            	<label class="name">
                <span> Name: </span>
                <input id="name" name="name" type="text" autocomplete="on"  value="<?php if(isset($row[6])) {echo $row[6];} ?>" readonly style="background-color:#DCDCDC;">
                </label>
                                            
                <label class="cn">
                <span>Contact No.:</span>
                <input id="cn" name="cn" type="text" value="<?php if(isset($row[23])) {echo $row[23];} ?>" readonly style="background-color:#DCDCDC;">
                </label>
                 
                <label class="age">
                <span>Age:</span>
                <input id="age" name="age" type="text" value="<?php if(isset($row[26])) {echo $row[26];} ?>" readonly style="background-color:#DCDCDC;">
                </label>
...
?>
                ...
                <label class="Date">
                <span>Referal Date:</span>
                <input id="appdate" name="appdate" type="text" readonly>
                ...
                <label class="type">
                <span>Type:</span>
                Consultation: <input type="radio"  name="type" style=" width:40px;" value="Consultation" checked="checked" /> 
                Follow Up: <input type="radio"  name="type" style=" width:40px;" value="Follow"/>
                </label>
                <label class="name">
                <span> Charges: </span>
                <input id="charges" name="charges" type="text" autocomplete="on" >
                </label>
...
//$result = mysqli_query($con,"SELECT date,time FROM appoint WHERE date >= CURRENT_DATE order by date;");
//$row = mysqli_fetch_row($result);
?>
...
<?php } */ ?>
                ...
                <a href="view_patient.php"> <button class="submit formbutton" type="button" onClick="javascript:location.href = 'view_patient.php';">Go Back</button></a>    
                </fieldset>
                </form>
</div>
<?php 
