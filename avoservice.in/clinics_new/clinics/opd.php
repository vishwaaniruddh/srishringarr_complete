...
<div id="" class="login-popup">
         ...
         <form method="post" class="signin" action="new_opd.php" name="opdform" onSubmit="return opdvalidate(this)" enctype="multipart/form-data">
                ...
                <td>
                <?php $rs=mysqli_query($con,"select * from opd where patient_id='$id'");
				$s=mysqli_num_rows($rs);
				if($s==0){
				?>
                <input style="background:none; border:none; width:20px; height:20px;" name="ode1[]" id="code1[]" type="checkbox" onclick="window.location.href='pre_inves.php?id=<?php echo $id ?>'" />Pre- Investigation
                <?php } ?>
                </td>
                </tr>
                <tr>
...
?>               
                <table border="1">
                <th>Medicine Name test </th> <th>How to Take </th><th>Dosage </th> 
                <?php for($j=0;$j<=2;$j++){?>
                <tr>
                <td>
            ...
            ?>                  
                ...
                <option value="0">Select</option>
                 <?php 
				    while($row_take=mysqli_fetch_array($result4)){ ?>
					<option value="<?php echo $row_take[2]; echo "selected";?>"><?php echo $row_take[2]; ?></option>
				 <?php } ?>
                </select>
                </td>
...
<tr>
<td>
<button class="submit formbutton" type="submit" name="Submit">Submit</button> &nbsp;&nbsp;&nbsp;<button class="submit formbutton" type="button" onClick="javascript:location.href = 'view_patient.php';">Cancel</button>
                   &nbsp;&nbsp;&nbsp;
 <button class="submit formbutton" type="button" onClick="javascript:popcontact('opt_surgery.php?id=<?php echo $id; ?>');">Surgery Waiting List</button>
 &nbsp;&nbsp;&nbsp;
 <button class="submit formbutton" type="button" onClick="javascript:popcontact('app_surgery.php?id=<?php echo $id; ?>');">Surgery Appointment</button> 
</td></tr>
                <tr>
                <td colspan="3">
                ...
                <option value="0">Select</option>
                <?php  $result3 = mysqli_query("select * from medicine ",$con);
