...
<div id="" class="login-popup">
          ...
          <form method="post" class="signin" action="update_ad.php" onSubmit="return advalidate(this)" name="adform">
                ...
                <td><input id="name" name="name" type="text" autocomplete="on"  value="<?php echo $row1[6]; ?>" readonly style="background-color:#DCDCDC;"></td>
                
<?php 
include('config1.php');
$result3 = mysql_query("select doc_id,name from doctor ");
?>
...
?>                
                ...
                <?php while ($row3=mysql_fetch_row($result3)) { ?>
				<option value="<?php echo $row3[0]; ?>" <?php if($row3[0]==$row[8]){ echo "selected"; }?>><?php echo $row3[0]; ?></option>
				<?php } ?>
                </select>
                </div></td>
                </tr>
