...
<th width="47">Transfer</th>
<th width="56">Delete</th>
<!--<th width="56">Deactivate Android App</th>
<th width="80">Exchange phone</th>-->

<?php } ?>
<th width="47">Edit-local</th>
...
if($_SESSION['branch']=='all' || $_SESSION['branch']=='0' || $_SESSION['branch']=='0')
{

$str= "select * from area_engg where status=1 and deleted=0";
}
else if($_SESSION['designation']=='4' ) {
    
$logqry=mysqli_query($con1,"select srno from login where username='".$eng_log."'");
$logid=mysqli_fetch_row($logqry);

$str.="select * from area_engg where status=1 and deleted=0 and loginid='".$logid[0]."' ";

} else 

$str.="select * from area_engg where status=1 and deleted=0 and area in (".$_SESSION['branch'].")";
...
$qry2=mysqli_query($con1,"select city, category from cities where city_id='".$row[3]."'");
$row2=mysqli_fetch_row($qry2);
$qry3=mysqli_query($con1,"select id, name from avo_branch where id='".$row[2]."'");
$row3=mysqli_fetch_row($qry3);

$qry34=mysqli_query($con1,"select * from avo_branch where id='".$row[15]."'");
...
$useqry=mysqli_query($con1,"Select username, password from login where srno='".$row[8]."'");
...
<td><?php if($row[7]!=''){  ?><a href="download.php?filename=<?php echo $row[7]; ?>"><?php echo $row[1]." Resume"; ?></a><?php  } ?></td>
<td><?php // echo $row[9];   ?>
...
<?php  } ?>
<td><a href="exchangeandi.php?engid=<?php echo $row[0];  ?>&logid=<?php echo $row[8]; ?>&desig=4" class="update">Exchange Mobile </a></td> -->
