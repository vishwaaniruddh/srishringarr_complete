...
if(isset($row['deposit']) && $row['deposit'] > 0 ){
    $final_deposit = $row['deposit'];
}


if(isset($row['sales_price']) && $row['sales_price'] > 0){
    $newselling_price = $row['sales_price'];
}
                        $data[] = ['pid'=>$pid,'category'=>$category,'link'=>$url,'image'=>$path,'product_name'=>$name,'deposite'=>$final_deposit, 
                        'selling_price'=>$newselling_price,'sku'=>$prcode,'rent_price'=>$rentprice,'imageframe'=>$imgframe,'jew'=>$jewsql,'gar'=>$garsql];
                }
            }
        }
