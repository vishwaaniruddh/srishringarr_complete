                 ...
                 if($type=="1"){
                            $sqlimg="SELECT img_name FROM `product_images_new` WHERE `product_id`='".$row[0]."'";
                              if($newsp<=40000){
                               ...
                               if(isset($necklace) && $necklace==1){    // necklace cat should have mimnim um 1500 rent and not less than that
                                        if($rentprice < 1500){
                                            $rentprice = 1500;
                                            $deposit = 3000; 
                                    }
...
$rentprice = round($rentprice,2);

    // $data[] = ['1'=>$source_img,'2'=>$destination_img,'snno'=>$i,'cur_sym'=>$cur_sym,'product_name'=>$product_name,'selling_price'=>$selling_price,'rent_price'=>$rentprice,'image'=>$path,'sku'=>$prcode,'deposite'=>$final_deposit,'discount'=>$discount,'link'=>$link,'statement'=>'1', 'booking'=>$booking_date,'imageframe'=>$imgframe,'newsp'=>$newsp];                        

    $data[] = ['snno'=>$i,'cur_sym'=>$cur_sym,'product_name'=>$product_name,'selling_price'=>$selling_price,'rent_price'=>$rentprice,'image'=>$path,'sku'=>$prcode,'deposite'=>$final_deposit,'discount'=>$discount,'link'=>$link,'statement'=>'1', 'booking'=>$booking_date,'newsp'=>$newsp];                        
