...
while($row = mysqli_fetch_array($raw_data)){
        ...
        mysqli_query($con,"update product set isReal=1 where product_id='".$product_id."'");
...
if(isset($row['sales_price']) && $row['sales_price'] > 0){
    $lastSellingPrice = $row['sales_price'];
        ...
        'discount'=>$discount,'link'=>$link,'courier'=>$courier,'sellPrice'=>$sellPrice,'lastSellingPrice'=>$lastSellingPrice,'commissionAmount'=>$commissionAmount,
        'booking'=>$booking_date,
        '$sqlimg'=>$sqlimg,'$sql_query'=>$sql_query
        ];
