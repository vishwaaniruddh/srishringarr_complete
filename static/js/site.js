...
function addToCart(product_id, isRent, rentAmount) {
	...
	var days = $('input[class="subject-list"]').val();
	//alert(days)
	...
	//alert(depositeAmount);
	//<!-- vikrant 130920 -->
	var bust = $('option:selected[name="bustsize"]').val();
	var waist = $('option:selected[name="waistsize"]').val();
	var hip = $('option:selected[name="hipsize"]').val();
	//alert(bust);
	//alert(waist);
	//alert(hip);
	//<!-- vikrant 130920 -->
	...
	function(response) {
		if (response.success == "True") {
			...
			$("#cartCountMob").text(response.cart_count);
