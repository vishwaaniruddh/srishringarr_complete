...
$sql = mysqli_query($con,"select * from recommended_suggestion where product_type like '".$type."'");
while($sql_result = mysqli_fetch_assoc($sql)){
    $product_sku[] = $sql_result['product_sku'];
    $prod_id[] = $sql_result['prod_id'];
}
echo $prod_id=json_decode($prod_id);
echo "<br>";
echo $product_code=json_decode($product_sku);



// if($type==1){
//     $sql= mysqli_query($con,"select * from products where product_code='".$product_code."'");
