...
if($razor_status=='captured'){
    ...
    try{
        ...
        $sql  = "update Order_ent set amount ='".$amount."', razorpay_payment_id = '".$razorpay_payment_id."',date = '".$datetime."',delivery_add='".$delivery_add."',
        pickup_add = '".$pickup_add."' , acc_type ='1',coupon_code='".$coupon."',razor_status = '".$razor_status."',razorpay_order_id ='".$orderId."' where id = '".$ss_shopping_order_id."'" ;
...
$acc_type = 1;
        ...
        if(mysqli_query($con3,$insert_people)){
            ...
            values('".$userid."','".$date."','".$total_amount."','".$total_amount."','A','Customer','Customer Return','Paid','".$rental_date[0]."','".$rental_date[1]."','Unpaid','Booked','".$ss_shopping_order_id."',1)";
        ...
        $crtids=get_cart_ids($userid);
                ...
                $approval_sql = "insert into approval(cust_id,bill_date,status,paid_amount,transaction_id,bill_by,typ,card_amt) 
                values('".$userid."','".$date."','S','".$total_amount."','".$txnid."','online','1','".$total_amount."')"; 
        ...
        foreach($crtids as $key => $val){
            ...
            $approval_detail = "insert into approval_detail(bill_id,item_id,qty,amount,item_per,final_amount)
            values('".$approval_id."','".$sku."','".$qty."','".$total_amount_."','Rs','".$total_amount_."')";
        ...
        $state_id = state_id_userid($userid);
         ...
         foreach($crtids as $key => $val){
            ...
            if($phppos_rent_id > 0){
                    ...
                    mysqli_query($con3,"insert into order_detail(bill_id,item_id,rent,deposit,qty,total_amount,pickup_date,return_date,commission_amt) 
                    values('".$phppos_rent_id."','".$sku."','".$product_amt."','".$rental_date[2]."','".$qty."','".$product_total."','".$rental_date[0]."','".$rental_date[1]."','".$product_amt."')");
...
<td colspan="5" valign="middle" id="m_3716871841364271743credit" style="border-radius:6px;border:0;color:#8a8a8a;font-family:&quot;Helvetica Neue&quot;,Helvetica,Roboto,Arial,sans-serif;font-size:12px;line-height:150%;text-align:center;padding:24px 0">
    ...
    // }
        ...
        $subject = 'Thanks You For Shopping At Srishringarr';
