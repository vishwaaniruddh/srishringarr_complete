...
$sug_sql = mysqli_query($con,"select * from suggestion where product_id='".$id."'");
while($sug_result = mysqli_fetch_assoc($sug_sql)){
    $sug[] = $sug_result['sug_sku'];    
}

$sug = array_unique($sug);
$sug = implode(', ',$sug);
$sug = explode(",", $sug);
...
if($searchText){
                ...
                echo $garsql = "SELECT g.*,gp.* FROM `garments` g left join  garment_product gp on g.garment_id = gp.product_for WHERE g.name like '%".$searchText."%' or gp.gproduct_code in($skid)";
                $Apparel=mysqli_query($conn,$garsql);


                $garment_row_count = mysqli_num_rows($Apparel);
                 echo $jewsql = "SELECT j.categories_name,j.subcat_id as m_category,js.name,js.subcat_id as sub_cat,p.* from jewel_subcat j join subcat1 js on j.subcat_id=js.maincat_id join product p on js.subcat_id = p.subcat_id where j.categories_name like '%".$searchText."%' or js.name like '%".$searchText."%' or p.product_code in ($skid) or p.product_name in($skid)";
