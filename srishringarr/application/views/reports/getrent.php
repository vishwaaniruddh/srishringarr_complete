 ...
 //$qryapp=mysql_query("select `cust_id`  from `approval` where `bill_date` between '".$frmdate."' and '".$todate."'  group by `cust_id` DESC");
  ...
  $qryapp = mysql_query($stry); ?>
     ...
     <td><?php echo $app[0]."   ". $app[1]; ?> </td><td><?php echo  $app[1]; ?> </td><td ><?php echo $app[2]?></td><td align="right"><?php
     	...
     	if($_GET["invno"]=="")
	{
     $qrybal=mysql_query("SELECT sum(amount) FROM `phppos_rent` where `cust_id`='$app[11]'  and ( `bill_date` between '".$frmdate."' and '".$todate."')");
	}else
	{
	   $qrybal=mysql_query("SELECT sum(amount) FROM `phppos_rent` where `cust_id`='$app[11]'  and bill_id='".$_GET["invno"]."'"); 
