...
{	
	...
	$qryyr=mysql_query("select * from phppos_people where person_id in (select distinct(`cust_id`) from `phppos_rent` where `bill_date` between '".$frmdate."' and '".$todate."' and (booking_status='Picked' or booking_status='Returned')) order by last_name");
	}
else
{
//echo "select `bill_id` from `order_detail` where `item_id` ='$item_id'";
$qryyr=mysql_query("SELECT `bill_id` FROM `phppos_rent` WHERE `bill_id` in(select `bill_id` from `order_detail` where `item_id` ='$item_id') ");
}
$arr=array();
$i=0;
