<?php 
	...
	$qryapp=mysql_query("select `bill_id` from `approval` where cust_id='$cust_id' and `bill_id` in (select `bill_id` from `return_qty` where `return_date` between STR_TO_DATE('".$frmdate."','%d/%m/%Y') and STR_TO_DATE('".$todate."','%d/%m/%Y') ) and status<>'S' order by bill_id DESC");
	...
	while($resapp=mysql_fetch_row($qryapp)){
		//echo "hii";
			$qryrtn=mysql_query("select `bill_id`,`item_code`,sum(`qty`),`return_date` from `return_qty` where `bill_id`='$resapp[0]' and (`return_date` between STR_TO_DATE('".$frmdate."','%d/%m/%Y') and STR_TO_DATE('".$todate."','%d/%m/%Y') ) group by `bill_id`,`item_code`,`return_date` order by return_date DESC");
			while($resrtn=mysql_fetch_row($qryrtn))
			{					//echo "SELECT `qty`,`amount` FROM `approval_detail` WHERE `bill_id`='$resapp[0]' and `item_id`='$qryrtn[1]'";
								$qryprice=mysql_fetch_row(mysql_query("SELECT `qty`,`amount` FROM `approval_detail` WHERE `bill_id`='$resapp[0]' and `item_id`='$resrtn[1]' and amount<>'0'"));
								$amt=round($qryprice[1]/$qryprice[0]);

$getcategoryr=mysql_fetch_row(mysql_query("SELECT category from phppos_items where name='".$resrtn[1]."'"));
