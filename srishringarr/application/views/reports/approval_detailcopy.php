...
mysql_query("BEGIN;");
	...
	$t1=mysql_query("insert into `approval`(cust_id,bill_date,status,paid_amount,discount_status,discount_wise,discount_per,bill_by,pay_by,note,typ,statetyp,card_perc,card_amt) values('$cid',STR_TO_DATE('".$odate."','%d/%m/%Y'),'$bs','$amt','$dis_st','$wise','$disper','$by','$pay_by','$note',1,'".$_POST['statetyp']."','".$cardperc."','".$_POST['cardpercamt']."')");
...
$t4=mysql_query("insert into approval_detail(bill_id,item_id,qty,discount,dis_amount,amount,item_per,`gstpec`, `sgstamt`, `cgstamt`, `igstamt`, `typ`,sgstperc,cgstperc,igstperc,price,taxable_price,final_amount) values('$rowordno[0]','$design[$i]','$prz[$i]','$ds','$p','".$sum."','$itemper','".$_POST['gstrate'][$i]."','".$_POST['sgstamt'][$i]."','".$_POST['cgstamt'][$i]."','".$_POST['igstamt'][$i]."',1,'".$_POST['sgstrate'][$i]."','".$_POST['cgstrate'][$i]."','".$_POST['igstrate'][$i]."','".$_POST['prz'][$i]."','".$_POST['przf'][$i]."','".$_POST['amtwithgsttot'][$i]."')");
