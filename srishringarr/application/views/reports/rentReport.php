...
<td width="102"><?php if ($row[17]=='Rs.') { echo $row[17]."".$row[18]; } else echo $row[18]."".$row[17];?></td>
<td width="144"><?php echo $row[19]; $com_total+=$row[19]; ?></td>
<td width="142"><?php echo $s; $ba+=$s; ?></td>
 <td  align="center" width="104"><a href="rent_report_detail.php?id=<?php echo $row[0]; ?>" target="_new">Bill Detail</a>
 ...
 <?php } ?>
			...
			<?php $i++;	} 
			$sql14=mysql_query("SELECT SUM( rent_amount)  FROM `phppos_rent`  WHERE `cust_id`='$id' and status='A'");
$row14=mysql_fetch_row($sql14);
			?>
            <tr>
