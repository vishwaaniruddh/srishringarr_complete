 ...
 <?php 
      ...
      /*   $result1 = mysql_query("SELECT sum(qty),sum(return_qty) FROM  approval_detail where item_id='$row[0]' and not qty=0");
         $sum=0;
              ...
              $amt=($sum+$row[7])*$row[5];
              $total=$total+$amt;*/
         $result2 = mysql_query("SELECT sum(qty),sum(return_qty) from approval_detail where bill_id in (SELECT bill_id FROM approval where bill_date>'2014-03-31' and status='S') and not qty=0 and item_id='$row[0]'");
