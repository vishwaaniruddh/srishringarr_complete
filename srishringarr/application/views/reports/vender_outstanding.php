     ...
     <form id="purchse" >
       <div id="printdiv">
         <hr/></div>
            ...
            <?php 
			$i=0;
			$qryven=mysql_query("select * from `phppos_suppliers` order by company_name");
			while($resven=mysql_fetch_row($qryven)){
				//echo "hii";
			$qry=mysql_query("SELECT * FROM `vendor_openbal` WHERE `v_id`='$resven[0]'");
			$num=mysql_num_rows($qry);
			$resoutstand=mysql_fetch_row($qry);
			
			$qryout=mysql_query("select sum(`outstanding`) from `phppos_purchase` where supp_id='$resven[0]'");
			$resout=mysql_fetch_row($qryout);
			//echo $resout[0];
