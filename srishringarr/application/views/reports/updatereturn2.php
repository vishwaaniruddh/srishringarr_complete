...
$t1=mysql_query("insert into `paid_amount`(bill_id,amount,return_date,payment_by,amt_of) values('$cid','$amt',STR_TO_DATE('".$bill_date."','%d/%m/%Y'),'$radio2','S')");
}
///echo "update approval set return_date=STR_TO_DATE('".$bill_date."','%d/%m/%Y'),paid_amount=paid_amount+$amt where bill_id='$cid'";


$sq3=mysql_query("select * from approval where cust_id='$cid' and status='S'  Order By bill_id DESC");
...
$sq=mysql_query("SELECT bill_id,qty,return_qty,aid FROM `approval_detail` WHERE item_id='$it[$i]' and  bill_id in (select bill_id from approval where cust_id='$cid' and status='S' ) Order By bill_id DESC");
...
$t5=  mysql_query("insert into `return_qty`(bill_id,qty,return_date,item_code) values('$sqrow[0]','$sum',STR_TO_DATE('".$bill_date."','%d/%m/%Y'),'$it[$i]')");
 $bdate1=mysql_query("select * from approval_detail where bill_id='$sqrow[0]' and item_id='$it[$i]'");
	...
	$qry12="SELECT * FROM  `phppos_items` where name='$it[$i]' ";
