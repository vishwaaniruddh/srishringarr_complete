...
$t1=  mysql_query("update approval set paid_amount=paid_amount+$amt where cust_id='$cid' LIMIT 1");
$t2=mysql_query("insert into `paid_amount`(bill_id,amount,return_date,payment_by) values('$cid','$amt',STR_TO_DATE('".$amt_date."','%d/%m/%Y'),'$pay_by')");

//echo "INSERT INTO `bank_transaction`(`trans_id`, `bank_id`, `trans_type`, `trans_amt`, `trans_date`, `trans_memo`, `reconcile`,`enrty_date`) VALUES ('','".$acc."','reciet','".$amt."',STR_TO_DATE('".$amt_date."','%d/%m/%Y'),'payment from customer $row[0] $row[1]','NO',now())";
$t3=mysql_query("INSERT INTO `bank_transaction`(`trans_id`, `bank_id`, `trans_type`, `trans_amt`, `trans_date`, `trans_memo`, `reconcile`,`enrty_date`) VALUES ('','".$acc."','receit','".$amt."',STR_TO_DATE('".$amt_date."','%d/%m/%Y'),'payment from customer $row[0] $row[1]','NO',now())");
