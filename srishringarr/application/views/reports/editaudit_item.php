...
<img src="bill.png" width="408" height="165"/><br/><br/>
 ...
 <td width="409">Item Code : <input type="text" name="barcode" onFocus="this.value=''" onClick="this.value=''"  id="barcode" onChange="MakeRequest();"/>
      ...
      <td width="372">Audit Date :<?php echo date('d/m/Y',strtotime($_GET['dt'])); ?>
        <br/><br/>
        Category: 
        <?php $sq=mysql_query("SELECT category FROM  `phppos_items` where name in(select item_id from audit where audit_date='".$_GET['dt']."') GROUP BY category");
...
echo $ro[0]; ?>
  ...
  $aud=mysql_query("select * from audit where audit_date='".$_GET['dt']."'");
  while($auro=mysql_fetch_array($aud))
  {
  $sr=$sr=$sr+1;
   $qry=mysql_query("select item_number,name,category,unit_price,quantity from  phppos_items where name='$auro[1]' order by name");
   $suggest=mysql_fetch_row($qry);
