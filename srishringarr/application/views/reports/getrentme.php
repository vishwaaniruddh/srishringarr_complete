...
//echo "select `cust_id`  from `approval` where `bill_date` between '".$frmdate."' and '".$todate."'  group by `cust_id` DESC";
 ...
 //$qryapp=mysql_query("select `cust_id`  from `approval` where `bill_date` between '".$frmdate."' and '".$todate."'  group by `cust_id` DESC");
  ...
  $qryapp = mysql_query("select * from phppos_people where person_id in (select distinct (`cust_id`) from `approval` where `bill_date` between '".$frmdate."' and '".$todate."') order by last_name"); ?>
	 ...
	 ?>
     <tr><td><?php echo $i;?></td>
     <td><?php echo $app[0]."   ". $app[1]; ?> </td><td><?php echo  $app[1]; ?> </td><td ><?php echo $app[2]?></td><td align="right"><?php $qrybal=mysql_query("SELECT qty,return_qty,amount FROM `approval_detail` where `bill_id`in( select `bill_id` from `approval` where `cust_id`='$app[11]' and status='a' and ( `bill_date` between '".$frmdate."' and '".$todate."'))");
