...
<div id="timeline">
	...
	<ul id="dates">
		...
		$bill_idall=mysql_query("select pur_id from phppos_purchase where supp_id='".$_GET['sup_id']."' ");
	...
	$qryid=mysql_query("Select * from phppos_purchase where `supp_id`='".$_GET['sup_id']."'");
	
	while($row1=mysql_fetch_row($qryid)){
     ...
     <table width="70%" border='1' style="color:#000;">
      ...
      <th width="18%">Total Amount</th>
     	...
     	<?php 	 
			...
			$total=0;
			$qty=0;
		$qryfetch=mysql_query("select * from `phppos_purchase_details` where `pur_id`='".$row1[0]."'");		
	 	while($resfetch=mysql_fetch_row($qryfetch)){?> 
     <tr>
