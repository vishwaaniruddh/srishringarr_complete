...
$gtitd=mysql_query("SELECT item_id FROM `phppos_items` WHERE `name`='".$_GET['itnm']."'");
$gtrws=mysql_fetch_array($gtitd);
...
while($restrans=mysql_fetch_row($trans))
{
	...
	// echo "select company_name from `phppos_suppliers` where `person_id`= $restrans[2]";
	   ...
	   $suup_name=mysql_query("select `company_name` from `phppos_suppliers` where `person_id`= '$restrans[2]'");
	   $supp_name=mysql_fetch_row($suup_name);
