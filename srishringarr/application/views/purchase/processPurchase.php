...
$pur_id=mysql_insert_id();
 ...
 // -----------Insert data phppos_items table-----------------
   ...
   $qryitm=mysql_query("INSERT INTO `phppos_items`(`name`, `category`, `supplier_id`, `item_number`, `description`, `cost_price`, `unit_price`, `quantity`) VALUES ('".$myitemid[$i]."-".$item_no[$i]."','".$item_cat[$i]."', '".$supp_id."','".$item_no[$i]."' , '".$bill_date."', '".$cprice[$i]."', '".$uprice[$i]."', '".$qty[$i]."' )");
    }else{
