...
{
	$qrybill=mysql_query("Select * from `phppos_purchase` where pur_id='$billno[$i]'");
	$resbill=mysql_fetch_row($qrybill);
	...
	if($resbill[6]<=$amt)
	{
		...
		$insqry[$i]=mysql_query("INSERT INTO `phppos_purchase_payments`(`trans_id`, `bill_no`, `mode`, `amt`, `paid_date`) VALUES ('','$resbill[0]','$mode','$resbill[6]',STR_TO_DATE('".$paydate."','%d/%m/%Y'))");
		echo $tr=mysql_insert_id();
		if($insqry)
		{
			$amt-=$resbill[6];
			//echo "UPDATE `phppos_purchase` SET `outstanding`='0' WHERE `pur_id`='$billno[$i]'<br/>";
			$qry[$i]=mysql_query("UPDATE `phppos_purchase` SET `outstanding`='0' WHERE `pur_id`='$billno[$i]'");	
		}	
	}
	...
	else if($resbill[6]>$amt)
	{
		...
		$insqry[$i]=mysql_query("INSERT INTO `phppos_purchase_payments`(`trans_id`, `bill_no`, `mode`, `amt`, `paid_date`) VALUES ('','$resbill[0]','$mode','$amt',STR_TO_DATE('".$paydate."','%d/%m/%Y'))");
	    $tr=mysql_insert_id();
		if($insqry)
		{
			$resbill[6]-=$amt;
			//echo "UPDATE `phppos_purchase` SET `outstanding`='$resbill[6]' WHERE `pur_id`='$billno[$i]' <br>";
			$qry[$i]=mysql_query("UPDATE `phppos_purchase` SET `outstanding`='$resbill[6]' WHERE `pur_id`='$billno[$i]'");	
		}	
	}
...
{	//echo $j;
	if(!$qry[$j] and !$qrybill[$j])
	{
			...
			?>
            <div align='center' style='background-color:#987564'> <br/><br/><br/><br/>Transaction aborted. Please Try Again<br/><br/> <a href='view_bills.php'><input type='button' value='BACK'></a>&nbsp<a href='../../../index.php/purchase'> <input type='button' value='EXIT'></a><br><br></div>
            <?php
			 $flag=1;
			 break;	
	}	
...
{
 mysql_query("COMMIT");
mysql_query("INSERT INTO `bank_transaction`(`trans_id`, `bank_id`, `trans_type`, `trans_amt`, `trans_date`, `trans_memo`, `reconcile`,`enrty_date`) VALUES ('','$acc','payment','".$amt1111."',STR_TO_DATE('".$paydate."','%d/%m/%Y'),'payment for supplier ".$supp."','NO',now())");
 ?>
<div align='center' style='background-color:#987564'> <br/><br/><br/><br/>Transaction Completed.<br/><br/> <a href='view_bills.php'><input type='button' value='BACK'></a>&nbsp<a href='../../../index.php/purchase'><input type='button' value='EXIT'></a> &nbsp <button type='button' onclick="printDiv('bill');">Print</button><br/></div>
...
<table  align="center"  bgcolor="#CCCCCC"  border="1">
           ...
           <tr> <td height="26" colspan="2">
            <p>
                                        <strong>Date Of Payment:</strong> <?php echo $paydate; ?>                                         
                                    </p>
           </td></tr>
