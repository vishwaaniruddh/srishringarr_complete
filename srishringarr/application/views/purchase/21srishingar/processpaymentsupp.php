...
for($i=0;$i<count($billno);$i++)
{
	...
	if($resbill[6]<=$amt)
	{
		...
		$insqry[$i]=mysql_query("INSERT INTO `phppos_purchase_payments`(`trans_id`, `bill_no`, `mode`, `amt`, `paid_date`) VALUES ('','$resbill[0]','$mode','$resbill[6]',now())");
	...
	else if($resbill[6]>$amt)
	{
		...
		$insqry[$i]=mysql_query("INSERT INTO `phppos_purchase_payments`(`trans_id`, `bill_no`, `mode`, `amt`, `paid_date`) VALUES ('','$resbill[0]','$mode','$amt',now())");
	    $tr=mysql_insert_id();
		if($insqry)
		{
			$resbill[6]-=$amt;
