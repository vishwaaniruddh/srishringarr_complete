...
from ecomadmin.import_data import dictfetchall
from django.db.models import Q,Sum
# Create your views here.

class site_settingView(TemplateView):
    template_name = 'ecomadmin/site_setting.html'
    #template_name = 'ecomadmin/dashboard1.html'
    successMessage = ''
    ...
    def post(self, request, *args, **kwargs):
        ...
        rseponseData = super(site_settingView, self).get(request, *args, **kwargs)     
        return  rseponseData 
    
    def get_context_data(self, **kwargs):
        context = super(site_settingView, self).get_context_data(**kwargs)
        ...
        today = date.today()
        print(today)
        print("madhjvaiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii")
       

        context['todays_orders_earning'] = Order.objects.filter(created_at__contains=today, status = 'P').aggregate(Sum('total'))
        a=context['todays_orders_earning']
        context['todays_orders_earning'] =a['total__sum']

        context['total_earning'] = Order.objects.filter(Q(status='P')|Q(status='C')|Q(status='D')).aggregate(Sum('total'))
        print(context['total_earning'])
        b=context['total_earning']
        context['total_earning'] =b['total__sum']

        context['cust_total'] = userDetails.objects.filter(is_active=1).count()

        obj = Order.objects.filter(created_at__contains='2018-12-11', status = 'P')
        context['todayso']=obj
        ...
        context['todays_orders_count'] = Order.objects.filter(created_at__contains=today, status = 'P').count()
...
class CustomerProfile(TemplateView):
    template_name = 'ecomadmin/customerProfile.html'
    
    def get_context_data(self, **kwargs):
        context = super(CustomerProfile, self).get_context_data(**kwargs)
        context['pageTitle'] = "Customer Profile"
        userId = self.kwargs['user_id']
        print(userId)
        context['customer'] = get_object_or_404(userDetails, id=userId)
        context['user_details'] = userDetails.objects.filter(id=userId)
        #22-10-2019 start
        # context['orders_count'] = Order.objects.filter(seller=self.request.session.get('userId')).count()
        ...
        # context['cancelled_order_count'] = Order.objects.filter(status='X',id=userId).count()
        #22-10-2019 end
...
class CustomerView(TemplateView):
    template_name = 'ecomadmin/customers.html'

    def get_context_data(self, **kwargs):
        context = super(CustomerView, self).get_context_data(**kwargs)
        context['pageTitle'] = "Orders"
        print(self.request.session.get('userId'))
...
def update_status(request):
    ...
    for items  in order_items:
        ...
        if orderDetails is not None:
            vvv=items.booking_dt
            date_time_obj = datetime.strptime(vvv, '%d/%m/%Y')
            ...
            if orderDetails.status=='C':
	            ...
	            subject, from_email, to = 'Your order is confirmed', 'Srishringarr <info@srishringarr.com>', user_email
                ...
                from django.core.mail import send_mail
                print("maiiiiiiiiiiiiiiiiiiillllllllllllllllllllllllllllll")
                subject, from_email, to = 'Your order is Shipped', 'Srishringarr <info@srishringarr.com>', user_email
                ...
                from django.core.mail import send_mail
                print("maiiiiiiiiiiiiiiiiiiillllllllllllllllllllllllllllll")
                subject, from_email, to = 'Your order is Delivered', 'Srishringarr <info@srishringarr.com>', user_email
                ...
                from django.core.mail import send_mail
                print("maiiiiiiiiiiiiiiiiiiillllllllllllllllllllllllllllll")
                subject, from_email, to = 'Your order is Cancelled', 'Srishringarr <info@srishringarr.com>', user_email
...
# end new code
    ...
    if orderDetails is not None:
        ...
        # vikrant7-11-2019
        update_depo_amt=0
        print(orderDetails.deposite_collect_amount)
        print(type(orderDetails.deposite_collect_amount))
        print("HHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHh")


        deposit_amount=orderDetails.final_deposite
        print(deposit_amount)
        print(type(orderDetails.deposite_collect_amount))
        print("YYYYYYYYYYYY")
        # bal_amount=postdata.get("deposite_collect_amount")
        orderDetails.deposite_collect = postdata.get("deposite_collect")
        update_depo_amt=int(orderDetails.deposite_collect_amount)+int(postdata.get("deposite_collect_amount"))

        
        # orderDetails.deposite_collect_amount = postdata.get("deposite_collect_amount")
        orderDetails.deposite_collect_amount=update_depo_amt

        print(orderDetails.deposite_collect_amount)
        print("YYYYYYYYYYYYYYYYYYYYYYYYYYYYy")
        orderDetails.balance_deposit=int(deposit_amount)-int(orderDetails.deposite_collect_amount)

        print(orderDetails.balance_deposit)
        print("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<")
        # vikrant7-11-2019


        # print("collecttttttttttttttttttttttttttttttttttttttttt")
        # print(orderDetails.deposite_collect)
        id = uuid.uuid1()
        ...
        from django.core.mail import send_mail
        print("maiiiiiiiiiiiiiiiiiiillllllllllllllllllllllllllllll")
        subject, from_email, to = "We've received your deposit", 'Srishringarr <info@srishringarr.com>', user_email
        html_content = render_to_string('ecomsite/deposite_collect_email.html', locals())
        text_content = 'This is an important message.'
        # text_content = strip_tags(html_content)
        # html_content = '<p>This is an <strong>important</strong> message.</p>'
...
def update_deposite_return_entry(request):
    ...
    if orderDetails is not None:
        ...
        orderDetails.return_deposit = postdata.get("deposite_refund_amount")
        ...
        from django.core.mail import send_mail
        print("maiiiiiiiiiiiiiiiiiiillllllllllllllllllllllllllllll")
        subject, from_email, to = 'Your deposit has been refunded!', 'Srishringarr <info@srishringarr.com>', user_email
        html_content = render_to_string('ecomsite/deposite_refund_email.html', locals())
        text_content = 'This is an important message.'
        # html_content = '<p>This is an <strong>important</strong> message.</p>'
        msg = EmailMultiAlternatives(subject, text_content, from_email, [to])
        msg.attach_alternative(html_content, "text/html")
