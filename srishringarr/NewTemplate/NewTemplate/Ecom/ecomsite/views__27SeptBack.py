...
class OrderListView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        cursor.execute(''' SELECT oi.order_id, prd.name, prd.product_image, oi.quantity, oi.subtotal, ord.status
...
def rezorPaySuccess(request):
    ...
    client = razorpay.Client(auth=(RAZORPAY_INFO["api_key"], RAZORPAY_INFO["secret_key"]))  #######31-07-19######
    ...
    import os,inspect
    path = os.path.dirname(os.path.abspath(inspect.getfile(inspect.currentframe()))) + "/error58.txt"
    file = open(path,"w+")  # to append line use a+ instead of w+              
    file.write(str('arehcdjp;lrfjs;l'))
    file.close()
    ...
    if str(payment_response["status"]) == "authorized":
        ...
        if str(payment_response["amount"]) == str(cart_amount_in_paise):
            ...
            import os,inspect
            path = os.path.dirname(os.path.abspath(inspect.getfile(inspect.currentframe()))) + "/error1.txt"
            file = open(path,"w+")  # to append line use a+ instead of w+              
            file.write(str(msg))
            file.close()
            msg.send()
