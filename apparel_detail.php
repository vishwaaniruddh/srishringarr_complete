...
if($productName){
    $getsql = mysqli_query($con,"select * from garment_product where newProductName='".$productName."'");
...
$categorysql = mysqli_query($custom_con,"select * from garment_product where gproduct_id='".$product_id."'");
...
$meta_title = $categorysqlResult['meta_title'] ? $categorysqlResult['meta_title'] : $categorysqlResult['gproduct_name'];
$meta_keywords = $categorysqlResult['meta_keywords'];
$meta_description = $categorysqlResult['meta_description'] ? $categorysqlResult['meta_description'] : $categorysqlResult['gproduct_desc'];
...
$a="SELECT * FROM `product_images_new` WHERE `gproduct_id`='$prid' order by rank";
$ab=mysqli_query($con,$a);
$abc=mysqli_fetch_array($ab);

$ogImage =  'https://www.srishringarr.com/compress_image'. $abc['img_name'] ; 
...
$product_name = $rws['ss_product_name'];
...
if($newbooking && isset($newbooking['pick_date'])){
...
$block_date_json = $mdy_block_date;
    ...
    $sql_query = "select *, CAST(REGEXP_SUBSTR(gproduct_code,'[0-9]+') AS UNSIGNED) as sku from `garment_product` where 
    gproduct_id = '".$product_id."' order by sku desc";
    
    $raw_data = mysqli_query($con,$sql_query);
    $row = mysqli_fetch_array($raw_data);


    $ss_seo_keywords = $row['ss_seo_keywords'] ; 
    
    ?>
    <?

        $deposit = 0;
        $productid = $product_id = $row[0];    
        $prcode=$row[2];
        $sku = $prcode;
        $discount = $row[21];
        $youtube = $row[35];
...
$img_path ='https://srishringarr.com/yn/uploads';
...
.like_h4{
    ...
    .button-container button.lefty {
      position: fixed;
      left: 20px;
    }
    .button-container button.righty {
      position: fixed;
      right: 20px;
    }
    ...
    .button-container button.righty::before {
      content: '→';
    }
  </style>

  <div class="button-container">
    <button class="lefty"></button>
    <button class="righty"></button>
  </div>

<section class="bgwhite p-t-55 p-b-65 ">
		<div>
		    <div>
        		<div style="display:none;"  class="col-sm-12 col-md-12 col-lg-12">
        			<div>
        				<div class="flex-w">
        					<ol class="breadcrumb" style="display:inline-flex">
                    		        ...
                    		        $category = $rws['product_for'] ;  
                    		        
                    		      //  echo "select * from garments where garment_id='".$category."'" ; 
                    		        $gtmctnm=mysqli_query($con,"select * from garments where garment_id='".$category."'");
...
<? }
    ...
    * {
    ...
    <? }else{ ?>
        <a href="<? echo $path; ?>?ixlib=rb-4.0.3&amp;ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&amp;auto=format&amp;fit=crop&amp;w=1200&amp;h=1200&amp;&amp;q=80" class="glightbox" data-glightbox="type: image">
          <img src="<? echo $path; ?>?ixlib=rb-4.0.3&amp;ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&amp;auto=format&amp;fit=crop&amp;w=1800&amp;h=1800&amp;&amp;q=80" alt="<?= $product_name; ?>">
        </a>
    <? }?>
    </div>
        ...
        <div class="sub">
            ...
            while ($rowimg23 = mysqli_fetch_array($qryimg)) {
              ...
              $zoom_img = $path;
                ...
                <div class="img-box">
                    <a href="<? echo $zoom_img; ?>?ixlib=rb-4.0.3&amp;ixid=M3wxMjA3fDB8MHxzZWFyY2h8MjB8fHdvbWVuJTIwc2hvcHBpbmd8ZW58MHx8MHx8fDA%3D&amp;auto=format&amp;fit=crop&amp;w=1200&amp;h=1200&amp;q=80" class="glightbox" data-glightbox="type: image">
                      <img src="<? echo $zoom_img; ?>?ixlib=rb-4.0.3&amp;ixid=M3wxMjA3fDB8MHxzZWFyY2h8MjB8fHdvbWVuJTIwc2hvcHBpbmd8ZW58MHx8MHx8fDA%3D&amp;auto=format&amp;fit=crop&amp;w=900&amp;h=900&amp;q=80" alt="<?= $product_name; ?>">
                    </a>
                </div>
  ...
  $("#overlay").fadeOut("slow");
        				...
        				<div class=" bo9 w-size14 p-t-0 respon5 p-b-0 ">
					        <div class=" p-l-20 p-r-20 p-t-20 p-b-20 mainbox">
        						...
        						<span class="p-b-10" style="color:#E6BE6E;">SKU: <?php echo $rws[2];?></span>
...
.clearfix::after {
    ...
    content: "";
							...
							<div class="fs-15 p-b-5">
                                ...
                                ?>
            									<strong>
            										<span class="fs-15 m-l-10" style="font-size: 18px;" ><? echo $currency_symbol . ' ' ; ?> <?php echo round(currencyAmount($currency,$rero[0]),2); ?> </label> </span>
            									</strong>
                                ...
                                ?>
        								...
        								<tr>
        									<td style="border:0px; padding-bottom: 10px; ">
        								        <span class="fs-15 "  style="color: #555555;"><b>Refundable Deposit</b> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>
        								    </td>
        								    <td style="border:0px; padding-bottom: 10px; "><b>:</b></td>
        									<td style="border:0px; padding-bottom: 10px; " >
            									<strong>
...
<div id="measurementcard" style="display:none;">
						    <hr style="background-color:#e6be6e">
    <strong>Size Extendable upto</strong>
    <div class="" id="measurementContainer" style="display:flex;"></div>
</div>
...
<div class="day_select">
        ...
        <div>
            ...
            <label class="for-checkbox-budget" for="6">
							...
							<?php if($qty > 0){ ?>
							<div class="">
										<input class="btn" type="button" value="Add To Cart For Rent"  id = "add_to_cart" name="add_to_cart"  style="background-color: #353746;color: white; width:100%;"/>
							</div>
							<? }
							else{ ?>
...
<input type="hidden" id="sellingPrice" name="sellingPrice" value="<?= $lastSellingPrice; ?>" />
<input class="btn" type="button" value="Buy Pre-Loved @ <?= $lastSellingPrice; ?>"  id = "add_to_cart_buy" name="add_to_cart_buy"  style="background-color: yellow;color: black;width:100%;border: 1px solid #dbdbdb;"/>
...
<i class="fa fa-info-circle tooltip-item" aria-hidden="true"></i>
...
<script>
	...
	$('#add_to_cart_buy').on('click',function() {
	...
	if($('#dateRangeP').val()==''){
	    ...
	    $.ajax({
           type: 'POST',
            url:'/addcart_process_selling.php',
...
$(document).on('click', '.lefty', function() {
  var currentProductId = '<? echo $prid; ?>';

  // API endpoint URL
  var apiUrl = "/getApparelProductsSameCategory.php?id=<? echo $category; ?>";
  ...
  $.ajax({
    ...
    success: function(responseData) {
      if (responseData && responseData.length > 0) {
        ...
        var currentIndex = responseData.findIndex(function(product) {
          return product.product_id === currentProductId;
        });

        // Check if the current product is found in the JSON data
        if (currentIndex !== -1) {
          // Check if there is a previous product
          if (currentIndex > 0) {
            var previousProductId = responseData[currentIndex - 1].product_id;
            var link = responseData[currentIndex - 1].link;
          ...
          console.log("Current product not found in the JSON data.");
        }
      } else {
...
console.log(apiUrl);
  ...
  $.ajax({
    ...
    success: function(responseData) {
      if (responseData && responseData.length) {
        ...
        // Check if the current product is found in the JSON data
        if (currentIndex !== -1) {
          // Check if there is a next product
          if (currentIndex < responseData.length - 1) {
            var nextProductId = responseData[currentIndex + 1].product_id;
            var link = responseData[currentIndex + 1].link;
  ...
  $(document).ready(function() {
    var container = $('#flexslider-container_selectors');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    fetch('fetch_measurements.php?product_id=<?php echo $productid; ?>')
        .then(response => response.json())
        .then(data => {
            var measurementContainer = document.getElementById("measurementContainer");

            data.forEach((item) => {
                var measurementBox = document.createElement('div');
                measurementBox.className = 'a'; // Adjust the column size as needed
                measurementBox.innerHTML = `

                        <div class="measurement_box">
                            <p class="measurement_box_title" style="font-weight:600;white-space: nowrap;">${item.measure_name} (In)</p>
                            <span class="measurement_box_text">${item.measure_value}</span>
                        </div>
                `;
                measurementContainer.appendChild(measurementBox);
            });
            $("#measurementcard").css('display','block');
        });
});
</script>
...
<div id="like_history">
    ...
    <div id="recommendations" class="flexslider row"></div>
</div>



<h2 style="text-align:center;">From same Category</h2>
    <div id="loading-spinner" class="loading-spinner"></div>
    <div class="container">
        <div id="flexslider-container" class="flexslider row category_slider"></div>        
    </div>
</div>
...
<script>
$(document).ready(function() {
console.log('product Id = ' + '<?php echo $prid ; ?>')
...
$(document).ready(function() {
  ...
  $.ajax({
    url: '/like_apparel.php?id=<? echo $category; ?>', // Update the URL with the correct endpoint
    dataType: 'json',
    success: function(data) {
      if (data && data.length > 0) {
            ...
            for (var i = 0; i < products.length; i++) {
              ...
              html += '<li class="product-slide" style="width: 210px; margin-right: 4px; float: left; display: block;">';
            ...
            var container = $('#flexslider-container');
            container.html(html);
...
function checkdelivery(){
    ...
    if (deliverypincode.length==6) {
      ...
      $.ajax({
    url: '/history_apparel.php?sku=<? echo $sku; ?>', // Update the URL with the correct endpoint
    dataType: 'json',
    success: function(history_data) {
      // Step 1: Parse the JSON data
      var history_products = history_data;

      // Step 2: Generate HTML structure for each product
      var html2 = '';
      for (var i = 0; i < history_products.length; i++) {
        var product = history_products[i];

        // Create the html2 structure for each product
        html2 += '<div class="col-xs-6 col-md-4 col-lg-3 ftco-animate fadeInUp ftco-animated product_col">';
        html2 += '<div class="product">';
        html2 += '<a href="/' + product.link + '" class="img-prod">';
        html2 += product.imgframe;
        html2 += '</a>';

        html2 += '<div class="text py-3 px-3">';
        html2 += '<h3>';
        html2 += '<a href="/' + product.link + '">' + product.product_name + '</a>';
        html2 += '</h3>';
        html2 += '<hr style="margin-top: 0rem; margin-bottom: 0.3rem;">';
        html2 += '<div class="subpart">';
        html2 += '<p style="display:block;">MRP ₹ ' + product.mrp + '</p>';
        html2 += '<h6 style="color:red; font-size: 13px; text-decoration: underline; font-weight: 700;">SKU : <a href="/' + product.link + '">' + product.sku + '</a></h6>';
        html2 += '<div class="d-flex">';

        html2 += '<div class="pricing">';
        html2 += '<p class="price">';
        html2 += '<span class="mr-2 price-dc">Rent ₹ <strong>' + product.addedRentPrice + '</strong> for 3 Days</span>';
        html2 += '<br>';
        html2 += '<span class="price-sale">Refundable Deposit ₹ <strong>' + product.deposit + '</strong></span>';
        html2 += '</p>';
        html2 += '</div>';

        html2 += '<div class="rating">';
        html2 += '<p class="text-right">';
        html2 += '<a href="#"><span class="ion-ios-star-outline"></span></a>';
        html2 += '<a href="#"><span class="ion-ios-star-outline"></span></a>';
        html2 += '<a href="#"><span class="ion-ios-star-outline"></span></a>';
        html2 += '<a href="#"><span class="ion-ios-star-outline"></span></a>';
        html2 += '<a href="#"><span class="ion-ios-star-outline"></span></a>';
      ...
      // Step 3: Append the html2 to the container element
      var container2 = $('#recommendations');
      container2.html(html2);

      // Step 4: Initialize the Flexslider
