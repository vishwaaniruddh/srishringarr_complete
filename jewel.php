...
$categorysql = mysqli_query($custom_con,"select * from jewel_subcat where subcat_id='".$product_id."'");
...
$meta_keyword = $categorysqlResult['meta_keyword'];
   ...
   <div class="row" style="display: flex;
    justify-content: end;">
      ...
      <?
        ...
        $typeFilter = ($_REQUEST['typeFilter'] ? $_REQUEST['typeFilter'] : 'addedRentPrice');
      ...
      <select class="form-control"
         ...
         <option value="jewel.php?id=<?php echo $product_id; ?>&type=1&pricefilter=1" <?php if
            (isset($_GET['pricefilter']) && $_GET['pricefilter']==1) { echo 'selected' ; } ?>>Rent - Higher To Lower
         </option>
         <option value="jewel.php?id=<?php echo $product_id; ?>&type=1&pricefilter=2" <?php if
            (isset($_GET['pricefilter']) && $_GET['pricefilter']==2) { echo 'selected' ; } ?>> Rent - Lower To Higher
         </option>

         <option value="jewel.php?id=<?php echo $product_id; ?>&type=1&pricefilter=3" <?php if
            (isset($_GET['pricefilter']) && $_GET['pricefilter']==3) { echo 'selected' ; } ?>>Sell - Higher To Lower
         </option>
         <option value="jewel.php?id=<?php echo $product_id; ?>&type=1&pricefilter=4" <?php if
            (isset($_GET['pricefilter']) && $_GET['pricefilter']==4) { echo 'selected' ; } ?>>Sell - Lower To Higher
         </option>
...
<div id="loadingOverlay" class="center" style="visibility: visible;text-align: center;">
   <img class=" ls-is-cached lazyloaded" data-src="assets/loader.gif" loading="lazy" src="assets/loader.gif"
      style="width:200px;">
</div>
...
<div class="container">
   <div class="row">
      ...
      <aside id="velaSidebar" class="col-sm-2 filterTagFullwidthContent sidebarLeft velaSidebar"
         style="display: none;">
...
function updateURL(select) {
    ...
    var pricefilter = '<?php echo $pricefilter; ?>'; // Assuming this is a string
    var typeFilter = selectedOption;

    var redirectURL = '?id=' + product_id + '&type=2&pricefilter=' + pricefilter + '&typeFilter=' + typeFilter;
   ...
   $(document).ready(function () {
      ...
      $.ajax({
         url: "jewel1.php?id=<?php echo $product_id; ?>&type=2&pricefilter=<? echo $pricefilter; ?>&minPrice=<? echo $minPrice; ?>&maxPrice=<? echo $maxPrice; ?>&typeFilter=<? echo $typeFilter; ?>",
         dataType: 'json',
         success: function (response) {
            ...
            } else if(typeFilter == 'lastSellingPrice') {
               var minLastSellingPrice = response.minLastSellingPrice;
               var maxLastSellingPrice = response.maxLastSellingPrice;

                var selectedminAddedSellPrice = '<? echo $minPrice; ?>';
                        if (!selectedminAddedSellPrice) {
                           selectedminAddedSellPrice = minLastSellingPrice;
                        }
            
                        var selectedmaxAddedSellPrice = '<? echo $maxPrice; ?>';
            
                        if (!selectedmaxAddedSellPrice) {
                           selectedmaxAddedSellPrice = maxLastSellingPrice;
                        }
            ...
            function updateActivePageLink(page) {
               ...
               $('#paginationContainer .page-link').eq(page - 1).addClass('active');
            }
            ...
            function setFiltersRange(minAddedRentPrice, maxAddedRentPrice) {

               htmlRent = `
               
               <div style="margin: 10px auto;">
                       <select class="form-control" name="typeFilter" onchange="updateURL(this)">
                       <option value="">Select</option>
                        <option value="addedRentPrice">Rent</option>
                        <option value="lastSellingPrice">Sell</option>
                    </select>
               </div>
                    ...
                    <form method="GET" action="jewel.php" id="rangefilterForm" style="">
                        ...
                        <div style="display:flex;">
                            <input type="text" id="minPrice" name="minPrice" value="${minAddedRentPrice}" placeholder="min price" style="width: 50%;">
                            <input type="text" id="maxPrice" name="maxPrice" value="${maxAddedRentPrice}" placeholder="max price" style="width: 50%;">
                        </div>
            ...
            updateActivePageLink(currentPage);
              if (typeFilter == 'addedRentPrice') {
                    setFiltersRange(minAddedRentPrice, maxAddedRentPrice);

            } else {
                            setFiltersRange(minLastSellingPrice, maxLastSellingPrice);
            }
         }
      });
   });
