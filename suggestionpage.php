...
// echo $type;
$sug_test = array();
$sug_sql = mysqli_query($con,"select * from recommended_suggestion where prod_type='".$type."'");
while($sug_result = mysqli_fetch_assoc($sug_sql)){
    $sug[] = json_decode($sug_result['product_sku']);   
    $prodid[]=$sug_result['product_id'];
    // array_push($sug_test,json_decode($sug_result['product_sku']));
//  echo $sug_result['product_sku']."<br/>";
}
...
if($searchText){
                ...
                if($type==2){
                    $Apparel= mysqli_query($conn,"SELECT g.*,gp.* FROM `garments` g left join  garment_product gp on g.garment_id = gp.product_for WHERE g.name like '%".$searchText."%' or gp.gproduct_code in(".$imp.")");
               ...
               {$Jewellery=mysqli_query($conn,"SELECT j.categories_name,j.subcat_id as m_category,js.name,js.subcat_id as sub_cat,p.* from jewel_subcat j join subcat1 js on j.subcat_id=js.maincat_id join product p on js.subcat_id = p.subcat_id where j.categories_name like '%".$searchText."%' or js.name like '%".$searchText."%' or p.product_code in (".$imp.") or p.product_name in(".$imp.")");
